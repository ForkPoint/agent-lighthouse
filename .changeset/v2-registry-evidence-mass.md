...
v2 registry: 8 categories, evidence-mass overall score, enforced slug ids.
...
**Breaking: `AuditMetaSchema` enforces the v2 contract.** `evidenceGrade`, `tier` and `dossier` are required, and `id` must match `/^[a-z-]+\/[a-z0-9-]+$/` (`category/slug`). Numeric v1 ids no longer validate.
