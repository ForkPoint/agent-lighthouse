...
const page = (slug: string) =>
  readFileSync(resolve(DIST, "docs", slug, "index.html"), "utf8");
...
describe.skipIf(!built)("rendered docs pages", () => {
  ...
  it("renders the source each section names, not an empty page", () => {
    for (const section of DOC_SECTIONS) {
      ...
      expect(
        identifier.length,
        `${section.slug}: nothing distinctive to look for`,
      ).toBeGreaterThan(11);
  ...
  it("gives every page exactly one h1", () => {
    for (const section of DOC_SECTIONS) {
      ...
      expect(
        headings,
        `${section.slug} has the wrong number of h1s`,
      ).toHaveLength(1);
    }
  });
  ...
  it("shows no escaped markup where the source wrote real HTML", () => {
    for (const section of DOC_SECTIONS) {
      expect(article(page(section.slug)), section.slug).not.toMatch(
        /&lt;\/?(?:div|p|a|img|strong)\b/,
      );
    }
  });
  ...
  it("lists every section in the sidebar and marks the one you are on", () => {
    for (const section of DOC_SECTIONS) {
      ...
      for (const other of DOC_SECTIONS) {
        expect(html, `${section.slug} does not link ${other.slug}`).toContain(
          `href="${docPath(other.slug)}"`,
        );
      }
      ...
      const sidebars =
        html.match(/<nav aria-label="Documentation"[\s\S]*?<\/nav>/g) ?? [];
      ...
      for (const sidebar of sidebars) {
        const current = [
          ...sidebar.matchAll(/<a\b[^>]*aria-current="page"[^>]*>/g),
        ].map((m) => m[0]);
        expect(current, section.slug).toHaveLength(1);
        expect(current[0], section.slug).toContain(
  ...
  it("leaves no unresolved relative link in the prose", () => {
    for (const section of DOC_SECTIONS) {
      const hrefs = [
        ...article(page(section.slug)).matchAll(/href="([^"]+)"/g),
      ].map((m) => m[1]!);
      for (const href of hrefs) {
        expect(
          href,
          `${section.slug} keeps a repository-relative link`,
