...
describe("chrome", () => {
  it("never hardcodes a site-absolute href", () => {
    ...
    for (const file of astroFiles(SRC)) {
      ...
      expect(offenders.length, `${file} hardcodes a root-relative href`).toBe(
        0,
      );
    }
  });
  ...
  it("renders at most the one conditional heading, and none in the shell", () => {
    expect(template("layouts/Doc.astro")).toMatch(
...
describe.skipIf(!built)("rendered dossier pages", () => {
  const pages = () =>
    readdirSync(resolve(DIST, "audits"), { withFileTypes: true })
      ...
      .flatMap((category) =>
        readdirSync(resolve(DIST, "audits", category.name), {
          withFileTypes: true,
        })
          .filter((entry) => entry.isDirectory())
          .map((slug) => {
  ...
  it("gives every page exactly one h1", () => {
    ...
    for (const page of all) {
      expect(
        page.html.match(/<h1[\s>]/g) ?? [],
        `${page.id} has the wrong number of h1s`,
      ).toHaveLength(1);
    }
  });
  ...
  it("marks the page you are on as current, and nothing else", () => {
    for (const page of pages()) {
      const current = [
        ...page.html.matchAll(/<a\b[^>]*aria-current="page"[^>]*>/g),
      ].map((m) => m[0]);
  ...
  it("keeps navigation reachable below the sidebar breakpoint", () => {
    ...
    expect(page!.html.match(/aria-label="Documentation"/g) ?? []).toHaveLength(
      2,
    );
  ...
  it("links the favicon at a path the build actually emits", () => {
    ...
    expect(
      existsSync(resolve(DIST, href!.replace("/agent-lighthouse/", ""))),
    ).toBe(true);
  });
});
...
describe.skipIf(!built)("head metadata", () => {
  ...
  const meta = (html: string, name: string) =>
    new RegExp(`<meta (?:property|name)="${name}" content="([^"]*)"`).exec(
      html,
    )?.[1];
  ...
  it("carries the full Open Graph and Twitter card on every page", () => {
    ...
    for (const page of htmlPages()) {
      ...
      for (const [name, value] of Object.entries(expected)) {
        expect(decode(meta(html, name) ?? ""), `${page.file}: ${name}`).toBe(
          value,
        );
      }
