...
describe("sourceTypeCounts", () => {
  it("counts every type present and nothing else", () => {
    ...
    expect(counts.reduce((total, entry) => total + entry.count, 0)).toBe(
...
describe("the policy source", () => {
  ...
  it("leaves no relative link unresolved", () => {
    const hrefs = [...readPolicySource().matchAll(/\]\(([^)]+)\)/g)].map(
      (match) => match[1]!,
    );
    ...
    for (const href of hrefs) {
      ...
      expect(resolved, `${href} is still relative`).toMatch(
        /^(?:https?:|mailto:|#|\/)/,
      );
    }
  });
  ...
  it("sends the registry the policy cites to the browsable sources page", () => {
    expect(resolveDocLink("./sources.json", EVIDENCE_DIR, new Set())).toBe(
      "/agent-lighthouse/sources/",
    );
  });
});
