...
describe("resolveDocLink", () => {
  ...
  it("sends a sibling docs link to its page and keeps the fragment", () => {
    expect(resolveDocLink("./cli.md#--experimental", "docs", published)).toBe(
      "/agent-lighthouse/docs/cli/#--experimental",
    );
  });

  it("sends the evidence policy to the published policy page", () => {
    expect(resolveDocLink("./evidence/policy.md", "docs", published)).toBe(
      "/agent-lighthouse/policy/",
    );
  });
  ...
  it("sends a published dossier to its page", () => {
    expect(
      resolveDocLink(
        "./evidence/audits/structured-data/service-schema.md",
        "docs",
        published,
      ),
  ...
  it("sends a source file to GitHub", () => {
    expect(
      resolveDocLink("../packages/core/src/scorer.ts", "docs", published),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/packages/core/src/scorer.ts",
    );
  });

  it("sends a markdown file this site does not publish to GitHub", () => {
    expect(resolveDocLink("docs/media-kit.md", "", published)).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/docs/media-kit.md",
    );
  });

  it("sends a directory to GitHub as a tree URL", () => {
    expect(resolveDocLink("docs/assets/", "", published)).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/tree/main/docs/assets",
    );
  });

  it("leaves an absolute link and a bare fragment alone", () => {
    expect(resolveDocLink("https://example.com/", "docs", published)).toBe(
      "https://example.com/",
    );
    expect(resolveDocLink("#fixed-limits", "docs", published)).toBe(
      "#fixed-limits",
    );
  });
});
...
describe("createDocRenderer", () => {
  it("rewrites the relative links the prose was written with", async () => {
    ...
    const { html } = await render(
      "See [the CLI](./cli.md) and [the policy](./evidence/policy.md).",
      "docs",
    );
  ...
  it("still escapes raw HTML on the dossier pipeline, which keeps that plugin", async () => {
    ...
    const { code } = await processor.render(
      '<div align="center">tags in prose</div>',
      {},
    );
