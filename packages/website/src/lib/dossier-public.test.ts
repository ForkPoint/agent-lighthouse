...
describe("publicDossier", () => {
  ...
  describe("supersede", () => {
    it("keeps the dated evidence section and withholds the placeholder above it", () => {
      ...
      expect(result.markdown).not.toContain(
        "No dedicated evidence signal was researched",
      );
  ...
  describe("labelled blocks", () => {
    ...
    it("names the signal each promoted block came from when there are several", () => {
      const result = publicDossier(
        read("agent-interfaces/webmcp-registered-tools"),
      );
  ...
  describe("compaction", () => {
    ...
    it("keeps each date when the sources were verified on different days", () => {
      ...
      expect(markdown).toContain(
        "[A](https://a.example) (verified 2026-08-20)",
      );
      expect(markdown).toContain(
        "[B](https://b.example) (verified 2026-08-21)",
      );
    });
  ...
  describe("voice and shape", () => {
    it("breaks a paragraph that has run past 120 words", () => {
      const sentence =
        "The vendor documents the behaviour for a named agent and states the consequence. ";
      const { markdown } = publicDossier(
    ...
    it("never breaks inside a citation or a quotation", () => {
      const filler =
        "The vendor documents this behaviour for one named agent today. ";
  ...
  describe("frontmatter overrides", () => {
    it("publishes a section named by public_extra, after the contract", () => {
      ...
      const result = publicDossier(source, {
        publicExtra: ["The GEO-benchmark rebuild"],
      });
      expect(result.published).toEqual([
        "What it checks",
        "The GEO-benchmark rebuild",
  ...
  describe("the whole corpus", () => {
    ...
    it("never publishes the internal process vocabulary", () => {
      ...
      for (const id of ids) {
        ...
        for (const phrase of banned)
          expect(markdown, `${id}: ${phrase}`).not.toContain(phrase);
      }
    });
    ...
    it("publishes no research scaffolding and no heading too long to be a label", () => {
      ...
      for (const id of ids) {
        const { markdown } = publicDossier(read(id));
        expect(markdown, `${id}: falsifiability protocol`).not.toMatch(
          /FALSIFIABLE\s+(TEST|FORM)/,
        );
        expect(markdown, `${id}: Evidence label`).not.toContain(
          "**Evidence:**",
        );
        expect(markdown, `${id}: sources run-on`).not.toMatch(
          /^\*\*Sources:\*\*.*·.*·.*·.*·/m,
        );
        // A tag outside a code span renders as a run of escaped angle brackets.
        const outsideCode = markdown.replace(/```[\s\S]*?```|`[^`]*`/g, " ");
        expect(outsideCode, `${id}: bare tag`).not.toMatch(
          /<\/?[a-z][a-z0-9]*[ />]/,
        );
    ...
    const PASTED_CODE =
      /\b(?:if|return|const|let|function)\s*\(?[^\n]{0,50}(?:===|=>|\)\s*(?:;|\{))/;
    ...
    it("prints its sections in the page-contract order", () => {
      for (const id of ids) {
        ...
        const ranked = contract.map((name) =>
          (PAGE_ORDER as readonly string[]).indexOf(name),
        );
        expect(
          [...ranked].sort((a, b) => a - b),
          id,
