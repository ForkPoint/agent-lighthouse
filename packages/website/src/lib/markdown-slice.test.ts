...
describe("DOC_SECTIONS", () => {
  it("names a real file for every section", () => {
    for (const section of DOC_SECTIONS) {
      expect(
        () => readFileSync(resolve(REPO, section.file), "utf8"),
        section.slug,
      ).not.toThrow();
    }
  });
  ...
  it("finds every heading it slices", () => {
    for (const section of DOC_SECTIONS) {
      ...
      expect(
        () => sliceSection(source, section.heading!),
...
describe("readDocSource", () => {
  it("gives every section prose to render", () => {
    for (const section of DOC_SECTIONS) {
      expect(
        readDocSource(section).trim().length,
        `${section.slug} renders nothing`,
      ).toBeGreaterThan(80);
    }
  });

  it("slices the section the heading names, and nothing after it", () => {
    const quickstart = readDocSource(
      DOC_SECTIONS.find((s) => s.slug === "quickstart")!,
    );
