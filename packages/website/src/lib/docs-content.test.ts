...
describe("docs/scoring.md", () => {
  it("states the weight law and every tier", () => {
    ...
    for (const tier of ["scored", "informative", "experimental"])
      expect(scoring).toContain(tier);
    for (const grade of ["A", "B", "C", "D"])
      expect(scoring).toMatch(new RegExp(`\\b${grade}\\b`));
  });
});
...
describe("docs/cli.md", () => {
  it("documents every flag the CLI accepts", () => {
    ...
    const flags = [...source.matchAll(/["']--([a-z-]+)["']/g)].map(
      (m) => `--${m[1]}`,
    );
    expect(flags.length).toBeGreaterThan(0);
    for (const flag of new Set(flags))
      expect(cli, `undocumented flag ${flag}`).toContain(flag);
  });
});
...
describe("docs/config.md", () => {
  it("names every category id", () => {
    ...
    for (const category of defaultConfig.categories)
      expect(config).toContain(category.id);
  });
});
