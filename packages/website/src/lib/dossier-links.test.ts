...
describe("resolveDossierLink", () => {
  ...
  it("sends an unpublished dossier to GitHub", () => {
    expect(
      resolveDossierLink(
        "../../merged/agentic-commerce/offer-dom-price-parity.md",
        "agentic-commerce/offer-truth-consistency",
        published,
      ),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/docs/evidence/merged/agentic-commerce/offer-dom-price-parity.md",
    );
  });
  ...
  it("sends a repo path outside docs to GitHub", () => {
    expect(
      resolveDossierLink(
        "../../../../packages/core/src/audits/rework-todo.md",
        "access-crawl-control/robots-directives",
        published,
      ),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/packages/core/src/audits/rework-todo.md",
    );
  });
  ...
  it("leaves a mailto link alone", () => {
    expect(resolveDossierLink("mailto:hi@example.com", "a/b", published)).toBe(
  ...
  it("keeps the fragment when the target falls through to GitHub", () => {
    expect(
      resolveDossierLink(
        "../../merged/agentic-commerce/offer-dom-price-parity.md#why",
        "agentic-commerce/offer-truth-consistency",
        published,
      ),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/docs/evidence/merged/agentic-commerce/offer-dom-price-parity.md#why",
    );
  });
  ...
  it("sends a directory link to GitHub as a tree URL", () => {
    expect(
      resolveDossierLink(
        "../../merged/access-crawl-control/",
        "agentic-commerce/offer-truth-consistency",
        published,
      ),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/tree/main/docs/evidence/merged/access-crawl-control",
    );
  });
  ...
  it("sends a dossier that has no page to GitHub even though it sits under audits/", () => {
    expect(
      resolveDossierLink(
        "./not-a-real-dossier.md",
        "structured-data/service-schema",
        published,
      ),
    ).toBe(
      "https://github.com/ForkPoint/agent-lighthouse/blob/main/docs/evidence/audits/structured-data/not-a-real-dossier.md",
    );
  });
});
...
describe("dossierLinksPlugin", () => {
  it("rewrites every link in a dossier and leaves absolute ones alone", async () => {
    ...
    expect(html).toContain(
      'href="/agent-lighthouse/audits/structured-data/service-schema/"',
    );
