...
const event = (over: Omit<Partial<KeyboardEvent>, 'target'> & { target?: unknown }) =>
...
describe('searchShortcut, on the cases the brief does not name', () => {
  it('opens from inside a field on the chord, where a slash would not', () => {
    expect(searchShortcut(event({ key: 'k', metaKey: true, target: { tagName: 'INPUT' } }))).toBe(true);
    expect(searchShortcut(event({ target: { tagName: 'TEXTAREA' } }))).toBe(false);
  });

  it('leaves a contenteditable alone', () => {
    expect(searchShortcut(event({ target: { tagName: 'DIV', isContentEditable: true } }))).toBe(false);
    expect(searchShortcut(event({ target: { tagName: 'DIV', isContentEditable: false } }))).toBe(true);
  });
...
describe('excerptSegments', () => {
  ...
  it('keeps every mark, not just the first', () => {
    ...
    expect(segments.filter((segment) => segment.mark).map((segment) => segment.text)).toEqual(['a', 'b']);
  });

  it('returns one plain run for an excerpt with no match in it', () => {
    expect(excerptSegments('nothing marked here')).toEqual([{ text: 'nothing marked here', mark: false }]);
  });
  ...
  it('drops every tag but the marks, so no markup can survive into the DOM', () => {
    ...
    const segments = excerptSegments('a <img src=x onerror=alert(1)> <mark>b<script>evil()</script></mark>');
  ...
  it('decodes the references Pagefind escapes its text with', () => {
    expect(excerptSegments('&lt;head&gt; &amp; &quot;body&quot; &#39;x&#39; &#x2014;')).toEqual([
      { text: '<head> & "body" \'x\' —', mark: false },
    ]);
  });
...
describe('resultHref', () => {
  it('puts Pagefind’s site-absolute url under the base path', () => {
    ...
    expect(resultHref('/audits/agentic-commerce/offer-truth-consistency/')).toBe(
      '/agent-lighthouse/audits/agentic-commerce/offer-truth-consistency/',
    );
  });

  it('does not prefix a url that is already based', () => {
    expect(resultHref('/agent-lighthouse/docs/quickstart/')).toBe('/agent-lighthouse/docs/quickstart/');
  });
});
