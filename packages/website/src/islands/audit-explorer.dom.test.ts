 ...
 */
const CARDS = [
  { id: 'agentic-commerce/offer-truth-consistency', category: 'agentic-commerce', tier: 'scored', title: 'Offer Truth Consistency', tags: 'price offer' },
  { id: 'access-crawl-control/robots-directives', category: 'access-crawl-control', tier: 'informative', title: 'Robots Directives', tags: 'robots' },
  { id: 'access-crawl-control/llms-txt', category: 'access-crawl-control', tier: 'experimental', title: 'LLMs Txt', tags: '' },
];
...
const pill = (kind: string, value: string) =>
  document.querySelector<HTMLElement>(`[data-filter="${kind}"][data-value="${value}"]`)!;
...
describe('mountExplorer', () => {
  ...
  it('reveals the controls it has just wired up', () => {
    expect(document.querySelector<HTMLElement>('#audit-controls')!.hidden).toBe(false);
  });
  ...
  it('hides the cards a search does not match, and counts what is left', () => {
    ...
    const gone = document.querySelector<HTMLElement>('[data-audit-id="agentic-commerce/offer-truth-consistency"]')!;
  ...
  it('moves the pressed state to the clicked pill and off its siblings', () => {
    ...
    expect(pill('category', 'access-crawl-control').getAttribute('aria-pressed')).toBe('true');
  ...
  it('shows the empty state only while nothing matches', () => {
    const empty = () => document.querySelector<HTMLElement>('#audit-empty')!.hidden;
