 ...
 */
const BADGE_DOC = readFileSync(
  resolve(__dirname, "../../../../docs/badge.md"),
  "utf8",
);
...
describe("docs/badge.md", () => {
  it("is the source of the bands the site renders", () => {
    const rows = [
      ...BADGE_DOC.matchAll(
        /^\|\s*(\d+)-(\d+)\s*\|\s*`([0-9a-f]{6})`\s*\|\s*([^|]+?)\s*\|$/gm,
      ),
    ];

    expect(rows, "the score table moved or changed shape").toHaveLength(
      BADGE_BANDS.length,
    );
    expect(
      rows.map((row) => ({
  ...
  it("publishes the exact snippet the generator produces", () => {
    const documented = /^\[!\[Agent Lighthouse\]\((\S+)\)\]\((\S+)\)$/m.exec(
      BADGE_DOC,
    );
    expect(
      documented,
      "the doc no longer shows a markdown badge",
    ).not.toBeNull();
