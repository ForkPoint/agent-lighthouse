...
describe('mountReportViewer', () => {
  ...
  it('accepts the same file dropped on the zone', async () => {
    ...
    expect(event.defaultPrevented, 'the browser would navigate away otherwise').toBe(true);
    expect(output().hidden).toBe(false);
    expect(output().textContent).toContain('74/100');
  });
  ...
  it('never lets report text become markup', async () => {
    ...
    expect(output().querySelector('img'), 'the report opened an element').toBeNull();
  ...
  it('swaps the border colour while a file is over the zone', () => {
    ...
    expect(over.defaultPrevented, 'the browser would open the file instead').toBe(true);
