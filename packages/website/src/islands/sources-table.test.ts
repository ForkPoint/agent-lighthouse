...
const record = (
  over: Partial<SourceRecord> & { id: string },
): SourceRecord => ({
  title: "A source",
...
describe("filterSources", () => {
  ...
  it("matches the id and the key findings, which is where the detail lives", () => {
    ...
    expect(filterSources(sources, "gptbot", "all")[0]?.id).toBe(
      "vercel-crawler-study",
    );
  });
  ...
  it("filters by type", () => {
    ...
    expect(filterSources(sources, "", "study")[0]?.id).toBe(
      "vercel-crawler-study",
    );
  });
...
describe("the registry on disk", () => {
  ...
  it("gives every entry the fields the table renders", () => {
    for (const source of raw.sources) {
      ...
      expect(
        source.verified,
        `${source.id} verified is not an ISO date`,
      ).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  ...
  it("carries no duplicate ids, because a dossier cites one", () => {
    expect(new Set(raw.sources.map((source) => source.id)).size).toBe(
      raw.sources.length,
    );
  });
  ...
  it("yields the facet list the page builds its filter from", () => {
    ...
    expect(new Set(raw.sources.map((source) => source.type)).size).toBe(
      types.length,
    );
  });
