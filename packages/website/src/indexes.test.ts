...
describe.skipIf(!built)('rendered /audits/', () => {
  ...
  it('offers one pressed pill per filter group, over every category and tier', () => {
    ...
    const pills = (kind: string) =>
      [...html.matchAll(new RegExp(`<button[^>]*data-filter="${kind}"[^>]*>`, 'g'))].map((m) => m[0]);
    const values = (kind: string) =>
      pills(kind).map((pill) => /data-value="([^"]+)"/.exec(pill)?.[1]);

    expect(values('category')).toEqual(['all', ...categoryList().map((category) => category.id)]);
    expect(values('tier')).toEqual(['all', 'scored', 'informative', 'experimental']);
    for (const kind of ['category', 'tier']) {
      // Every pill carries a pressed state, and exactly one starts pressed.
      expect(pills(kind).every((pill) => pill.includes('aria-pressed='))).toBe(true);
      expect(pills(kind).filter((pill) => pill.includes('aria-pressed="true"'))).toHaveLength(1);
    }
  });
  ...
  it('keeps the registry out of the browser bundle', () => {
    ...
    expect(scripts.some(([, code]) => code.includes('#audit-search'))).toBe(true);
...
describe.skipIf(!built)('rendered category indexes', () => {
  ...
  it('publishes one page per category and nothing else', () => {
    const dirs = readdirSync(resolve(DIST, 'categories'), { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .map((entry) => entry.name);

    expect(dirs.sort()).toEqual(categories.map((category) => category.id).sort());
  });
  ...
  it('lists exactly its own audits, in registry order', () => {
    ...
    for (const category of categories) {
      ...
      const own = audits.filter((audit) => audit.category === category.id).map((a) => a.id);
