...
describe.skipIf(!built)("rendered /policy/", () => {
  ...
  it("resolves the one relative link the policy carries", () => {
    const hrefs = [...article(html()).matchAll(/href="([^"]+)"/g)].map(
      (match) => match[1]!,
    );

    expect(hrefs.length, "the policy renders no links at all").toBeGreaterThan(
      0,
    );
    for (const href of hrefs) {
      expect(href, `${href} is still repository-relative`).toMatch(
        /^(?:https?:|mailto:|#|\/)/,
      );
    }
  ...
  it("carries the documentation furniture the layout promises", () => {
    ...
    for (const section of DOC_SECTIONS)
      expect(rendered).toContain(`href="${docPath(section.slug)}"`);
    expect(rendered).toContain(`href="${withBase("sources/")}"`);
  });

  it("marks itself current in the sidebar and in the header", () => {
    const current = [
      ...body(html()).matchAll(/<a\b[^>]*aria-current="page"[^>]*>/g),
    ].map((m) => m[0]);

    expect(current.length).toBeGreaterThan(0);
    for (const anchor of current)
      expect(anchor).toContain(`href="${withBase("policy/")}"`);
  });
});
...
describe.skipIf(!built)("rendered /sources/", () => {
  ...
  it("offers one pressed pill per facet the registry actually contains", () => {
    const pills = [
      ...html().matchAll(/<button[^>]*data-filter="type"[^>]*>/g),
    ].map((m) => m[0]);
    const values = pills.map((pill) => /data-value="([^"]+)"/.exec(pill)![1]!);

    expect(values).toEqual([
      "all",
      ...sourceTypeCounts(registry.sources).map((entry) => entry.type),
    ]);
    const pressed = pills.filter((pill) =>
      pill.includes('aria-pressed="true"'),
    );
    ...
    for (const entry of sourceTypeCounts(registry.sources)) {
      expect(text(html()), entry.type).toContain(
  ...
  it("announces the result count from a live region that is in the a11y tree", () => {
    ...
    const region = new RegExp('<[a-z]+[^>]*\\bid="sources-status"[^>]*>').exec(
      html(),
    )?.[0];
    ...
    expect(
      region,
      "the live region is hidden, so its aria-live does nothing",
  ...
  it("never inlines the registry into the page", () => {
    ...
    expect(
      bytes,
      "the sources page is heavy enough to be carrying the registry",
    ).toBeLessThan(60_000);
  ...
  it("says what a reader without JavaScript is looking at, and links the file", () => {
    ...
    expect(tight, "the panel runs two words together").toContain(
      "sources as sources.json",
    );
    expect(tight, "the panel runs two words together").toContain(
      ", or on GitHub",
    );
  ...
  it("keeps the registry and the core package out of the browser bundle", () => {
    const rendered = html();
    const inline = [
      ...rendered.matchAll(/<script type="module">([\s\S]*?)<\/script>/g),
    ...
    expect(
      scripts.length,
      "the sources browser ships no script",
    ).toBeGreaterThan(0);
    expect(scripts.some(([, code]) => code.includes("#source-search"))).toBe(
      true,
    );
    for (const [where, code] of scripts) {
      ...
      expect(code, `${where} reaches for the filesystem`).not.toContain(
        "node:fs",
      );
    }
  });
...
describe.skipIf(!built)("the served registry", () => {
  ...
  it("is the file in the repository, not a copy inside this package", () => {
    ...
    expect(existsSync(resolve(__dirname, "../public/sources.json"))).toBe(
      false,
    );
  });
});
