...
function dossierIds(): string[] {
  ...
  for (const category of readdirSync(DOSSIERS, { withFileTypes: true })) {
    ...
    for (const file of readdirSync(resolve(DOSSIERS, category.name))) {
      if (file.endsWith(".md"))
        out.push(`${category.name}/${file.replace(/\.md$/, "")}`);
    }
  }
  return out.sort();
}
...
const DOSSIER_TEMPLATE = resolve(
  __dirname,
  "pages/audits/[category]/[slug].astro",
);
...
describe("dossier content", () => {
  ...
  it("opens every dossier body with a human title, never the audit id", () => {
    for (const id of dossierIds()) {
      ...
      expect(
        heading![1]!.trim(),
        `${id} titles itself with its own id`,
      ).not.toBe(audit);
    }
  });
  ...
  it("takes both its title and its heading from the registry", () => {
    const template = readFileSync(DOSSIER_TEMPLATE, "utf8");
    // The public slice discards the dossier's own `# ` line with the rest of the
    // intro block, so the layout supplies the page's only `<h1>` — and the
    // registry title is the better name regardless: the body heading is a
    // working title on most dossiers (`og-type (4.7)`), while every list that
    // links here shows the registry title. The rendered check over all 215
    // built pages is in `layouts/chrome.test.ts`.
    expect(
      template,
      "the title comes from the registry, not the body heading",
    ).toMatch(/const title = audit\.title;/);
    expect(template, "the layout is given that title as the heading").toMatch(
      /heading=\{title\}/,
    );
  });
  ...
  it("keeps every tier it does declare inside the schema enum", () => {
    for (const id of dossierIds()) {
      const tier = /^tier:\s*(\S+)/m.exec(
        readFileSync(resolve(DOSSIERS, `${id}.md`), "utf8"),
      );
      if (!tier) continue;
