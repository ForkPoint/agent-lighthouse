...
describe('Verify scan results against real sites', () => {
  ...
  describe('example.com', () => {
    ...
    let allResults: Map<string, { status: string; score: number; found: string }>;
    ...
    it('prints full results summary for manual review', () => {
      ...
      console.log(`Pass: ${[...allResults.values()].filter((r) => r.status === 'pass').length}`);
      console.log(`Warn: ${[...allResults.values()].filter((r) => r.status === 'warn').length}`);
      console.log(`Fail: ${[...allResults.values()].filter((r) => r.status === 'fail').length}`);
