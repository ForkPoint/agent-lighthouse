...
function loadFixture(filename: string): string {
  return readFileSync(
    resolve(__dirname, "../../test-data/corpus", filename),
    "utf8",
  );
}
...
describe("Golden Corpus Conformance Tests (False-Positive Elimination)", () => {
  ...
  it("correctly evaluates SaaS / Documentation fixtures with OpenAPI & Code blocks", async () => {
    ...
    const ctx = buildTestContext("https://docs.cloudstack.dev", [page], {
      "/llms.txt": createMockFetch(
        200,
        "# CloudStack Docs\n\n> AI-first documentation.\n\n## Pages\n- [API](/api): Endpoints",
      ),
      "/openapi.json": createMockFetch(
        200,
        JSON.stringify({
          openapi: "3.1.0",
          info: { title: "CloudStack API", version: "1.0" },
          paths: {},
        }),
    ...
    ctx.fetch = async ({ url }) =>
      url === "https://docs.cloudstack.dev/api/authentication.md"
        ? createMockFetch(200, markdownMirror(html), {
