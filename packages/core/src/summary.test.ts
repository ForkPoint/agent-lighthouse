...
describe('generateScanSummary', () => {
  it('builds a full summary with tier, vitals, strongest and weakest', () => {
    const report: Partial<ScanReport> = {
      ...
      readinessVitals: { commerce: 80, content: 60, botAccessibility: 40, technical: 90 },
      categories: [
        makeCategory({ id: 'strong', name: 'Strong', score: 95, passCount: 2, checks: [makeCheck(), makeCheck()] }),
        makeCategory({ id: 'weak', name: 'Weak', score: 20, passCount: 0, failCount: 1, checks: [makeCheck('fail')] }),
      ],
      recommendations: [
    ...
    expect(summary).toContain('Scan Report for example.com: Overall Readiness 72% (Partially Ready).');
    expect(summary).toContain('Commerce 80%, Content 60%, AI Bot Accessibility 40%, Technical Readiness 90%.');
  ...
  it('uses N/A for missing tier and applies defaults for an empty report', () => {
    ...
    expect(summary).toContain('Commerce 0%, Content 0%, AI Bot Accessibility 0%, Technical Readiness 0%.');
  ...
  it('omits improvement area when strongest and weakest share an id', () => {
    ...
    const summary = generateScanSummary({
      domain: 'dup.com',
      categories: [shared, makeCategory({ id: 'same', name: 'Same', score: 50 })],
    });
