...
function first(v: unknown): Record<string, unknown> | undefined {
  ...
  return arr.find((x) => x && typeof x === 'object') as Record<string, unknown> | undefined;
}
 ...
 */
export function extractProductFieldVerification(pages: PageContext[]): ProductFieldVerification {
  const productPages = pages.filter((p) => p.pageType === 'product');

  let product: Record<string, unknown> | undefined;
  let sourceUrl: string | undefined;
  for (const page of productPages) {
    const found = flatten(page.structuredData ?? page.jsonLd).find((s) => isType(s, PRODUCT_TYPES));
    if (found) {
      product = found;
      sourceUrl = page.url;
      break;
  ...
  const hasGtin = ['gtin', 'gtin8', 'gtin12', 'gtin13', 'gtin14'].some((k) => has(product![k]));
  const sku =
    has(product['sku']) ||
  ...
  const currency = has(offer?.['priceCurrency']) || has(product['priceCurrency']);
  const availability = has(offer?.['availability']) || has(product['availability']);
  const inventory = has(offer?.['inventoryLevel']) || has(product['inventoryLevel']);
  const reviewCount =
    has(rating?.['reviewCount']) || has(product['reviewCount']) || has(rating?.['ratingCount']);
  ...
  return {
    ...
    priceCurrency: price && currency ? 'found' : price || currency ? 'partial' : 'missing',
