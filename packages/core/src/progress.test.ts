...
const PHASES: PhaseId[] = ['fetch-root', 'fetch-pages', 'analyze', 'audits', 'report'];
...
describe('ProgressTracker', () => {
  ...
  it('fraction within a phase tracks completed/total × phase weight', () => {
    ...
    tracker.unitDone();
    expect(tracker.fraction).toBeCloseTo(PHASE_WEIGHTS['fetch-root'] * 0.25, 10);
    tracker.unitDone();
    tracker.unitDone();
    expect(tracker.fraction).toBeCloseTo(PHASE_WEIGHTS['fetch-root'] * 0.75, 10);
  });
  ...
  it('unit:fail counts as settled work and carries the error', () => {
    ...
    expect(fail).toMatchObject({ phase: 'audits', label: 'a A', error: 'kaboom' });
  ...
  it('stamps scan:done with score and duration', () => {
    ...
    if (done.type === 'scan:done') expect(done.durationMs).toBeGreaterThanOrEqual(0);
  });
});
