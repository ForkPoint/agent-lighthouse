...
describe('normalizeUrl', () => {
  ...
  it('lowercases the hostname but not the path', () => {
    expect(normalizeUrl('https://SHOP.TEST/Products/Hat')).toBe('https://shop.test/Products/Hat');
  });
  ...
  it('keeps the trailing slash on a directory path', () => {
    expect(normalizeUrl('https://shop.test/collections/')).toBe('https://shop.test/collections/');
  });
...
describe('joinUrl', () => {
  it('joins a base and a path', () => {
    expect(joinUrl('https://shop.test', 'robots.txt')).toBe('https://shop.test/robots.txt');
  });
