...
describe("CATEGORY_IDS", () => {
  ...
  it("matches the ids the default config carries", () => {
    expect([...CATEGORY_IDS]).toEqual(
      defaultConfig.categories.map((c) => c.id),
    );
  });
});
...
describe("filterConfig", () => {
  ...
  it("returns the config untouched when there is nothing to filter", () => {
    expect(filterConfig(defaultConfig, { includeExperimental: true })).toBe(
      defaultConfig,
    );
  });
  ...
  it("excludes experimental audits by default and includes them on request", () => {
    ...
    const withExperimental: ScanConfig = {
      categories: defaultConfig.categories.filter(
        (c) => c.id === "machine-discovery",
      ),
      audits: { "machine-discovery": [...base, experimental] },
    };

    const ids = (c: ScanConfig) =>
      c.audits["machine-discovery"]!.map((r) => r.meta.id);
    expect(
      ids(filterConfig(withExperimental, { includeExperimental: false })),
    ).not.toContain("machine-discovery/probe-x");
    expect(
      ids(filterConfig(withExperimental, { includeExperimental: true })),
    ).toContain("machine-discovery/probe-x");
  });
  ...
  it("drops exactly the experimental audits from the live registry by default", () => {
    ...
    expect(
      count(filterConfig(defaultConfig, { includeExperimental: true })),
    ).toBe(count(defaultConfig));
  });
