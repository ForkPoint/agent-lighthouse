...
const prose = (n: number) =>
  Array.from({ length: n }, (_v, i) => `Sentence number ${i} explains the mug in some detail.`).join(' ');

describe('extraction', () => {
  it('returns null from readability on a document with nothing in it', () => {
    expect(readabilityArticle('<html><body></body></html>', 'https://example.com/')).toBeNull();
  });
  ...
  it('picks the block with the most text per link', () => {
    const links = Array.from({ length: 40 }, (_v, i) => `<a href="/p/${i}">Product number ${i} link text</a>`).join('');
