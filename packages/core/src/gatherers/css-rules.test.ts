...
describe("parseCssRules", () => {
  ...
  it("skips at-rules whose body holds declarations, not selectors", () => {
    const rules = parseCssRules(
      "@font-face { font-family: X; src: url(x.woff) } .a { color: red }",
    );
  ...
  it("skips a keyframes body", () => {
    const rules = parseCssRules(
      "@keyframes spin { from { opacity: 0 } to { opacity: 1 } } .a { color: red }",
    );
    expect(rules.map((r) => r.selector)).toEqual([".a"]);
  });
...
describe("collectPageCss", () => {
  ...
  it("fetches a same-origin stylesheet and scans it", async () => {
    const { ctx, seen } = ctxWith({
      "https://a.test/s.css": ".ghost { display: none }",
    });
    const page = mockPageContext(
      "https://a.test/",
    ...
    expect(css.rules[0]).toMatchObject({
      selector: ".ghost",
      origin: "https://a.test/s.css",
    });
  ...
  it("does not fetch a cross-origin stylesheet, and reports it", async () => {
    const { ctx, seen } = ctxWith({
      "https://cdn.test/s.css": ".ghost { display: none }",
    });
    const page = mockPageContext(
      "https://a.test/",
  ...
  it("caps the number of stylesheets it fetches", async () => {
    ...
    const page = mockPageContext(
      "https://a.test/",
      `<html><head>${links.join("")}</head><body></body></html>`,
    );
