...
describe("classifyResponse", () => {
  ...
  it("classifies 402 with a crawler-price header as pay-per-crawl", () => {
    expect(
      classifyResponse(ok(), res(402, { "crawler-price": "USD 0.01" }))
        .blockClass,
    ).toBe("pay-per-crawl");
  });
...
describe("probeUaParity", () => {
  ...
  it("issues one baseline per URL and one probe per URL per token", async () => {
    const r = recorder(() => ok());
    const probes = await probeUaParity(
      r.fetch,
...
describe("sharedUaProbes", () => {
    ...
    return {
      ...
      fetch: async (o: FetchOptions): Promise<FetchResult> => {
        ...
        return mockFetchResult(
          "<html><body><main><p>Copy here.</p></main></body></html>",
          200,
          "text/html",
        );
      },
  ...
  it("fetches each baseline and each crawler pair once per scan", async () => {
    const ctx = scan();
    const first = await sharedUaProbes(
      ctx,
      ["https://example.com/"],
      ["gptbot", "claudebot"],
    );
    const second = await sharedUaProbes(
      ctx,
      ["https://example.com/"],
      ["gptbot"],
    );
  ...
  it("never probes a URL that fails the SSRF gate", async () => {
    const ctx = scan();
    expect(
      await sharedUaProbes(ctx, ["https://localhost/"], ["gptbot"]),
    ).toEqual([]);
  ...
  it("fetches the control bot once per URL per scan", async () => {
    ...
    await sharedControlProbe(ctx, "https://example.com/");
    expect(ctx.calls).toEqual([
      { url: "https://example.com/", ua: CONTROL_UA },
    ]);
  });
