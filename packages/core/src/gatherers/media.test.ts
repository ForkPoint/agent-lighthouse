...
describe('media gatherer', () => {
  describe('containerOf', () => {
    it('names each container from its signature', () => {
      ...
      expect(containerOf(join(raw('\x00\x00\x00\x18ftypavif'), raw('0000')))).toBe('bmff');
  ...
  describe('findC2paManifest', () => {
    ...
    it('finds a WebP C2PA chunk', () => {
      const webp = join(raw('RIFF\x20\x00\x00\x00WEBP'), raw('C2PA'), new Uint8Array([5, 0, 0, 0]), raw('store'));
  ...
  describe('extractXmp', () => {
    it('reads the packet out of a JPEG APP1 segment', () => {
      ...
      const found = extractXmp(jpeg(0xe1, `http://ns.adobe.com/xap/1.0/\x00${packet}`));
      expect(found).toBe(packet);
    });

    it('reads the packet out of a PNG iTXt chunk', () => {
      const packet = '<?xpacket begin="?"?><x:xmpmeta/><?xpacket end="w"?>';
      const found = extractXmp(png('iTXt', `XML:com.adobe.xmp\x00\x00\x00\x00\x00${packet}`));
      expect(found).toBe(packet);
    });
  ...
  describe('originOfVariant', () => {
    it('decodes a Next.js optimized image back to its origin', () => {
      expect(originOfVariant('https://example.com/_next/image?url=%2Fhero.jpg&w=640&q=75')).toBe(
        'https://example.com/hero.jpg',
      );
    });

    it('decodes a Cloudflare resized image back to its origin', () => {
      expect(originOfVariant('https://example.com/cdn-cgi/image/width=800,quality=75/photos/a.jpg')).toBe(
        'https://example.com/photos/a.jpg',
      );
    });

    it('strips a WordPress rendition suffix', () => {
      expect(originOfVariant('https://example.com/wp-content/uploads/2026/08/a-300x200.jpg')).toBe(
        'https://example.com/wp-content/uploads/2026/08/a.jpg',
      );
    });
  ...
  describe('imageCandidates', () => {
    it('collects src, srcset, og:image and JSON-LD images, same host only', () => {
      const jsonLd = JSON.stringify({
        ...
        '@type': 'Article',
        image: ['https://example.com/ld.jpg', { url: 'https://example.com/ld2.jpg' }],
      });
