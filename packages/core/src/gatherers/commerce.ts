...
const STRUCK_SELECTOR =
  'del, s, strike, .was-price, .compare-at, [class*="strikethrough"], [class*="compare-at"]';
...
export const IN_STOCK_PHRASES =
  /(in stock|add to (cart|bag|basket)|buy now|available now)/i;
...
export function parseAmount(raw: string): number | undefined {
  ...
  if (lastDot !== -1 && lastComma !== -1) {
    ...
    body = /,\d{3}(\D|$)/.test(`${body} `)
      ? body.split(",").join("")
      : body.replace(",", ".");
  } else if (lastDot !== -1) {
    const after = body.length - lastDot - 1;
    if (after === 3 && body.split(".").length > 2)
...
export function priceCandidates(
  ...
  const struck = new Set(
    $(STRUCK_SELECTOR, region)
      .toArray()
      .flatMap((el) => [el, ...$(el).find("*").toArray()]),
  );
    ...
    for (const child of el.children) {
      if (child.type === "tag") {
        ...
        for (const match of text.matchAll(pattern)) {
          ...
          if (value !== undefined)
            out.push({ value, text: match[0].trim(), struck: nowStruck });
        }
      }
    }
  };
