...
import { parseDictionary, parseLinkHeader, linksWithRel } from './structured-fields';
...
describe('parseDictionary', () => {
  ...
  it('rejects a string value, an empty value and a bad key with a reason', () => {
    ...
    expect(parseDictionary('train-ai=')).toEqual({ ok: false, error: '"train-ai" has no value' });
...
describe('parseLinkHeader', () => {
  it('parses href and parameters, unquoting values', () => {
    const [entry] = parseLinkHeader('<https://example.com/l.xml>; rel="license"; type=application/rsl+xml');
  ...
  it('does not split on a comma inside a quoted parameter', () => {
    const entries = parseLinkHeader('<https://a.example/1>; rel="self"; title="one, two"');
    expect(entries).toHaveLength(1);
    expect(entries[0]?.params['title']).toBe('one, two');
  });

  it('splits genuine multiple entries', () => {
    const entries = parseLinkHeader('<https://a.example/hub>; rel=hub, <https://a.example/f>; rel=self');
    expect(entries.map((e) => e.href)).toEqual(['https://a.example/hub', 'https://a.example/f']);
  });
