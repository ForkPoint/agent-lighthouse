...
describe("commerce gatherer", () => {
  ...
  describe("productRegion", () => {
    ...
    it("falls back to main, then body", () => {
      const withMain = cheerio.load(
        "<body><main><p>No product here.</p></main></body>",
      );
  ...
  describe("priceCandidates", () => {
    it("reads amounts on either side of the currency", () => {
      const $ = cheerio.load(
        '<div id="r"><span>$49.00</span><span>1.234,56 EUR</span></div>',
      );
    ...
    it("does not read a bare number as a price", () => {
      const $ = cheerio.load(
        '<div id="r"><span>4.7 stars</span><span>128 reviews</span></div>',
      );
    ...
    it("marks a strikethrough amount as struck", () => {
      const $ = cheerio.load(
        '<div id="r"><del>$99.00</del><span class="now">$49.00</span></div>',
      );
    ...
    it("marks a compare-at wrapper and everything inside it", () => {
      const $ = cheerio.load(
        '<div id="r"><div class="compare-at"><span>$99.00</span></div></div>',
      );
  ...
  describe("platformFingerprint", () => {
    it("names each platform from its markup", () => {
      const cases: Array<[string, string]> = [
        ...
        [
          '<script src="/static/version1234/frontend/Magento_Theme/x.js"></script>',
          "magento",
        ],
      ];
