 ...
 */
export function findC2paManifest(
  bytes: Uint8Array,
  ...
  if (container === "bmff") {
    for (const [type, start, length] of bmffBoxes(bytes)) {
      ...
      const uuid = Buffer.from(bytes.subarray(start, start + 16)).toString(
        "hex",
      );
      if (uuid === BMFF_C2PA_UUID)
        return { container, start: start + 16, length: length - 16 };
    }
    return undefined;
  }
...
export function originOfVariant(raw: string): string | undefined {
  ...
  const wordpress = /^(.+)-\d+x\d+(\.(?:jpe?g|png|webp|avif|gif))$/i.exec(
    url.pathname,
  );
  if (wordpress) {
...
const imageCache = new WeakMap<
  object,
  Map<string, Promise<Uint8Array | undefined>>
>();
...
const imageHeadCache = new WeakMap<
  object,
  Map<string, Promise<FetchResult | undefined>>
>();

export function fetchImageHead(
  ctx: MediaContext,
  url: string,
