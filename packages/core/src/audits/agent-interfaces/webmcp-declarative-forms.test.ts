...
describe('WebmcpDeclarativeFormsAudit', () => {
  ...
  it('does not count a form with tooldescription but no toolname', () => {
    const ctx = mockCheckContext([
      page('<form tooldescription="Search products" action="/search"><input name="q" /></form>'),
    ]);
    const result = audit.audit(ctx);
  ...
  it('ignores an empty toolname', () => {
    const ctx = mockCheckContext([
      page('<form toolname="" tooldescription="Search" action="/search"><input name="q" /></form>'),
    ]);
  ...
  it('warns when a named tool has no tooldescription for an agent to select on', () => {
    const ctx = mockCheckContext([
      page('<form toolname="search_products" action="/search"><input name="q" /></form>'),
    ]);
    const result = audit.audit(ctx);
    expect(result.status).toBe('warn');
  ...
  it('counts forms across every crawled page', () => {
    const ctx = mockCheckContext([
      page('<form toolname="search_products" tooldescription="Search" action="/search"><input name="q" /></form>'),
      page('<form action="/contact" method="POST"><input name="email" /></form>', 'https://example.com/contact', 1),
    ]);
    const result = audit.audit(ctx);
    expect(result.status).toBe('warn');
    expect(result.found).toContain('1/2');
  ...
  it('points at the live Chrome declarative-API docs, not the dead webmcp.link', () => {
    ...
    expect(docsUrl).toBe('https://developer.chrome.com/docs/ai/webmcp/declarative-api');
  });
  ...
  it('names the four spec attributes in its guidance', () => {
    ...
    for (const attr of ['toolname', 'tooldescription', 'toolparamdescription', 'toolautosubmit']) {
