...
import { McpToolDescriptionCoverageAudit, collectLeaves } from './mcp-tool-description-coverage';
...
vi.mock('../../fetcher', async (importOriginal) => {
  ...
  return { ...actual, isSafeUrl: async (url: string) => url.startsWith('https://api.example.com') };
});

const ENDPOINT = 'https://api.example.com/mcp';
const strings = (result: AuditResult, key: string): string[] => (result.details?.[key] ?? []) as string[];
const num = (result: AuditResult, key: string): number => result.details?.[key] as number;

const LONG = 'Search the product catalogue and return matching items with prices.';
...
function run(server: Server = {}) {
  ...
  const ctx: CheckContext = mockCheckContext([], server.undeclared ? {} : servers());
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (body.method === 'tools/list') {
      return mockFetchResult(
        JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: server.tools ?? [] } }),
        200,
        'application/json',
      );
    }
    if (body.method === 'server/discover') {
      const result: Record<string, unknown> = { serverInfo: { name: 'x', version: '1' } };
...
const GOOD = () => ({
  ...
  inputSchema: {
    type: 'object',
    properties: {
      query: { type: 'string', description: 'Free-text search over product titles.' },
      locale: { type: 'string', description: 'BCP 47 tag.', enum: ['en-GB', 'de-DE'] },
    },
    required: ['query'],
  },
...
describe('McpToolDescriptionCoverageAudit', () => {
  ...
  it('sends only tools/list and server/discover', async () => {
    const { result, methods } = run({ tools: [GOOD()], instructions: 'Call search first.' });
    await result;
    expect(new Set(methods)).toEqual(new Set(['tools/list', 'server/discover']));
    expect(methods.filter((m) => m === 'tools/list')).toHaveLength(1);
  });

  it('passes a fully documented tool surface', async () => {
    const r = await run({ tools: [GOOD()], instructions: 'Call search_products first.' }).result;
  ...
  it('counts a description under 40 characters as a stub and warns', async () => {
    const r = await run({ tools: [{ ...GOOD(), description: 'Searches.' }], instructions: 'x' })
      .result;
  ...
  it('fails an undocumented required parameter and names its path', async () => {
    ...
    expect(strings(r, 'undescribedRequiredParams')).toEqual(['search_products.query']);
  });
  ...
  it('walks arrays of objects and names the path with []', async () => {
    ...
    const r = await run({ tools: [tool], instructions: 'x' }).result;
    expect(strings(r, 'undescribedParams')).toEqual(['create_invoice.line_items[].tax_code']);
  });
  ...
  it('fails below the 90% parameter threshold and passes at exactly 90%', async () => {
    const props = (documented: number, total: number) => {
      ...
      for (let i = 0; i < total; i += 1) {
        out[`p${i}`] = i < documented ? { type: 'string', description: 'A documented one.' } : { type: 'string' };
      }
      return out;
    };
    ...
    const atThreshold = await run({ tools: [tool(9, 10)], instructions: 'x' }).result;
    ...
    const leaves = collectLeaves(
      {
        ...
        properties: {
          customer: {
            type: 'object',
            properties: { email: { type: 'string', description: 'Billing email.' } },
          },
        },
        required: ['customer'],
      },
  ...
  it('marks a string leaf constrained by enum, format or pattern', () => {
    ...
    expect(leaves.filter((l) => l.constrained).map((l) => l.path)).toEqual(['a', 'b', 'c']);
