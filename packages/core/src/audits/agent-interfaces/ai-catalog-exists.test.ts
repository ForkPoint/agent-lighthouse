...
const LEGACY = JSON.stringify({
  version: '1.0',
  name: 'Site',
  services: [{ name: 'Search', url: 'https://example.com/api/search', type: 'rest' }],
});
...
const withCatalog = (body: string, status = 200, contentType = 'application/ai-catalog+json') =>
  mockCheckContext([], {
...
describe('AiCatalogExistsAudit', () => {
  ...
  it('fails when host is missing', () => {
    expect(audit.audit(withCatalog(ard({ host: undefined }))).status).toBe('fail');
  });

  it('fails when entries is not an array', () => {
    expect(audit.audit(withCatalog(ard({ entries: { a: 1 } }))).status).toBe('fail');
  });
  ...
  it('does not require the ai-catalog media type to pass', () => {
    expect(audit.audit(withCatalog(ard(), 200, 'application/json')).status).toBe('pass');
  });
  ...
  it('diagnoses an HTML soft-404 as "no manifest served", not "invalid JSON"', () => {
    const result = audit.audit(withCatalog('<!doctype html><html></html>', 200, 'text/html'));
    expect(result.status).toBe('fail');
  ...
  it('matches the rel token case-insensitively and inside a multi-token rel', () => {
    const ctx = mockCheckContext(
      [page('<link rel="Alternate AI-Catalog" type="application/ai-catalog+json" href="/c.json">')],
      { '/.well-known/ai-catalog.json': mockFetchResult('', 404) },
    );
  ...
  it('finds the advertisement on any crawled page, not only the homepage', () => {
    const ctx = mockCheckContext(
      [page(''), page('<link rel="ai-catalog" href="/c.json">', 'https://example.com/developers', 1)],
      { '/.well-known/ai-catalog.json': mockFetchResult('', 404) },
    );
    expect(audit.audit(ctx).status).toBe('warn');
  });
  ...
  it('does not treat the old title-matched alternate link as an advertisement', () => {
    const ctx = mockCheckContext(
      [page('<link rel="alternate" type="application/json" href="/c.json" title="AI Catalog">')],
      { '/.well-known/ai-catalog.json': mockFetchResult('', 404) },
    );
  ...
  it('reports the advertisement alongside a valid well-known manifest', () => {
    const ctx = mockCheckContext([page('<link rel="ai-catalog" href="/ai-catalog.json">')], {
      '/.well-known/ai-catalog.json': mockFetchResult(ard(), 200, 'application/ai-catalog+json'),
    });
    const result = audit.audit(ctx);
    expect(result.status).toBe('pass');
  ...
  it('does not claim "advertised but not served" when the served manifest is merely malformed', () => {
    const ctx = mockCheckContext([page('<link rel="ai-catalog" href="/ai-catalog.json">')], {
      '/.well-known/ai-catalog.json': mockFetchResult(LEGACY, 200, 'application/json'),
    });
    const result = audit.audit(ctx);
    expect(result.status).toBe('fail');
