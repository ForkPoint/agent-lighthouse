...
function run(handler: (o: FetchOptions) => FetchResult, files = servers()): Promise<unknown> {
...
const DISCOVER_OK = {
  ...
  result: {
    ...
    capabilities: { tools: {}, resources: {}, extensions: { 'io.example/search': {} } },
...
describe('McpModernEraReachabilityAudit', () => {
        ...
        ? json(
            {
              ...
              error: {
                ...
                data: { supported: ['2025-03-26', '2025-11-25'], requested: '2026-07-28' },
              },
            },
            400,
          )
  ...
  it('warns on a 401 challenge and hands off to the OAuth chain audit by name', async () => {
    ...
    expect(result.status).toBe('warn');
    expect(result.message).toContain('agent-interfaces/mcp-oauth-discovery-chain');
  });
  ...
  it('fails a -32601 on server/discover as a MUST violation', async () => {
    const result = (await run((o) =>
      o.method === 'POST'
        ? json(
            { jsonrpc: '2.0', id: 'al-1', error: { code: -32601, message: 'Method not found' } },
            404,
          )
        : mockFetchResult('', 405),
    )) as Result;
    ...
    const result = (await run((o) => {
      ...
      if (body.includes('"initialize"')) {
        const res = json({
          jsonrpc: '2.0',
          id: 1,
          result: { protocolVersion: '2025-03-26', capabilities: {}, serverInfo: { name: 'old' } },
        });
        res.headers['mcp-session-id'] = '1868a90c-0e2f';
        return res;
      }
      return mockFetchResult('Bad Request: expected initialize as the first request', 400, 'text/plain');
    })) as Result;
    expect(result.status).toBe('fail');
  ...
  it('reports legacy residue when a modern server answers GET with something other than 405', async () => {
    const result = (await run((o) => {
      if (o.method === 'POST') return json(DISCOVER_OK);
      if (o.method === 'GET') return mockFetchResult('', 200, 'text/event-stream');
      return mockFetchResult('', 405);
    })) as Result;
    expect(result.status).toBe('warn');
  ...
  it('fails a declared endpoint on a private address without probing it', async () => {
    ...
    const ctx: CheckContext = mockCheckContext([], servers('http://10.0.0.4/mcp'));
