...
const PROOF_GRAMMAR =
  /^v=MCPv1;\s*k=(ed25519|ecdsap384);\s*p=[A-Za-z0-9+/]+={0,2}\s*$/;
...
export function parseListings(body: string): Listing[] {
  ...
  for (const entry of parsed["servers"] as unknown[]) {
    ...
    const official = isObject(meta["io.modelcontextprotocol.registry/official"])
      ? (meta["io.modelcontextprotocol.registry/official"] as Record<
          string,
          unknown
        >)
      : {};
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const term of terms) {
      ...
      for (const listing of parseListings(response.body)) {
        ...
        if (
          !listing.remotes.some((remote) => remoteBelongsTo(remote.url, apex))
        )
          continue;
        if (!listings.some((seen) => seen.name === listing.name))
    ...
    for (const listing of listings) {
      ...
      if (kind === "aggregator") {
        ...
        warnings.push(
          `${listing.name} is bound to an individual's GitHub account rather than to this domain`,
        );
      }

      if (listing.status !== "" && listing.status !== "active") {
        warnings.push(
          `${listing.name} has registry status "${listing.status}"`,
        );
      }
      if (!listing.isLatest && listing.status === "active") {
        warnings.push(
          `${listing.name} is not the latest version in the registry`,
        );
      }
      if (
        listing.remotes.length > 0 &&
        !listing.remotes.some((remote) => remote.type === "streamable-http")
      ) {
        warnings.push(
          `${listing.name} offers only ${listing.remotes.map((r) => r.type || "untyped").join(", ")} transport`,
        );
      }
    }
