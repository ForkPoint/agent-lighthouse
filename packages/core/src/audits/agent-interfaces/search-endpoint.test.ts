...
const RESULTS_HTML =
  '<html><body><ol class="results"><li>Pricing</li></ol></body></html>';
...
describe("SearchEndpointAudit", () => {
  ...
  it("passes on a 200 JSON response carrying a non-empty result array", async () => {
    ...
    ctx.fetch = async () =>
      mockFetchResult(
        JSON.stringify({ results: [{ title: "Pricing" }] }),
        200,
        "application/json",
      );
    const result = await audit.audit(ctx);
      ...
      page(
        ld({
          ...
          potentialAction: {
            "@type": "SearchAction",
            target:
              "https://example.com/search?q={search_term_string}&lang={lang}",
          },
        }),
      ),
    ]);
        ...
        ld({
          ...
          mainEntity: {
            ...
            potentialAction: {
              ...
              target: {
                "@type": "EntryPoint",
                urlTemplate: "https://example.com/s?q={q}",
              },
            },
          },
        }),
  ...
  it("matches a hyphenated search path segment", async () => {
    const spec = JSON.stringify({
      paths: {
        "/api/product-search": { get: { operationId: "productSearch" } },
      },
    });
    const ctx = mockCheckContext([page("")], {
      "/openapi.json": mockFetchResult(spec, 200),
  ...
  it("ignores a non-GET search operation", async () => {
    const spec = JSON.stringify({
      paths: { "/search": { post: { operationId: "search" } } },
    });
    const ctx = mockCheckContext([page("")], {
      "/openapi.json": mockFetchResult(spec, 200),
    });
  ...
  it("fails when the spec has no paths key", async () => {
    const ctx = mockCheckContext([page("")], {
      "/openapi.json": mockFetchResult(
        JSON.stringify({ openapi: "3.0.3", info: {} }),
        200,
      ),
    });
    const result = await audit.audit(ctx);
  ...
  it("fails when a paths entry is not an object", async () => {
    const ctx = mockCheckContext([page("")], {
      "/openapi.json": mockFetchResult(
        JSON.stringify({ paths: { "/null-path": null } }),
        200,
      ),
    });
    const result = await audit.audit(ctx);
