...
function rpcError(code: number, data?: unknown, status = 400): FetchResult {
  return json(
    { jsonrpc: '2.0', id: 'x', error: { code, message: 'nope', ...(data ? { data } : {}) } },
    status,
  );
}
...
function probeOf(o: FetchOptions): Probe {
  ...
  return {
    ...(header ? { header } : {}),
    body: parsed.params?._meta?.['io.modelcontextprotocol/protocolVersion'] ?? '',
  };
}
...
const REJECTION = rpcError(-32022, { supported: ['2025-11-25', CURRENT], requested: '1900-01-01' });
...
function run(over: Partial<Record<'a' | 'b' | 'c' | 'discover', FetchResult>> = {}): Promise<Result> {
  ...
  ctx.fetch = async (o: FetchOptions) => {
    ...
    if (probe.header === CURRENT && probe.body === '2025-11-25') return over.b ?? MISMATCH;
...
describe('McpVersionDowngradeAudit', () => {
  ...
  it('fails at critical when the impossible revision is accepted with 200', async () => {
    const result = await run({ a: json({ jsonrpc: '2.0', id: 'x', result: {} }) });
  ...
  it('fails when data.supported is empty', async () => {
    const result = await run({ a: rpcError(-32022, { supported: [], requested: '1900-01-01' }) });
    expect(result.status).toBe('fail');
  ...
  it('warns when data.supported disagrees with supportedVersions from server/discover', async () => {
    const result = await run({
      a: rpcError(-32022, { supported: ['2025-03-26'], requested: '1900-01-01' }),
    });
    expect(result.status).toBe('warn');
    expect(result.message).toContain('2025-03-26');
  ...
  it('fails at high when a header/body version disagreement is answered with 200', async () => {
    const result = await run({ b: json({ jsonrpc: '2.0', id: 'x', result: {} }) });
    expect(result.status).toBe('fail');
    expect(result.message).toContain('never validates the header against the body');
  });
  ...
  it('warns when a headerless request is answered as the current revision', async () => {
    const result = await run({ c: json({ jsonrpc: '2.0', id: 'x', result: { capabilities: {} } }) });
    expect(result.status).toBe('warn');
  ...
  it('records a rejected headerless request without a finding', async () => {
    ...
    expect(result.status).toBe('pass');
    expect(result.message).toContain('no `MCP-Protocol-Version` header is rejected');
  });
  ...
  it('sends the impossible revision in both the header and _meta', async () => {
    ...
    ctx.fetch = async (o: FetchOptions) => {
      ...
      if (probe.header === CURRENT && probe.body === '2025-11-25') return MISMATCH;
