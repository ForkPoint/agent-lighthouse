...
export class McpOriginValidationCorsAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const allowCredentials =
      (cors["access-control-allow-credentials"] ?? "").trim().toLowerCase() ===
      "true";
    const allowHeaders = (
      cors["access-control-allow-headers"] ?? ""
    ).toLowerCase();
    const admitsAuthorization =
      allowHeaders.includes("authorization") || allowHeaders.includes("*");
    const challenged =
      baseline.status === 401 &&
      baseline.headers["www-authenticate"] !== undefined;
    ...
    const differentiates =
      withOrigin !== undefined && withOrigin.status !== baseline.status;
    ...
    if (!differentiates && !credentialAccepting) {
      notes.push(
        `${url} applies no Origin differentiation, on an endpoint with no authentication surface`,
      );
    }
    const buffering = (
      preflight?.headers["x-accel-buffering"] ??
      withOrigin?.headers["x-accel-buffering"] ??
      ""
    ).trim();
    ...
    const displayValue =
      reflected && allowCredentials
        ? "Reflected origin + credentials"
        : allowOrigin || "no CORS headers";
