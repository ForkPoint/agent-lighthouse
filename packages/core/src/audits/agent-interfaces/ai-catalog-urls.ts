...
function describeProbe(probe: Probe): string {
  const reason = probe.status === -1 ? 'refused' : probe.status === 0 ? 'unreachable' : `HTTP ${probe.status}`;
...
export class AiCatalogUrlsAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const linked = entries.filter((e): e is ArdEntry & { url: string } => Boolean(e.url));
    ...
    const probes = await pooled(linked, MAX_CONCURRENCY, async (entry): Promise<Probe> => {
    ...
    const inlineNote = inline.length > 0 ? `; ${inline.length} inline entr${inline.length === 1 ? 'y' : 'ies'} skipped` : '';
    ...
    const rest = broken.length > MAX_LISTED ? `, and ${broken.length - MAX_LISTED} more` : '';
