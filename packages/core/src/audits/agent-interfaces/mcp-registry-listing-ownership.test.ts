...
const FIRST_PARTY = () => entry('com.example/analytics', [{ type: 'streamable-http', url: ENDPOINT }]);
...
describe('McpRegistryListingOwnershipAudit', () => {
  ...
  it('sends two searches and one proof request, and never paginates', async () => {
    const { result, requests } = run({ servers: [FIRST_PARTY()], proof: VALID_PROOF });
    await result;
    ...
    expect(requests[2]?.url).toBe('https://example.com/.well-known/mcp-registry-auth');
  ...
  it('fails a first-party listing whose proof is missing', async () => {
    ...
    expect(strings(r, 'failures')[0]).toContain('/.well-known/mcp-registry-auth');
  });

  it('fails a first-party listing whose proof does not match the grammar', async () => {
    const { result } = run({ servers: [FIRST_PARTY()], proof: 'MCPv1 ed25519 abc' });
    const r = await result;
    expect(r.status).toBe('fail');
  ...
  it('ignores an entry whose name mentions the domain but whose remotes point elsewhere', async () => {
    const { result } = run({
      servers: [
        entry('com.example/analytics', [{ type: 'streamable-http', url: 'https://evil.test/mcp' }]),
      ],
      proof: VALID_PROOF,
    });
    const r = await result;
    ...
    const { result } = run({
      servers: [
        entry('ai.smithery/example-analytics', [
          ...
          { type: 'streamable-http', url: 'https://server.smithery.ai/@example/analytics/mcp' },
        ]),
      ],
      proof: VALID_PROOF,
    });
  ...
  it('warns rather than fails when the only listing is bound to a GitHub account', async () => {
    const { result } = run({
      servers: [entry('io.github.alice/example-analytics', [{ type: 'streamable-http', url: ENDPOINT }])],
      proof: VALID_PROOF,
    });
    const r = await result;
  ...
  it('warns on a listing that is not active or not the latest version', async () => {
    ...
    expect(strings(await stale.result, 'warnings').join(' ')).toContain('not the latest');
    ...
    expect(strings(await deleted.result, 'warnings').join(' ')).toContain('"deleted"');
  });

  it('warns on a listing that offers only the deprecated sse transport', async () => {
    const { result } = run({
      servers: [entry('com.example/analytics', [{ type: 'sse', url: ENDPOINT }])],
      proof: VALID_PROOF,
    });
    const r = await result;
    expect(r.status).toBe('warn');
  ...
  it('classifies namespaces by prefix', () => {
    expect(namespaceKind('com.example/analytics', 'example.com')).toBe('first-party');
    expect(namespaceKind('io.github.alice/thing', 'example.com')).toBe('github-account');
    expect(namespaceKind('ai.smithery/example', 'example.com')).toBe('aggregator');
    // A namespace that merely starts with the same letters is not the domain.
    expect(namespaceKind('com.example-evil/thing', 'example.com')).toBe('aggregator');
  });

  it('parses the live response shape and tolerates junk', () => {
    expect(parseListings('not json')).toEqual([]);
    expect(parseListings(JSON.stringify({ servers: [{}, 3, null] }))).toHaveLength(1);
