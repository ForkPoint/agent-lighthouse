...
function rpcRouter(handlers: {
  initialize: () => FetchResult;
  toolsList?: () => FetchResult;
}): (options: FetchOptions) => Promise<FetchResult> {
  return async (options) => {
    const method = options.body?.includes('"tools/list"')
      ? "tools/list"
      : "initialize";
    if (method === "tools/list") {
      return handlers.toolsList
        ? handlers.toolsList()
        : mockFetchResult("", 404);
    }
...
const ctxWithServers = (url?: string) =>
  mockCheckContext([], {
    "/.well-known/mcp/servers.json": mockFetchResult(serversFile(url), 200),
  });
...
describe("McpEndpointAudit", () => {
  ...
  it("reports the capabilities negotiated in the initialize response", async () => {
    ...
    ctx.fetch = rpcRouter({
      initialize: () =>
        mockFetchResult(initializeResult({ tools: {}, resources: {} }), 200),
  ...
  it("rejects a non-boolean readOnlyHint rather than counting its presence", async () => {
    ...
    ctx.fetch = rpcRouter({
      ...
      toolsList: () =>
        mockFetchResult(
          toolsListResult([
            { name: "search", annotations: { readOnlyHint: null } },
          ]),
          200,
        ),
    });
