...
const DELETION =
  "conforming Streamable HTTP clients MUST drop this tool from tools/list";
...
export class McpToolContractValidityAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const [index, tool] of tools.entries()) {
      ...
      if (!name) {
        ...
        if (name.length > MAX_NAME) {
          shoulds.push(
            `\`${name.slice(0, 24)}…\` is ${name.length} characters, over the ${MAX_NAME} limit`,
          );
        }
        ...
        if (
          [...name].some(
            (ch) => ch.codePointAt(0)! < 0x21 || ch.codePointAt(0)! > 0x7e,
          )
        ) {
          shoulds.push(
      ...
      if (!isObject(schema)) {
        ...
        for (const use of uses) {
          ...
          if (typeof value !== "string" || value.length === 0) {
            ...
            if (/[\r\n]/.test(value))
              violations.push("the value embeds CR or LF");
            else if (!TCHAR.test(value))
              violations.push(`"${value}" is not an RFC 9110 token`);
            const lower = value.toLowerCase();
            byLower.set(lower, (byLower.get(lower) ?? 0) + 1);
            if ((byLower.get(lower) ?? 0) > 1) {
              violations.push(
                `"${value}" repeats case-insensitively inside one inputSchema`,
              );
            }
          }
          ...
          if (violations.length > 0) {
            headerViolations += 1;
            musts.push(
              `${label} has an \`x-mcp-header\` where ${violations.join(", and ")} — ${DELETION}`,
            );
          }
        }
      }
    ...
    for (const [name, count] of seenNames) {
      if (count > 1)
        shoulds.push(`\`${name}\` is defined ${count} times within one server`);
    }
    ...
    const list = (items: string[]) => {
      ...
      return items.length > MAX_SHOWN
        ? `${shown} (${items.length - MAX_SHOWN} more)`
        : shown;
    };
