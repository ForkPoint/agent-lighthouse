...
function challenge(resourceMetadata = PRM_URL, header?: string): FetchResult {
  ...
  res.headers['www-authenticate'] =
    header ?? `Bearer resource_metadata="${resourceMetadata}", scope="mcp:tools"`;
  return res;
}
...
function run(discover: FetchResult, docs: Record<string, FetchResult>) {
  ...
  return new McpOauthDiscoveryChainAudit().audit(w.ctx).then((r) => ({ ...w, result: r as Result }));
}

const HEALTHY: Record<string, FetchResult> = { [PRM_URL]: json(PRM), [AS_URL]: json(AS_META) };
...
describe('McpOauthDiscoveryChainAudit', () => {
  ...
  it('warns when scopes_supported advertises offline_access', async () => {
    const { result } = await run(challenge(), {
      [PRM_URL]: json({ ...PRM, scopes_supported: ['mcp:tools', 'offline_access'] }),
      [AS_URL]: json(AS_META),
    });
  ...
  it('warns when the AS does not advertise S256', async () => {
    const { result } = await run(challenge(), {
      [PRM_URL]: json(PRM),
      [AS_URL]: json({ ...AS_META, code_challenge_methods_supported: ['plain'] }),
    });
    expect(result.status).toBe('warn');
  ...
  it('records an unauthenticated 200 as a positive signal and still probes the well-knowns', async () => {
    const ok = mockFetchResult(
      JSON.stringify({ jsonrpc: '2.0', id: 'al-1', result: { supportedVersions: ['2026-07-28'] } }),
      200,
      'application/json',
    );
    const { result, seen } = await run(ok, { [PRM_ROOT]: json(PRM), [AS_URL]: json(AS_META) });
    expect(seen).toContain(PRM_URL);
    expect(result.status).toBe('pass');
  ...
  it('is notApplicable for an open endpoint that publishes no PRM at all', async () => {
    const ok = mockFetchResult('{"jsonrpc":"2.0","id":"al-1","result":{}}', 200, 'application/json');
