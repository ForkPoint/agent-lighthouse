...
function run(tools: unknown[], pages?: Record<string, unknown[]>): Promise<Result> {
  ...
  ctx.fetch = async (o: FetchOptions) => {
    ...
    if (cursor && pages) {
      return mockFetchResult(
        JSON.stringify({ jsonrpc: '2.0', id: 1, result: { tools: pages[cursor] ?? [] } }),
        200,
        'application/json',
      );
    }
...
describe('McpToolContractValidityAudit', () => {
  ...
  it('fails a required entry that properties does not define', async () => {
    const result = await run([
      withSchema({ type: 'object', properties: { query: { type: 'string' } }, required: ['q'] }),
    ]);
    expect(result.status).toBe('fail');
  ...
  it('fails an empty x-mcp-header value', async () => {
    const result = await run([
      withSchema({ type: 'object', properties: { a: { type: 'string', 'x-mcp-header': '' } } }),
    ]);
    expect(result.status).toBe('fail');
  ...
  it('fails an x-mcp-header reached through items', async () => {
    const result = await run([
      withSchema({
        type: 'object',
        properties: {
          tags: { type: 'array', items: { type: 'string', 'x-mcp-header': 'X-Tag' } },
        },
      }),
    ]);
    expect(result.status).toBe('fail');
  ...
  it('fails an x-mcp-header reached through a $ref hop', async () => {
    const result = await run([
      withSchema({
        type: 'object',
        properties: { who: { $ref: { type: 'string', 'x-mcp-header': 'X-Who' } } },
      }),
    ]);
    expect(result.status).toBe('fail');
  ...
  it('fails a server with 9 clean tools and 1 header violation', async () => {
    const clean = Array.from({ length: 9 }, (_, i) => ({ ...CLEAN, name: `tool${i}` }));
    const broken = withSchema(
      { type: 'object', properties: { a: { type: 'number', 'x-mcp-header': 'X-Limit' } } },
      'brokenTool',
    );
  ...
  it('assesses every page of a paginated tools/list', async () => {
    const broken = withSchema({ type: 'object', properties: {}, required: ['missing'] }, 'page2Tool');
