...
async function listOnce(ctx: CheckContext, url: string, attempt: number): Promise<Call | undefined> {
  const pages: Page[] = [];
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const [index, page] of complete.entries()) {
      ...
      if (ttl === undefined || ttl === null) {
        ...
        musts.push(`\`ttlMs\`${where} is ${JSON.stringify(ttl)} rather than a non-negative integer`);
      } else if (ttl === 0) {
        musts.push(
    ...
    if (complete.some((p) => p.cacheScope === 'public')) {
      ...
      if (discover && discover.status === 401 && discover.headers['www-authenticate']) {
        shoulds.push(
    ...
    const ttls = complete.map((p) => (typeof p.ttlMs === 'number' ? `${p.ttlMs}` : 'absent'));
    const found = [
      url,
