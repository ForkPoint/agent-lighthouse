...
function indexedKeys(entry: ArdEntry): string[] {
  ...
  if (entry.representativeQueries.length > 0)
    present.push("representativeQueries");
  return present;
}
...
function bonuses(manifest: ArdManifest): string[] {
  ...
  if (dated > 0)
    found.push(`${dated}/${manifest.entries.length} entries with updatedAt`);
  if (
    manifest.hostHasTrustManifest ||
    manifest.entries.some((e) => e.hasTrustManifest)
  ) {
