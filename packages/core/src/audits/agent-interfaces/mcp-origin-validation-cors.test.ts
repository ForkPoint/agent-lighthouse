...
vi.mock('../../fetcher', async (importOriginal) => {
  ...
  return { ...actual, isSafeUrl: async (url: string) => url.startsWith('https://mcp.example.com') };
});
...
function run(server: Server = {}) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (server.allowCredentials) cors['access-control-allow-credentials'] = 'true';
    if (server.allowHeaders !== undefined) cors['access-control-allow-headers'] = server.allowHeaders;
    if (server.accelBuffering !== undefined) cors['x-accel-buffering'] = server.accelBuffering;
    ...
    const status = requestOrigin === '' ? (server.baselineStatus ?? 200) : (server.originStatus ?? server.baselineStatus ?? 200);
    const result = mockFetchResult(
      JSON.stringify({ jsonrpc: '2.0', id: 'al-1', result: { serverInfo: { name: 'x', version: '1' } } }),
      status,
      'application/json',
    );
    Object.assign(result.headers, cors);
    if (server.challenges && requestOrigin === '') result.headers['www-authenticate'] = 'Bearer realm="mcp"';
    return result;
  };
...
describe('McpOriginValidationCorsAudit', () => {
  ...
  it('fails a reflected Origin paired with credentials', async () => {
    const { result } = run({ allowOrigin: 'reflect', allowCredentials: true, challenges: true });
    const r = await result;
    expect(r.status).toBe('fail');
    expect(strings(r, 'findings').join(' ')).toContain('reflects an arbitrary Origin');
    expect(r.details?.['allowCredentials']).toBe(true);
  });

  it('fails a wildcard ACAO on an endpoint that issues an auth challenge', async () => {
    const { result } = run({ allowOrigin: '*', baselineStatus: 401, challenges: true });
    const r = await result;
    expect(r.status).toBe('fail');
    expect(strings(r, 'findings').join(' ')).toContain('Access-Control-Allow-Origin: *');
  });

  it('fails a wildcard ACAO on an endpoint that admits an Authorization header', async () => {
    const { result } = run({ allowOrigin: '*', allowHeaders: 'content-type, authorization' });
    const r = await result;
    expect(r.status).toBe('fail');
  ...
  it('warns when a credential-accepting endpoint answers identically with and without an Origin', async () => {
    ...
    expect(strings(r, 'findings').join(' ')).toContain('applies no Origin policy');
  });
  ...
  it('passes an endpoint that answers 403 to a throwaway Origin', async () => {
    const { result } = run({ originStatus: 403, allowHeaders: 'content-type, authorization' });
    const r = await result;
    expect(r.status).toBe('pass');
  ...
  it('sends a throwaway .example Origin and at most three requests beyond discovery', async () => {
    ...
    await result;
    const origins = requests.map((o) => o.headers?.['Origin']).filter(Boolean) as string[];
    expect(origins.length).toBeGreaterThan(0);
    for (const origin of origins) expect(origin).toMatch(/^https:\/\/al-probe-[0-9a-f]{12}\.example$/);
