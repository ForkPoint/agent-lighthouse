...
function errorOf(res: FetchResult): { code: number; data?: unknown } | undefined {
...
async function declaredVersions(ctx: CheckContext, url: string): Promise<string[]> {
...
export class McpVersionDowngradeAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const probeA = await postRpcRaw(ctx, url, 'al-v-a', 'server/discover', meta(IMPOSSIBLE), {
    ...
    if (probeA.status === 200) {
      ...
      if (probeA.status !== 400) {
        reviews.push(`the ${UNSUPPORTED} rejection arrived as HTTP ${probeA.status} rather than 400`);
      }
    ...
    const probeB = await postRpcRaw(ctx, url, 'al-v-b', 'server/discover', meta(MISMATCH_BODY), {
    ...
    const probeC = await postRpcRaw(ctx, url, 'al-v-c', 'server/discover', meta(HEADERLESS_DEFAULT));
    if (probeC) {
      ...
      if (probeC.status !== 200) {
        notes.push(`a request with no \`MCP-Protocol-Version\` header is rejected (HTTP ${probeC.status})`);
