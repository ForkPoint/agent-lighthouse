...
function isChrome(page: PageContext, el: unknown): boolean {
  ...
  return $el
    .parents()
    .toArray()
    .some((p) => CHROME_ITEMTYPE.test(page.$(p).attr("itemtype") ?? ""));
}
...
function countPseudoLists(page: PageContext): number {
  ...
  $("body *").each((_, parent) => {
    ...
    const marked = children.filter((child) => {
      ...
      if (childTag === "li" || childTag === "dt" || childTag === "dd")
        return false;
...
function tallyPage(page: PageContext): PageTally {
  ...
  $("ul, ol, dl").each((_, el) => {
    ...
    if (tag === "dl") {
      const paired =
        $el.children("dt").length > 0 && $el.children("dd").length > 0;
      if (paired) {
        tally.semantic++;
        tally.definitions++;
      } else {
...
export class SemanticListsAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const firstOffender = tallies.find((entry) => entry.tally.pseudo > 0)?.page
      .url;
