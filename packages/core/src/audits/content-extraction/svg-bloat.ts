 ...
 */
const DATA_URI_RE = /data:[a-z0-9.+-]+\/[a-z0-9.+-]+;base64,[A-Za-z0-9+/=]{200,}/gi;
...
function svgPathTokens(markup: string): number {
  let total = 0;
  for (const match of markup.matchAll(/\s(?:d|points)="([^"]*)"/gi)) total += countTokens(match[1] ?? '');
  return total;
}
...
export class SvgBloatAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    if (flagged.length === 0 && unhiddenBytes <= TOTAL_WARN_BYTES && dataUriTokens < DATA_URI_WARN_TOKENS) {
      return {
        ...this.pass(
