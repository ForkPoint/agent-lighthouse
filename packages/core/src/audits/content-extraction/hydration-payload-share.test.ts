...
const PROSE =
  "<p>Our winter boots are waterproof, insulated and resoleable.</p>".repeat(
    300,
  );
...
function run(body: string) {
  ...
  return audit.audit(
    mockCheckContext([mockPageContext("https://example.test/", html)]),
  );
}
...
describe("HydrationPayloadShareAudit", () => {
  ...
  it("fails a __NEXT_DATA__ payload over 128,000 bytes and names it", () => {
    const blob = JSON.stringify({
      props: { pageProps: { filler: "x".repeat(130_000) } },
    });
    const result = run(
  ...
  it("sums concatenated self.__next_f.push flight frames into one payload", () => {
    const frame = (i: number) =>
      `<script>self.__next_f.push([1,"${String(i)}${"y".repeat(45_000)}"])</script>`;
    const result = run(
  ...
  it("passes a small payload that duplicates nothing", () => {
    const blob = JSON.stringify({
      props: { pageProps: { sku: "AB-1", variants: "q".repeat(2_000) } },
    });
    const result = run(
      `<main>${PROSE}</main><script id="__NEXT_DATA__" type="application/json">${blob}</script>`,
    );
  ...
  it("warns when total state sits between 15% and 30% of the document", () => {
    const blob = JSON.stringify({
      props: { pageProps: { filler: "z".repeat(4_500) } },
    });
    const result = run(
  ...
  it("reports the estimated token cost of the payloads", () => {
    const blob = JSON.stringify({
      props: { pageProps: { variants: "q".repeat(2_000) } },
    });
    const result = run(
      `<main>${PROSE}</main><script id="__NEXT_DATA__" type="application/json">${blob}</script>`,
    );
