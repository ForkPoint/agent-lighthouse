...
const prose = (seed: string, n = 25) =>
  Array.from(
    { length: n },
    (_v, i) =>
      `The ${seed} kettle boils in ${i} minutes and keeps its heat for another hour.`,
  ).join(" ");
...
const page = (body: string): CheckContext =>
  mockCheckContext([
    mockPageContext(
      ...
      `<html><head><title>Kettles</title></head><body>${body}</body></html>`,
      1,
    ),
  ]);
...
describe("ExtractionDeterminismAudit", () => {
  ...
  it("fails when readability returns less than its own 500-character threshold", async () => {
    const nav = Array.from(
      { length: 30 },
      (_v, i) => `<a href="/c/${i}">Category number ${i}</a>`,
    ).join("");
    const result = await audit.audit(
  ...
  it("declines when no response can be attributed to this site", async () => {
    ...
    expect(plan.runnable.map((entry) => entry.reg.meta.id)).not.toContain(
      ExtractionDeterminismAudit.meta.id,
    );
    expect(
      plan.skipped.find(
