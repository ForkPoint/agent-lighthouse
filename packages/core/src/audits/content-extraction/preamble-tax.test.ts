...
const prose = (n: number) =>
  Array.from(
    { length: n },
    (_v, i) =>
      `Sentence number ${i} explains how the mug keeps coffee hot for hours.`,
  ).join(" ");
...
describe("PreambleTaxTokensBeforeTheFirstContentTokenAudit", () => {
  ...
  it("fails when the content sits behind a huge inline style block", async () => {
    const css =
      ".selector-name{color:#ffffff;background:#000000;padding:12px}".repeat(
        1200,
      );
    const result = await audit.audit(
      page(
