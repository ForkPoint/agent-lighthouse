...
const SECTIONS = [
  [
    "Boiling time",
    "The copper kettle reaches a rolling boil in about three minutes on gas.",
  ],
  [
    "Descaling",
    "Fill the kettle with equal parts water and white vinegar, then leave it overnight.",
  ],
  [
    "Warranty",
    "Every kettle carries a two year warranty covering the element and the handle.",
  ],
] as const;
  ...
  it("reports na when a declared alternate does not resolve, and names it", async () => {
    const ctx = mockCheckContext([
      mockPageContext(
        ...
        HTML.replace(
          "</head>",
          '<link rel="alternate" type="text/markdown" href="/kettles.md"></head>',
        ),
        1,
      ),
    ]);
  ...
  it("prefers the .md URL over a stale site-wide declared link", async () => {
    ...
    const ctx = mockCheckContext([
      mockPageContext(
        ...
        HTML.replace(
          "</head>",
          '<link rel="alternate" type="text/markdown" href="/docs/index.md"></head>',
        ),
        1,
      ),
    ]);
