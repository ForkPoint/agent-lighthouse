...
export class ExtractionDeterminismAudit extends Audit {
  static override meta: AuditMeta = {
    ...
    failureTitle: 'What an agent reads from this page depends on which extractor it uses',
    description:
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    if (readability.text.length < READABILITY_CHAR_THRESHOLD) {
      return {
        ...
        details: { readabilityChars: readability.text.length, visibleChars: visible.length },
      };
    }

    const extractions: Extracted[] = [readability, semanticText(html), densityText(html)];
    ...
    for (let i = 0; i < extractions.length; i += 1) {
      for (let j = i + 1; j < extractions.length; j += 1) {
        const similarity = jaccard(sets[i] as Set<string>, sets[j] as Set<string>);
    ...
    const onlyA = difference(sets[worst.a] as Set<string>, sets[worst.b] as Set<string>);
    const onlyB = difference(sets[worst.b] as Set<string>, sets[worst.a] as Set<string>);
    ...
    const details = {
      ...
      symmetricDifference: symmetricDifference.map((word) => word.slice(0, 1000)),
    };
