...
describe('FakeHeadingsAudit', () => {
  ...
  it('declines when no response can be attributed to this site', async () => {
    ...
    expect(plan.runnable.map((entry) => entry.reg.meta.id)).not.toContain(FakeHeadingsAudit.meta.id);
    expect(plan.skipped.find((stub) => stub.id === FakeHeadingsAudit.meta.id)?.status).toBe('na');
  });
