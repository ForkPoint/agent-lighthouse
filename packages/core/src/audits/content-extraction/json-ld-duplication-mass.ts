...
function bodyStrings(node: object): Array<{ property: string; text: string }> {
  ...
  const walk = (value: unknown, property: string): void => {
    if (typeof value === 'string') {
      if (value.length >= BODY_STRING_CHARS && BODY_PROPERTIES.has(property.toLowerCase())) {
    ...
    if (value && typeof value === 'object') {
      for (const [key, child] of Object.entries(value as Record<string, unknown>)) walk(child, key);
    }
  };
  walk(node, '');
  return out;
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const node of nodes) {
      ...
      if (count === 2) {
        ...
        duplicateTypes.push(Array.isArray(type) ? String(type[0]) : String(type ?? 'node'));
      }
    }
    ...
    for (const node of nodes) {
      for (const { property, text } of bodyStrings(node)) {
        ...
        if (overlap >= OVERLAP_REPORT_FLOOR) duplicatedTokens += Math.round(countTokens(text) * overlap);
      }
    }
    ...
    const found = `${jsonLdTokens} tokens of this ${documentTokens}-token page are structured data (${(share * 100).toFixed(1)}%); ${duplicatedTokens} of those repeat text already in the DOM${
      worstProperty ? ` (worst: ${worstProperty} at ${(worstOverlap * 100).toFixed(0)}% overlap)` : ''
    ...
    const expected = 'Structured data carries facts a parser needs, not a second copy of the page text';
