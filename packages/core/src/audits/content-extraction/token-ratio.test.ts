...
describe("TokenRatioAudit", () => {
  ...
  it("fails when tiny text is buried in huge markup padding", () => {
    const padding =
      `<div class="${"a".repeat(200)}" data-x="${"b".repeat(200)}"><span></span></div>`.repeat(
        100,
      );
  ...
  it("measures tokens, so base64 padding costs more than prose padding", () => {
    ...
    const prose = "the quick brown fox jumps over the lazy dog again ".repeat(
      105,
    );
    const ratioOf = (html: string) => {
      ...
      return (
        Number(result.details?.["contentTokens"]) /
        Number(result.details?.["deliveredTokens"])
      );
    };
    expect(Math.abs(base64.length - prose.length) / prose.length).toBeLessThan(
      0.05,
    );
  ...
  it("names the extractor it used for the numerator", () => {
    const text = "The mug holds three hundred millilitres of coffee. ".repeat(
      30,
    );
  ...
  it("splits the denominator into buckets that sum to the delivered tokens", () => {
    ...
    expect(
      d["script"] + d["style"] + d["comment"] + d["text"] + d["structure"],
    ).toBe(d["deliveredTokens"]);
  });
