...
describe('HeaderFooterAudit', () => {
  ...
  it('declines when no response can be attributed to this site', async () => {
    ...
    expect(plan.runnable.map((entry) => entry.reg.meta.id)).not.toContain(HeaderFooterAudit.meta.id);
    expect(plan.skipped.find((stub) => stub.id === HeaderFooterAudit.meta.id)?.status).toBe('na');
  });
});
