...
const page = (head: string, body = '<main><p>Kettles.</p></main>'): CheckContext =>
  mockCheckContext([
    mockPageContext('https://example.com/kettles', `<html><head>${head}</head><body>${body}</body></html>`, 1),
  ]);
...
describe('JsonLdDuplicationMassAudit', () => {
  ...
  it('reports a small Organization block without a finding', async () => {
    const result = await audit.audit(
      page(block({ '@context': 'https://schema.org', '@type': 'Organization', name: 'Kettle Co' })),
    );
  ...
  it('reports an articleBody that repeats text already in the DOM', async () => {
    ...
    expect(Number(result.details?.['duplicatedBodyOverlap'])).toBeGreaterThan(0.8);
  ...
  it('states the cost in the dossier terms', async () => {
    ...
    expect(result.found).toMatch(/\d+ tokens of this \d+-token page are structured data/);
  });
  ...
  it('never fails', async () => {
    ...
    const result = await audit.audit(
      page(
        block({ '@context': 'https://schema.org', '@type': 'Article', articleBody: body }),
        `<main><p>${body}</p></main>`,
      ),
    );
