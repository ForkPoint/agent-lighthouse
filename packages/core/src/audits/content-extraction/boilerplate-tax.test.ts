...
const body = (seed: string, n = 60) =>
  Array.from(
    { length: n },
    (_v, i) => `${seed}${i}alpha ${seed}${i}bravo ${seed}${i}charlie ${seed}${i}delta ${seed}${i}echo ${seed}${i}foxtrot.`,
  ).join(' ');
...
function site(pages: Array<{ url: string; content: string; chrome?: boolean }>): CheckContext {
...
describe('BoilerplateTaxAudit', () => {
  ...
  it('marks a shingle boilerplate at document frequency 0.8 but not at 0.6', async () => {
    ...
    const at80 = site([
      { url: 'https://example.com/a', content: withShared('alpha'), chrome: false },
      { url: 'https://example.com/b', content: withShared('bravo'), chrome: false },
      { url: 'https://example.com/c', content: withShared('charlie'), chrome: false },
      { url: 'https://example.com/d', content: withShared('delta'), chrome: false },
    ...
    expect(Number((await audit.audit(at80)).details?.['boilerplateShingles'])).toBeGreaterThan(0);
    expect(Number((await audit.audit(at60)).details?.['boilerplateShingles'])).toBe(0);
  });
  ...
  it('states the cost in the dossier terms: tokens paid for tokens received', async () => {
    ...
    const result = await audit.audit(ctx);
    expect(result.found).toMatch(/reading 3 pages .* pays \d+ tokens to receive \d+ tokens/);
  });
  ...
  it('stratifies the sample by URL path depth', async () => {
    ...
    const products = [
      ...
      { url: 'https://example.com/kettle', content: body('kettle'), chrome: false },
    ];
