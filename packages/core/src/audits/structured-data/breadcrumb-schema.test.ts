...
const breadcrumb = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 2, name: 'Cat', item: 'https://example.com/cat/' },
  ],
};
...
describe('BreadcrumbSchemaAudit', () => {
  ...
  it('passes when all deep pages have BreadcrumbList', () => {
    const ctx = mockCheckContext([page('https://example.com/cat/sub', ld(breadcrumb))]);
    const result = audit.audit(ctx);
  ...
  it('handles a page with an invalid URL gracefully (urlDepth catch block)', () => {
    ...
    const invalidUrlPage = { ...page('https://example.com/cat/item'), url: 'not-a-url' };
