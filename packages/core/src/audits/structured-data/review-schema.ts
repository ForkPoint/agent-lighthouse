...
const STAR_WIDGET_CLASS =
  /(star[-_ ]?rating|rating[-_ ]?stars?|stars?[-_ ]?widget)/i;
...
function matchesAnyType(
  ...
  if (Array.isArray(st))
    return st.some((t) => typeof t === "string" && types.includes(t));
  return false;
}
...
function tallyRatings(nodes: object[]): RatingTally {
  ...
  for (const node of nodes) {
    ...
    const usable =
      (isUsableAggregateRating(rating) ? 1 : 0) + usableReviewCount(reviews);
    if (usable === 0) continue;

    total += usable;
    if (matchesAnyType(rec, PRODUCT_TYPES)) onProduct += usable;
    if (rating && typeof rating === "object") counted.add(rating as object);
    for (const r of asArray(reviews))
      if (r && typeof r === "object") counted.add(r as object);
  }
  ...
  for (const node of nodes) {
    ...
    const rec = node as Record<string, unknown>;
    if (
      matchesAnyType(rec, ["AggregateRating"]) &&
      isUsableAggregateRating(rec)
    )
      total++;
    else if (
      matchesAnyType(rec, ["Review", "UserReview", "CriticReview"]) &&
      isUsableReview(rec)
    ) {
      total++;
    }
...
function hasStructuralReviewContent(page: PageContext): boolean {
  ...
  components += page.$(
    '[itemtype*="Review"], [itemprop="review"], [itemprop="reviewRating"]',
  ).length;
...
export class ReviewSchemaAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const productPages = findings.filter(
      (f) =>
        f.ratings.onProduct > 0 ||
        /\/(products?|items?|p|dp)\b/i.test(f.page.url),
    );
    const productPagesWithRating = productPages.filter(
      (f) => f.ratings.onProduct > 0,
    );

    if (
      productPages.length > 0 &&
      productPagesWithRating.length === 0 &&
      total > 0
    ) {
      return this.warn(
