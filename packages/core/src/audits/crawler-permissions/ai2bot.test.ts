...
describe('Ai2botAudit', () => {
  const audit = new Ai2botAudit();

  it('passes when AI2Bot is explicitly allowed in robots.txt', () => {
  ...
  it('warns when AI2Bot is allowed only via wildcard (not explicit)', () => {
  ...
  it('fails when AI2Bot is blocked via Disallow: /', () => {
    const robots = 'User-agent: AI2Bot\nDisallow: /';
    const ctx = mockCheckContext([], {
      '/robots.txt': mockFetchResult(robots, 200),
    });
