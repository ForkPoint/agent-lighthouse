...
describe('ApplebotExtendedAudit', () => {
  const audit = new ApplebotExtendedAudit();

  it('passes when Applebot-Extended is explicitly allowed in robots.txt', () => {
  ...
  it('warns when Applebot-Extended is allowed only via wildcard (not explicit)', () => {
  ...
  it('fails when Applebot-Extended is blocked via Disallow: /', () => {
    const robots = 'User-agent: Applebot-Extended\nDisallow: /';
    const ctx = mockCheckContext([], {
      '/robots.txt': mockFetchResult(robots, 200),
    });
