...
import { VNode, toVNode, parseTabindex, registerMatchers, type AnyNode } from './core';
import {
  getExplicitRole,
  getImplicitRole,
  getRole,
...
import { isVisibleToScreenReaders, isVisibleOnScreen, isFocusable, querySelectorAll } from './dom';
...
const ariaHasAttr: MatchFn = (_node, vNode) => vNode.attrNames.some((attr) => attr.startsWith('aria-'));
...
const autocompleteMatch: MatchFn = (_node, vNode) => {
  ...
  if (['textarea', 'input', 'select'].includes(nodeName) === false) return false;
  const ariaReadonly = vNode.attr('aria-readonly') || 'false';
  if (vNode.hasAttr('readonly') || ariaReadonly.toLowerCase() === 'true') return false;
  const excludedInputTypes = ['submit', 'reset', 'button', 'hidden'];
  if (nodeName === 'input' && excludedInputTypes.includes(vNode.props.type as string)) return false;
  const ariaDisabled = vNode.attr('aria-disabled') || 'false';
  if (vNode.hasAttr('disabled') || ariaDisabled.toLowerCase() === 'true') return false;
...
const frameTitleHasTextMatch: MatchFn = (node) => {
  const title = (node as unknown as { getAttribute: (a: string) => string | null }).getAttribute('title');
...
const noExplicitNameRequiredMatch: MatchFn = (_node, vNode) => {
  ...
  const { accessibleNameRequired } = (ariaRoles[role] || {}) as { accessibleNameRequired?: boolean };
  if (accessibleNameRequired || isFocusable(vNode)) return true;
  return false;
};

const labelMatch: MatchFn = (_node, vNode) => {
  if (vNode.props.nodeName !== 'input' || vNode.hasAttr('type') === false) return true;
...
const noNamingMethodMatch: MatchFn = (_node, vNode) => {
  const { namingMethods } = getElementSpec(vNode) as { namingMethods?: string[] };
  if (namingMethods && namingMethods.length !== 0) return false;
  if (getExplicitRole(vNode) === 'combobox' && querySelectorAll(vNode, 'input:not([type="hidden"])').length) {
    return false;
  }
...
const landmarkUniqueMatch: MatchFn = (_node, vNode) => isLandmark(vNode) && isVisibleToScreenReaders(vNode);
...
const RULES: RuleDef[] = [
  { id: 'aria-hidden-body', selector: 'body', matches: isInitiator, excludeHidden: false, all: [], any: ['aria-hidden-body'], none: [] },
  { id: 'aria-roles', selector: '[role]', matches: noEmptyRole, excludeHidden: true, all: [], any: [], none: ['invalidrole', 'abstractrole', 'unsupportedrole'] },
  { id: 'aria-deprecated-role', selector: '[role]', matches: noEmptyRole, excludeHidden: true, all: [], any: [], none: ['deprecatedrole'] },
  { id: 'aria-allowed-role', selector: '[role]', matches: ariaAllowedRoleMatch, excludeHidden: false, all: [], any: ['aria-allowed-role'], none: [] },
  { id: 'aria-valid-attr', selector: '*', matches: ariaHasAttr, excludeHidden: true, all: [], any: ['aria-valid-attr'], none: [] },
  { id: 'aria-valid-attr-value', selector: '*', matches: ariaHasAttr, excludeHidden: true, all: ['aria-valid-attr-value', 'aria-errormessage', 'aria-level'], any: [], none: [] },
  { id: 'aria-allowed-attr', selector: '*', matches: ariaHasAttr, excludeHidden: true, all: ['aria-allowed-attr', 'aria-allowed-attr-elm'], any: [], none: ['aria-unsupported-attr'] },
  { id: 'aria-prohibited-attr', selector: '*', matches: ariaHasAttr, excludeHidden: true, all: [], any: [], none: ['aria-prohibited-attr'] },
  { id: 'aria-required-attr', selector: '[role]', excludeHidden: true, all: [], any: ['aria-required-attr'], none: [] },
  { id: 'aria-required-children', selector: '[role]', matches: ariaRequiredChildrenMatch, excludeHidden: true, all: [], any: ['aria-required-children'], none: [] },
  { id: 'aria-required-parent', selector: '[role]', matches: ariaRequiredParentMatch, excludeHidden: true, all: [], any: ['aria-required-parent'], none: [] },
  { id: 'duplicate-id-aria', selector: '[id]', matches: duplicateIdAriaMatch, excludeHidden: false, all: [], any: ['duplicate-id-aria'], none: [], reviewOnFail: true },
  { id: 'autocomplete-valid', selector: '*', matches: autocompleteMatch, excludeHidden: true, all: ['autocomplete-valid'], any: [], none: [] },
  { id: 'nested-interactive', selector: '*', matches: nestedInteractiveMatch, excludeHidden: true, all: [], any: ['no-focusable-content'], none: [] },
  { id: 'td-has-header', selector: 'table', matches: dataTableLargeMatch, excludeHidden: true, all: ['td-has-header'], any: [], none: [] },
  { id: 'th-has-data-cells', selector: 'table', matches: dataTableMatch, excludeHidden: true, all: ['th-has-data-cells'], any: [], none: [] },
  { id: 'td-headers-attr', selector: 'table', matches: tableOrGridRoleMatch, excludeHidden: true, all: ['td-headers-attr'], any: [], none: [] },
  { id: 'scope-attr-valid', selector: 'td[scope], th[scope]', excludeHidden: true, all: ['html5-scope', 'scope-value'], any: [], none: [] },
  { id: 'document-title', selector: 'html:not(html *)', matches: isInitiator, excludeHidden: true, all: [], any: ['doc-has-title'], none: [] },
  { id: 'frame-title', selector: 'frame, iframe', matches: noNegativeTabindexMatch, excludeHidden: true, all: [], any: ['non-empty-title', 'aria-label', 'aria-labelledby', 'presentational-role'], none: [] },
  ...
  { id: 'link-name', selector: 'a[href]', excludeHidden: true, all: [], any: ['has-visible-text', 'aria-label', 'aria-labelledby', 'non-empty-title'], none: ['focusable-no-name'] },
  { id: 'label', selector: 'input, textarea', matches: labelMatch, excludeHidden: true, all: [], any: ['implicit-label', 'explicit-label', 'aria-label', 'aria-labelledby', 'non-empty-title', 'non-empty-placeholder', 'presentational-role'], none: ['hidden-explicit-label'] },
  { id: 'select-name', selector: 'select', excludeHidden: true, all: [], any: ['implicit-label', 'explicit-label', 'aria-label', 'aria-labelledby', 'non-empty-title', 'presentational-role'], none: ['hidden-explicit-label'] },
  { id: 'aria-dialog-name', selector: '[role="dialog"], [role="alertdialog"]', matches: noNamingMethodMatch, excludeHidden: true, all: [], any: ['aria-label', 'aria-labelledby', 'non-empty-title'], none: [] },
  { id: 'landmark-unique', selector: '[role=banner], [role=complementary], [role=contentinfo], [role=main], [role=navigation], [role=region], [role=search], [role=form], form, footer, header, aside, main, nav, section', matches: landmarkUniqueMatch, excludeHidden: true, all: [], any: ['landmark-is-unique'], none: [] },
];
...
function runCheck(checkId: string, node: AnyNode, vNode: VNode): RawCheckResult {
  ...
  return { result, data: builder._data, related: builder._related, vNode, node };
}
...
export function runRule(rule: RuleDef, doc: Document): RuleResult {
  ...
  for (const cid of checkIds) {
    ...
    if (!after) continue;
    const list = perNode.map((p) => ({ data: p.byCheck[cid].data, result: p.byCheck[cid].result }));
  ...
  for (const p of perNode) {
    ...
    const removed = checkIds.some((cid) => (p.byCheck[cid] as { _removed?: boolean })._removed);
    if (removed) continue;
...
export function runRules(doc: Document, ruleIds: string[]): Record<string, RuleResult> {
  setDocument(doc);
