...
function assess(
  ...
  // ── identity and copy ───────────────────────────────────────
  const itemId =
    text(product["sku"]) ?? text(product["mpn"]) ?? text(product["productID"]);
  if (!itemId) {
    problems.push(
      "no item_id source: the Product carries no sku, mpn or productID",
    );
  } else if (itemId.length > CAPS.item_id) {
    problems.push(
      `item_id is ${itemId.length} characters, over the ${CAPS.item_id}-character cap`,
    );
  } else {
    row.item_id = itemId;
  }
  ...
  if (!title) problems.push("no name to map to the feed title");
    ...
    problems.push(
      `title is ${title.length} characters, over the ${CAPS.title}-character cap`,
    );
  ...
  if (!description) problems.push("no description");
    ...
    problems.push(
      `description is ${description.length} characters, over the ${CAPS.description}-character cap`,
    );
  }
  ...
  if (!brand) problems.push("no brand name");
    ...
    problems.push(
      `brand is ${brand.length} characters, over the ${CAPS.brand}-character cap`,
    );
  ...
  if (!/^https:\/\//i.test(url))
    problems.push("url is not an absolute HTTPS URL");
  else row.url = url;
  const canonical = page.$('link[rel="canonical"]').attr("href");
  if (
    canonical &&
    text(product["url"]) &&
    new URL(canonical, page.url).toString() !== url
  ) {
    problems.push(
      "the Product url does not match the page canonical, so the feed row and the page disagree",
    );
  }
  ...
  if (!image) problems.push("no image");
    ...
    problems.push("image_url is not an absolute HTTPS URL");
  else if (!/\.(jpe?g|png|webp)(\?|$)/i.test(image)) {
    ambiguous.push(
      `image_url ${image} is not a JPEG, PNG or WebP by extension`,
    );
  ...
  if (!offer) {
    ...
    problems.push(
      "the Product carries an AggregateOffer, so its price is not a single resolvable number",
    );
  } else {
    ...
    if (!/^[A-Z]{3}$/.test(currency)) {
      problems.push(
        `priceCurrency "${currency}" is not a 3-letter ISO 4217 code`,
      );
    ...
    const listFromSpec =
      spec && /ListPrice/i.test(String(spec["priceType"] ?? ""))
        ? Number(spec["price"])
        : undefined;
    const listPrice =
      listFromSpec ??
      (typeof offer["highPrice"] === "number" ? offer["highPrice"] : undefined);
    if (
      price !== undefined &&
      listPrice !== undefined &&
      ...
      problems.push(
        `the sale price ${price} is higher than the list price ${listPrice}`,
      );
    }
    ...
    if (!availability) {
      ...
      if (!mapped) {
        ambiguous.push(
          `availability ${availability} maps to no feed enum value, so the row's stock state is ambiguous`,
        );
      } else {
        row.availability = mapped;
        if (
          DATED_AVAILABILITY.has(mapped) &&
          !text(offer["availabilityStarts"])
        ) {
          problems.push(
  ...
  const gtin = gtinKey
    ? text(product[gtinKey])!.replace(/\s|-/g, "")
    : undefined;
  if (!gtin && !text(product["mpn"])) {
    problems.push(
      "no gtin and no mpn, so the row is rejected unless identifier_exists is set to no",
    );
  }
  if (gtin) {
    if (!/^\d+$/.test(gtin) || !GTIN_LENGTHS.has(gtin.length)) {
      ...
      if (Number(gtin[gtin.length - 1]) !== expected) {
        problems.push(
          `${gtinKey} "${gtin}" has the wrong check digit — it should end in ${expected}`,
        );
      } else row.gtin = gtin;
    }
  }
