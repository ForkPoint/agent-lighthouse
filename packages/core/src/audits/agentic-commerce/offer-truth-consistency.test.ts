...
const SOLD_OUT =
  '<h1>Merino Crew</h1><p class="price">£59.00</p><p>Sold out</p>';
const IN_STOCK =
...
describe("OfferTruthConsistencyAudit", () => {
  ...
  it("fails InStock markup when the add-to-cart control is disabled", () => {
    const body =
      '<h1>Merino Crew</h1><p class="price">£59.00</p><button disabled>Add to cart</button>';
    expect(strings(run(body + ld(offer())), "failures")[0]).toContain(
      "disabled",
    );
  });
  ...
  it("fails two Product nodes for one URL that disagree", () => {
    const r = run(
      IN_STOCK +
        ld([
          offer({ priceValidUntil: "2099-12-31" }),
