...
function assessShipping(offer: Record<string, unknown>): ShippingLeg {
  ...
  const currency =
    rate && typeof rate["currency"] === "string" ? rate["currency"] : undefined;
  ...
  if (!handling || !transit) {
    const misplaced =
      details["handlingTime"] !== undefined ||
      details["transitTime"] !== undefined;
    problems.push(
      misplaced
  ...
  const region =
    destination && typeof destination["addressRegion"] === "string"
...
function assessReturns(
  ...
  const hasLink =
    typeof policy["merchantReturnLink"] === "string" &&
    policy["merchantReturnLink"].trim();
  ...
  if (!category || !RETURN_CATEGORIES.has(category)) {
    problems.push(
      "returnPolicyCategory is missing or outside the three-value enum",
    );
  }
  if (typeof country !== "string" || !/^[A-Za-z]{2}$/.test(country)) {
    problems.push(
      "applicableCountry is missing or is not an ISO 3166-1 alpha-2 code",
    );
  }
  ...
  return {
    ...
    ...(days !== undefined && days > 0
      ? { feed: `return_deadline_in_days=${days}` }
      : {}),
  };
}
...
export class LandedCostAndReturnsAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const problems = [
      ...shipping.problems,
      ...(returns.linkOnly ? [] : returns.problems),
    ];
