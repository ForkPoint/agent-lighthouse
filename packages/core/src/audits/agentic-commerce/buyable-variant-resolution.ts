...
function missingOfferFields(offers: Record<string, unknown>[]): string[] {
  ...
  const plain =
    offers.find((offer) => !typesOf(offer).includes("AggregateOffer")) ??
    offers[0]!;
  const missing: string[] = [];
  if (str(plain["price"]) === undefined) missing.push("offers.price");
  if (str(plain["priceCurrency"]) === undefined)
    missing.push("offers.priceCurrency");
  if (str(plain["availability"]) === undefined)
    missing.push("offers.availability");
  return missing;
}
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const page of productPages) {
      ...
      const groups = nodes.filter((node) =>
        typesOf(node).includes("ProductGroup"),
      );
      const products = nodes.filter((node) =>
        typesOf(node).includes("Product"),
      );
      ...
      for (const group of groups) {
        ...
        if (str(group["productGroupID"]) === undefined)
          problems.push("productGroupID");
        ...
        variants.forEach((variant, index) => {
          ...
          if (identifierOf(variant) === undefined)
            problems.push(`${name}: sku or gtin`);
      ...
      const aggregateOnly = allOffers.every((offer) =>
        typesOf(offer).includes("AggregateOffer"),
      );
      ...
      if (groupProblems.length > 0) {
        ...
        failures.push(
          `${label} shows ${evidence.count} variant(s) and publishes no Offer at all`,
        );
