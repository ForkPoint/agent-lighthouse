...
import { BuyableVariantResolutionAudit, detectVariants } from './buyable-variant-resolution';
...
function run(body: string) {
  return new BuyableVariantResolutionAudit().audit(mockCheckContext([product(body)]));
}
...
describe('BuyableVariantResolutionAudit', () => {
  ...
  it('is notApplicable when the product page offers no variant choice', () => {
    const r = run(ld({ '@type': 'Product', sku: 'MC-100', offers: { '@type': 'Offer', price: '79' } }));
    expect(r.status).toBe('na');
  });

  it('passes a ProductGroup that resolves every variant it shows', () => {
    const r = run(SIZE_SELECT + ld(GROUP([variant('MC-100-M', '79.00'), variant('MC-100-L', '79.00')])));
  ...
  it('passes two Product nodes each carrying a unique identifier and a complete Offer', () => {
    const r = run(SIZE_SELECT + ld([variant('MC-100-M', '79.00'), variant('MC-100-L', '89.00')]));
    expect(r.status).toBe('pass');
    ...
    const r = run(
      SIZE_SELECT +
        ld({
          '@type': 'Product',
          name: 'Merino Crew',
          offers: { '@type': 'AggregateOffer', lowPrice: '49', highPrice: '79', priceCurrency: 'GBP' },
        }),
    );
  ...
  it('names the exact missing per-variant field', () => {
    ...
    const r = run(SIZE_SELECT + ld(GROUP([incomplete, { '@type': 'Product', color: 'Blue' }])));
    expect(r.status).toBe('fail');
  ...
  it('warns when the markup resolves fewer variants than the page shows', () => {
    ...
    const r = run(three + ld(GROUP([variant('MC-100-M', '79.00'), variant('MC-100-L', '79.00')])));
