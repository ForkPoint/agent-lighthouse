...
const FULL = {
  ...
  description: 'A complete resoling kit for welted boots, with outsole, midsole and adhesive.',
  sku: 'ARK-001',
...
function run(overrides: Record<string, unknown> = {}, offerOverrides: Record<string, unknown> = {}) {
  ...
  if (overrides['offers']) product.offers = overrides['offers'] as typeof FULL.offers;
  const page = mockPageContext(PRODUCT_URL, `<html><head>${ld(product)}</head><body><main><p>Widget</p></main></body></html>`, 1);
...
describe('CheckoutOfferFieldMappingAudit', () => {
  ...
  it('is notApplicable when no product page was scanned', () => {
    const home = mockPageContext('https://example.com/', '<html><head></head><body><p>Home</p></body></html>', 0);
  ...
  it('fails an AggregateOffer, which carries no single resolvable price', () => {
    const result = run({
      offers: { '@type': 'AggregateOffer', lowPrice: 19.99, highPrice: 39.99, priceCurrency: 'USD' },
    });
    expect(result.status).toBe('fail');
  ...
  it('warns on an availability value that maps to no feed enum', () => {
    const result = run({}, { availability: 'https://schema.org/LimitedAvailability' });
