...
function run(options: RunOptions = {}) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (path === '/cart' && options.cart === false) return mockFetchResult('', 404);
    if (o.userAgent === BASELINE_UA) return mockFetchResult(BODY, 200, 'text/html');
...
describe('AgentUaCommerceParityAudit', () => {
  ...
  it('fails on a challenge interstitial and names the fingerprint', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: () => mockFetchResult('<html><title>Just a moment...</title><body>checking</body></html>', 200, 'text/html'),
    });
    expect(result.status).toBe('fail');
  ...
  it('fails on soft cloaking where the agent gets a fraction of the product page', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: (url) => (url === PDP_URL ? mockFetchResult(THIN, 200, 'text/html') : undefined),
    });
    expect(result.status).toBe('fail');
  ...
  it('probes the policy URLs the page links', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: (url) => (url.endsWith('/terms') ? mockFetchResult('', 403) : undefined),
    });
    expect(result.status).toBe('fail');
