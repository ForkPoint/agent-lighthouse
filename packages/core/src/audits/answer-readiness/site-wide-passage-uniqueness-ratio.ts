...
function cluster(measures: PageMeasure[]): Cluster[] {
  ...
  return [...groups.entries()]
    ...
    .map(([root, members]) => ({
      members,
      similarity: strongest.get(root) ?? 1,
    }));
}
...
export class SiteWidePassageUniquenessRatioAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const measure of measures) {
      for (const sentence of new Set(measure.sentences)) {
        documentFrequency.set(
          sentence,
          (documentFrequency.get(sentence) ?? 0) + 1,
        );
      }
    }
    const threshold = Math.max(
      BOILERPLATE_MIN_PAGES,
      Math.ceil(BOILERPLATE_SHARE * measures.length),
    );
    ...
    for (const measure of measures) {
      ...
      measure.uniqueFraction =
        measure.chars === 0 ? 0 : (measure.chars - repeated) / measure.chars;
    }

    const fractions = measures
      .map((m) => m.uniqueFraction)
      .sort((a, b) => a - b);
    const median = fractions[Math.floor(fractions.length / 2)] ?? 0;
    const flagged = measures.filter(
      (m) => m.uniqueFraction < UNIQUE_FRACTION_FLOOR,
    );
    ...
    const details = {
      ...
      lowUniquenessPages: flagged.map(
        (m) => `${m.url} (${(m.uniqueFraction * 100).toFixed(0)}% unique)`,
      ),
