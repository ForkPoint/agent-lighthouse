...
const UNIT_TOKEN =
  /[$€£¥₹]|%|\b(?:usd|eur|gbp|jpy|kg|mg|km|cm|mm|ms|kwh|mph|kmh|gb|mb)\b/gi;
  ...
  rows.forEach((tr, rowIndex) => {
    ...
    for (const node of cells) {
      ...
      const colspan = Math.min(
        MAX_SPAN,
        Math.max(1, Number(node.attribs["colspan"]) || 1),
      );
      const rowspan = Math.min(
        MAX_SPAN,
        Math.max(1, Number(node.attribs["rowspan"]) || 1),
      );
      const cell: Cell = {
...
function serialize(grid: Cell[][]): string {
  ...
  const written = grid.map((row, r) =>
    row.map((cell, c) =>
      cell.row === r && cell.column === c ? cell.text : "",
    ),
  );
...
function diff(grid: Cell[][], reparsed: string[][]): string[] {
  ...
  for (let r = 0; r < grid.length; r += 1) {
    ...
    for (let c = 0; c < row.length; c += 1) {
      ...
      const cause =
        row[c]!.colspan > 1 || row[c]!.rowspan > 1 ? "spanned cell" : "cell";
      lost.push(
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    tables.forEach((table, index) => {
          ...
          const declared = $(tr)
            ...
            .reduce(
              (sum, cell) =>
                ...
                Math.min(
                  MAX_SPAN,
                  Math.max(1, Number(cell.attribs["colspan"]) || 1),
                ),
              0,
            );
          const filled =
            grid[rowIndex]?.filter((c) => c.text !== "").length ?? 0;
          if (declared !== width && filled !== width) {
            ragged.push(
              `row ${rowIndex + 1} declares ${declared} cell(s) where the header has ${width}`,
            );
          }
        });
      ...
      const blockContent = cells
        ...
        .map(
          (cell) =>
            `<${cell.tagName}> "${flatten($(cell).text()).slice(0, 60)}" carries block content`,
        );
      ...
      const inside = grid
        ...
        .join(" ");
      const strandedUnits = [
        ...new Set(outside.match(UNIT_TOKEN) ?? []),
      ].filter(
        (token) =>
          !new RegExp(token.replace(/[$€£¥₹%]/g, "\\$&"), "i").test(inside),
      );
    ...
    const broken = reports.filter(
      (r) =>
        r.lost.length > 0 || r.headerlessNumeric || r.strandedUnits.length > 0,
    );
    const corrupting = reports.filter(
      (r) => r.headerlessNumeric || r.strandedUnits.length > 0,
    );
    ...
    for (const report of reports) {
      const label =
        report.caption === ""
          ? `table ${report.index}`
          : `table ${report.index} (“${report.caption}”)`;
      for (const cell of report.lost.slice(0, REPORTED_CELLS))
        findings.push(`${label}: ${cell}`);
      if (report.lost.length > REPORTED_CELLS) {
        findings.push(
          `${label}: ${report.lost.length - REPORTED_CELLS} further cell(s) lost`,
        );
      }
      for (const row of report.ragged) findings.push(`${label}: ${row}`);
      for (const cell of report.blockContent)
        findings.push(`${label}: ${cell}`);
      if (report.nested)
        findings.push(
          `${label}: contains a nested table, which markdown cannot express`,
        );
      if (report.headerlessNumeric) {
        findings.push(
          `${label}: two or more columns of numbers and no header cell to name them`,
        );
      }
    ...
    const expected =
      "Every main-content table converts to markdown and back with no cell lost";
    ...
    const details = {
      ...
      blockContentCells: reports.reduce(
        (sum, r) => sum + r.blockContent.length,
        0,
      ),
      headerlessNumericTables: reports.filter((r) => r.headerlessNumeric)
        .length,
      strandedUnitTables: reports.filter((r) => r.strandedUnits.length > 0)
        .length,
