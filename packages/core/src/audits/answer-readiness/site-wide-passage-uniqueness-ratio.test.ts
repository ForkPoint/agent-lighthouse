...
const own = (seed: string, n = 10) => {
  ...
  return Array.from(
    { length: n },
    (_v, i) =>
      `${Seed}${i}alpha ${seed}${i}bravo ${seed}${i}charlie ${seed}${i}delta ${seed}${i}echo.`,
  );
};
...
describe("SiteWidePassageUniquenessRatioAudit", () => {
  ...
  it("calls a sentence boilerplate on three pages but not on two", async () => {
    const onTwo = site([
      {
        ...
        sentences: [...own("alpha"), SHARED[0]!],
      },
      {
        url: "https://example.com/b",
        sentences: [...own("bravo"), SHARED[0]!],
      },
      { url: "https://example.com/c", sentences: own("charlie") },
    ]);
    expect((await audit.audit(onTwo)).details?.["boilerplateSentences"]).toBe(
      0,
    );
    ...
    const onThree = site([
      ...
      },
      {
        ...
        sentences: [...own("charlie"), SHARED[0]!],
      },
    ]);
    expect((await audit.audit(onThree)).details?.["boilerplateSentences"]).toBe(
      1,
    );
  });
  ...
  it("flags a page under 30% unique sentence text while the median page passes", async () => {
    const ctx = site([
      ...
      {
        url: "https://example.com/c",
        sentences: [...own("charlie"), ...SHARED],
      },
      {
        url: "https://example.com/thin",
        sentences: [...SHARED, ...own("delta", 1)],
      },
    ]);
