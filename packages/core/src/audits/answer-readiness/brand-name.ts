...
function findJsonLdByType(
  ...
  return (flattenJsonLd(jsonLd) as Record<string, unknown>[]).filter(
    (record) => {
      const objType = record["@type"];
      if (!objType) return false;
      const typeArr = Array.isArray(objType) ? objType : [objType];
      return typeArr.some(
        (t) => typeof t === "string" && lowerTypes.has(t.toLowerCase()),
      );
    },
  );
}
...
function stripLegalSuffix(name: string): string {
  return name
    .replace(/[,]?\s+(?:inc|llc|ltd|co|corp|gmbh|company)\.?$/i, "")
    .trim();
}
...
export class BrandNameAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const p of ctx.pages) {
      const orgs = findJsonLdByType(p.jsonLd, [
        "Organization",
        "LocalBusiness",
        "Corporation",
      ]);
    ...
    const matchNames = [
      ...new Set(
        candidates.flatMap((c) => [c, stripLegalSuffix(c)]).filter(Boolean),
      ),
