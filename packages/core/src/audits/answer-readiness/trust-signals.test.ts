...
function homepage(body: string, htmlAttrs = "") {
  return mockPageContext(
    "https://example.com/",
    `<html${htmlAttrs}><body>${body}</body></html>`,
  );
}
...
describe("TrustSignalsAudit", () => {
  ...
  it("requires both measured factors to pass", () => {
    const both = audit.audit(
      mockCheckContext([
        homepage(
          `<main><p>Rated 4.8 out of 5 across 1,204 reviews.</p>${CITATIONS}</main>`,
        ),
      ]),
    );
    const ratingOnly = audit.audit(
      mockCheckContext([
        homepage(
          "<main><p>Rated 4.8 out of 5 across 1,204 reviews.</p></main>",
        ),
      ]),
    );
  ...
  it("warns rather than fails when review markup is the only factor present", () => {
    ...
    const bare = audit.audit(
      mockCheckContext([homepage("<main><p>We build software.</p></main>")]),
    );
    const withMarkup = audit.audit(
      mockCheckContext([
