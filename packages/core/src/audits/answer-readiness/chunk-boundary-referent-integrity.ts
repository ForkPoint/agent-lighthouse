...
function chunksOf(page: PageContext): Chunk[] {
  ...
  for (const node of container
    .find("h2, h3, p, li, td, dd, blockquote")
    .toArray() as Element[]) {
...
<p>Descale the copper kettle monthly with equal parts water and white vinegar.</p>`,
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const entityStems = entities.map((entity) =>
      normalizeText(entity).split(" ").map(stem).join(" "),
    );
    ...
    for (const chunk of chunks) {
      ...
      if (opener) {
        const after = first
          ...
          .trim()
          .split(/\s+/)
          .slice(0, ANAPHORA_WINDOW);
        const headingWords = new Set(
          normalizeText(chunk.heading).split(" ").map(stem),
        );
        const resolved = after.some((word) =>
          headingWords.has(stem(normalizeText(word))),
        );
        if (!resolved) {
      ...
      if (wordCount(chunk.text) >= ENTITY_MIN_WORDS) {
        ...
        if (
          !entityStems.some(
            (entity) => entity !== "" && haystack.includes(entity),
          )
        ) {
          chunk.flags.push("entityAbsent");
          if (chunk.evidence === "")
            chunk.evidence = sentences(chunk.text)[0] ?? "";
        }
      }
      ...
      if (positional) {
        ...
        if (chunk.evidence === "") {
          chunk.evidence =
            sentences(chunk.text).find((s) => POSITIONAL.test(s)) ??
            positional[0];
    ...
    const lines = failing.map(
      (chunk) =>
        `"${chunk.heading}": ${chunk.flags.join(", ")} — "${chunk.evidence}"`,
    );
    const found =
      failing.length === 0
