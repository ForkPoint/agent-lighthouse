...
const ANSWER = 'Resoling replaces the outsole and midsole of a welted boot while keeping the upper.';
...
function faqJsonLd(answer: string) {
  return `<script type="application/ld+json">${JSON.stringify({
    ...
    '@type': 'FAQPage',
    mainEntity: [
      { '@type': 'Question', name: 'Do prices change?', acceptedAnswer: { '@type': 'Answer', text: answer } },
    ],
  })}</script>`;
}
...
describe('TextFragmentAddressabilityAudit', () => {
  ...
  it('does not fail on a <meta http-equiv="Document-Policy"> with no such header', () => {
    const result = run(SIMPLE, '<meta http-equiv="Document-Policy" content="force-load-at-top">');
  ...
  it('flags a soft hyphen in an answer span as a normalization hazard', () => {
    const result = run(`<main><h2>What is resoling?</h2><p>Resoling replaces the out­sole and midsole.</p></main>`);
