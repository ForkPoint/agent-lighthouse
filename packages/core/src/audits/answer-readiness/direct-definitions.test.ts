...
const page = (body: string, lang = 'en', url = 'https://example.com/blog/post') =>
  mockPageContext(url, `<html lang="${lang}"><body><main>${body}</main></body></html>`);
...
describe('DirectDefinitionsAudit', () => {
  ...
  describe('definitional intent gate', () => {
    it('is na when no page has definitional intent', () => {
      const result = audit.audit(
        mockCheckContext([
          page('<h1>Our 2026 roadmap</h1><p>We shipped a lot of things this year.</p>'),
        ]),
      );
    ...
    it('reads intent from a "what is" heading', () => {
      const result = audit.audit(
        mockCheckContext([
          page('<h1>What is retrieval-augmented generation?</h1><p>Some prose here.</p>'),
        ]),
      );
    ...
    it('reads intent from a glossary heading', () => {
      const result = audit.audit(
        mockCheckContext([page('<h1>Glossary</h1><p>Terms we use across the docs.</p>')]),
      );
      expect(result.status).not.toBe('na');
    });
    ...
    it('reads intent structurally from definition markup, whatever the language', () => {
      const result = audit.audit(
        mockCheckContext([
          page('<h1>Notizen</h1><p><dfn>Kubernetes</dfn> orchestriert Container.</p>', 'de'),
        ]),
      );
  ...
  describe('language-neutral detection', () => {
    it('detects intent in each supported language', () => {
      ...
      for (const [lang, heading] of cases) {
        const result = audit.audit(
          mockCheckContext([page(`<h1>${heading}</h1><p>Body copy.</p>`, lang)]),
        );
  ...
  describe('explicit definition markup', () => {
    ...
    it('does not count a spec-sheet <dl> with fragment values', () => {
      const result = audit.audit(
        mockCheckContext([
          page('<h1>What is the X200?</h1><dl><dt>Weight</dt><dd>200g</dd></dl>'),
        ]),
      );
      expect(result.status).toBe('warn');
    });
  ...
  describe('prose definitions are not punished', () => {
    ...
    it('warns when only some definitional pages carry markup', () => {
      const result = audit.audit(
        mockCheckContext([
          page('<h1>Glossary</h1><dl><dt>RAG</dt><dd>Retrieval-augmented generation, a way to ground answers in sources.</dd></dl>'),
          page(
