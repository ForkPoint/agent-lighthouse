...
function serializeAtomic($: CheerioAPI, el: Element): string {
  ...
  return ($(el).find("li").toArray() as Element[])
    .map(
      (li, index) =>
        `${index + 1}. ${$(li).text().replace(/\s+/g, " ").trim()}`,
    )
    .join("\n");
}
...
export class SectionSplitRiskProfileAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const $ = page.$;
    const container = $("main, article").first().length
      ? $("main, article").first()
      : $("body");
    ...
    for (const heading of headings) {
      ...
      const text =
        `${$(heading).text()} ${nodes.map((node) => $(node).text()).join(" ")}`
          .replace(/\s+/g, " ")
          .trim();
      ...
      if (section.tokens > WINDOW_TOKENS) {
        section.findings.push(
          `SPLIT(${Math.ceil(section.tokens / WINDOW_TOKENS) - 1} headless tails)`,
        );
      }
      if (section.tokens < THIN_TOKENS) section.findings.push("THIN");

      for (const node of nodes) {
        if (!["table", "ol", "ul"].includes(node.tagName.toLowerCase()))
          continue;
        const serialized = serializeAtomic($, node);
        const tokens = countTokens(serialized);
        if (tokens > WINDOW_TOKENS) {
          section.findings.push(
            `ATOMIC-SPLIT(${node.tagName.toLowerCase()}, ${tokens} tokens)`,
          );
        }
      }
    ...
    const score = blob
      ...
      : sectionTokens === 0
        ? 0
        : insideWindow / sectionTokens;
    const headingDistance = sections.reduce(
    ...
    const worstSeverity = sections.reduce(
      (max, section) =>
        Math.max(
          max,
          Math.max(0, Math.ceil(section.tokens / WINDOW_TOKENS) - 1),
        ),
      0,
    );
    ...
    const lines = sections
      ...
      .map(
        (section) =>
          `"${section.heading}" (${section.tokens} tokens): ${section.findings.join(", ")}`,
      );
    if (blob) {
      lines.unshift(
