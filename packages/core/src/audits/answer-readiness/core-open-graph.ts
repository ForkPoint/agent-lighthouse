...
function twitterReport(meta: MetaBag): string {
  ...
  if (
    ...
    lines.push(
      '  twitter:card is "summary_large_image" but no image is declared — X renders no image',
    );
  }
...
export class CoreOpenGraphAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const siteNameUsable =
      Boolean(siteName) && !isPlaceholderSiteName(siteName);
