...
const withDesc = (desc: string, head = '', body = '') =>
  mockCheckContext([
    mockPageContext('https://example.com/', doc(`<meta name="description" content="${desc}">${head}`, body)),
  ]);
...
describe('MetaDescriptionAudit', () => {
  ...
  it('skips the overlap check when the page has neither a title nor an H1', () => {
    const result = audit.audit(
      withDesc('An accurate page summary with no headings on the page to compare it against here.'),
    );
    expect(result.status).toBe('pass');
  });
  ...
  it('reports the length problem before the quality problems', () => {
    const result = audit.audit(withDesc('Discover more.', '<title>Merino Wool Sweaters</title>'));
    expect(result.status).toBe('warn');
    expect(result.message).toContain('should be 50-300');
  });
