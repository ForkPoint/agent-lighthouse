...
export function findUpdateIndicator(
  ...
  if (jsonLdModified)
    return { value: jsonLdModified, source: "JSON-LD dateModified" };

  const metaModified =
    p.meta["article:modified_time"] ?? p.meta["og:updated_time"];
  if (metaModified?.trim()) {
    const key = p.meta["article:modified_time"]?.trim()
  ...
  if (domHit)
    return {
      value: domHit.slice(0, 100),
      source: 'visible "last updated" label on a <time>',
    };
  ...
  while ((m = UPDATED_PATTERN.exec(text)) !== null) {
    ...
    if (DATE_PATTERN.test(window)) {
      return {
        value: window.trim().slice(0, 100),
        source: 'visible "last updated" text',
      };
    }
  }
