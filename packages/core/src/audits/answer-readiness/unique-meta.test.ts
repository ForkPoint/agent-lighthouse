...
describe('UniqueMetaAudit', () => {
  ...
  it('declines when no response can be attributed to this site', async () => {
    ...
    const plan = planAudits(unreachedSiteContext([...pages, second], rootFiles), defaultConfig);
    expect(plan.runnable.map((entry) => entry.reg.meta.id)).not.toContain(UniqueMetaAudit.meta.id);
    expect(plan.skipped.find((stub) => stub.id === UniqueMetaAudit.meta.id)?.status).toBe('na');
  });
});
