 ...
 * language/locale prefix (`/en/cart`, `/en-gb/checkout`).
 */
function classify(pathname: string): { segment: string; rest: string } | undefined {
  ...
  return SEGMENT_TO_FAMILY.has(head) ? { segment: head, rest: pathname } : undefined;
}
...
function observedCandidates(ctx: CheckContext): Candidate[] {
  ...
  const note = (raw: string, base: string) => {
    ...
    if (!hit || bySegment.has(hit.segment)) return;
    bySegment.set(hit.segment, {
...
export class SensitivePathsAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const label = (list: Candidate[]) => list.map((c) => `${c.family.id} (${c.path})`).join(', ');
    ...
    const fixCode = crawled.map((c) => `Disallow: /${c.segment}`).join('\n');
