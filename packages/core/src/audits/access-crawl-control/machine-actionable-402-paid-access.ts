...
function readX402(raw: string): { ok: boolean; problem?: string } {
  ...
  if (typeof parsed !== 'object' || parsed === null) {
    return { ok: false, problem: 'PAYMENT-REQUIRED decodes to a value that is not an object' };
  }
  ...
  if (!Array.isArray(accepts) || accepts.length === 0) {
    return { ok: false, problem: 'PAYMENT-REQUIRED carries no non-empty accepts array' };
  }
  ...
  for (const [index, item] of accepts.entries()) {
    ...
    const missing = required.filter((key) => (item as Record<string, unknown>)[key] === undefined);
    if (missing.length > 0) {
      return { ok: false, problem: `accepts[${index}] is missing ${missing.join(', ')}` };
    }
  }
  return { ok: true };
}
...
function rslCoversCrawl(xml: string, path: string): boolean {
  ...
  return $('content')
    ...
    .some((el) => {
      ...
      const covers = path.startsWith(prefix.startsWith('http') ? new URL(prefix).pathname : prefix);
      ...
      return $(el)
        ...
        .some((payment) => {
          ...
          return isIso4217(amount.attr('currency') ?? '') && /^\d+(\.\d+)?$/.test(amount.text().trim());
        });
    });
}
...
export class MachineActionable402PaidAccessAudit extends Audit {
  static override meta: AuditMeta = {
    ...
    failureTitle: 'This site charges crawlers but does not say how to pay in a form they can read',
    description:
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const licenceAdvertised =
      directiveLines(robots?.status === 200 ? robots.body : '', 'license').length > 0 ||
      ctx.pages.some((page) => linksWithRel(page.fetchResult.headers?.['link'] ?? '', 'license').length > 0);
    ...
    for (const probe of probes) {
      ...
      const headers = probe.probeStatus === 402 ? probe.probeHeaders : probe.baselineHeaders;
      const path = (() => {
        try {
      ...
      if (price !== undefined) {
        if (!CRAWLER_PRICE.test(price.trim())) {
          problems.push(`crawler-price: "${price}" is not "<ISO 4217 code> <decimal>"`);
        } else if (!isIso4217(price.trim().split(/\s+/)[0]!)) {
          problems.push(`crawler-price: "${price}" names a currency that is not an active ISO 4217 code`);
        } else {
      ...
      if (cacheControl !== '' && !/no-store|private|max-age=0/i.test(cacheControl)) {
        problems.push(`Cache-Control: ${cacheControl} lets a shared cache serve this 402 to other clients`);
      }
    ...
    for (const challenge of challenges) {
      ...
      for (const problem of challenge.problems) findings.push(`${where}: ${problem}`);
      if (challenge.hitsBrowsers) {
        findings.push(`${where}: the browser baseline was answered 402 too, so the rule is hitting people`);
      }
    }

    const displayValue = `${challenges.length - unreadable.length}/${challenges.length} 402s payable`;
    const expected = 'Every 402 carries a payment mechanism a crawler can act on';
