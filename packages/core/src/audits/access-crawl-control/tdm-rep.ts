...
function readPolicyFile(ctx: CheckContext): FileVerdict {
  ...
  const entries = parsed.filter(
    (e): e is Record<string, unknown> =>
      ...
      e["tdm-reservation"] !== undefined,
  );
  if (entries.length !== parsed.length) return { kind: "wrong-shape" };

  const reservations = [
    ...new Set(entries.map((e) => String(e["tdm-reservation"]).trim())),
  ];
...
function readHeader(
  ...
  for (const page of ctx.pages) {
    ...
    if (value?.trim()) {
      return {
        value: value.trim(),
        policy: page.fetchResult.headers[POLICY_HEADER]?.trim(),
      };
    }
  }
  return undefined;
...
function readMeta(ctx: CheckContext): {
  values: string[];
  policy?: string;
  pageUrl?: string;
} {
