  ...
  for (const part of tokenize(header)) {
    ...
    if (colon !== -1 && !VALUED_DIRECTIVES.has(prefix)) {
      ...
      if (DIRECTIVE_META_NAMES.has(prefix)) {
        found.push({
          token: part.slice(colon + 1).trim(),
          source: `X-Robots-Tag (${prefix})`,
        });
      }
      continue;
    }
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const verdicts: PageVerdict[] = ctx.pages.map((page) => {
      ...
      return {
        url: page.url,
        isHomepage:
          page.url.replace(/\/$/, "") === ctx.baseUrl.replace(/\/$/, ""),
    ...
    const blockedContent = verdicts.filter(
      (v) => v.blocking.length > 0 && !v.isUtility,
    );
    const blockedUtility = verdicts.filter(
      (v) => v.blocking.length > 0 && v.isUtility,
    );
    const suppressedContent = verdicts.filter(
      (v) =>
        v.suppressing.length > 0 && v.blocking.length === 0 && !v.isUtility,
    );
