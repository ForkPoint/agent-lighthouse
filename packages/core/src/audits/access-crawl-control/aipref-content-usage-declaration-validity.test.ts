...
function site(robots?: string, headers: Record<string, string> = {}): CheckContext {
  ...
  if (robots !== undefined) rootFiles['/robots.txt'] = mockFetchResult(robots, 200, 'text/plain');
...
describe('AiprefContentUsageDeclarationValidityAudit', () => {
  ...
  it('fails an unknown category token', async () => {
    const result = await audit.audit(site('User-agent: *\nAllow: /\nContent-Usage: ai-input=n\n'));
    expect(result.status).toBe('fail');
    expect((result.details?.['syntaxErrors'] as string[])[0]).toContain('not an AIPREF category');
  });
  ...
  it('fails a legacy Content-Signal value and names the syntax', async () => {
    ...
    expect(result.status).toBe('fail');
    expect((result.details?.['syntaxErrors'] as string[])[0]).toContain('legacy Content-Signal syntax');
  });
  ...
  it('fails a declaration attached only to a disallowed path, naming the deciding rule', async () => {
    ...
    expect(result.status).toBe('fail');
    expect((result.details?.['inertDeclarations'] as string[])[0]).toContain('"disallow: /private/"');
  });
  ...
  it('fails when robots.txt and the response header disagree for the same path', async () => {
    ...
    expect(result.status).toBe('fail');
    expect((result.details?.['channelDisagreements'] as string[])[0]).toContain('train-ai over /');
  });
  ...
  it('declines a Content-Usage header a bot wall answering 200 attached', async () => {
    ...
    expect(
      plan.skipped.find(
        (stub) => stub.id === AiprefContentUsageDeclarationValidityAudit.meta.id,
      )?.status,
    ).toBe('na');
  });
