...
function run(options: RunOptions = {}) {
  ...
  if (options.llms) rootFiles['/llms.txt'] = mockFetchResult('# Site\n', 200, 'text/plain');
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    return options.probe ? options.probe() : mockFetchResult(FULL, 200, 'text/html');
  };
...
function withHeaders(result: FetchResult, headers: Record<string, string>): FetchResult {
...
describe('AiCrawlerEdgeParityAudit', () => {
  ...
  it('fails when robots.txt allows a crawler the edge answers with a non-2xx', async () => {
    const result = await run({ robots: ALLOW_ALL, probe: () => mockFetchResult('', 503) });
    expect(result.status).toBe('fail');
    expect(result.message).toContain('503');
  });

  it('passes when robots.txt disallows the crawler the edge also blocks', async () => {
    const result = await run({ robots: DENY_ALL, probe: () => mockFetchResult('', 503) });
  ...
  it('fails a Cloudflare challenge and names it', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: () => withHeaders(mockFetchResult('', 403, 'text/html'), { 'cf-mitigated': 'challenge' }),
    });
    expect(result.status).toBe('fail');
  ...
  it('fails a pay-per-crawl 402 and names it', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: () => withHeaders(mockFetchResult('', 402), { 'crawler-price': 'USD 0.01' }),
    });
    expect(result.status).toBe('fail');
  ...
  it('fails a proof-of-work wall and names it', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: () => mockFetchResult('<html><body>Protected by Anubis</body></html>', 200, 'text/html'),
    });
    expect(result.status).toBe('fail');
  ...
  it('warns on an opaque 403 and states why it cannot be adjudicated', async () => {
    const result = await run({
      robots: ALLOW_ALL,
      probe: () => withHeaders(mockFetchResult('', 403, 'text/html'), { server: 'cloudflare' }),
    });
  ...
  it('probes /llms.txt when the site publishes one', async () => {
    const result = await run({ robots: ALLOW_ALL, llms: true, probe: () => mockFetchResult('', 503) });
