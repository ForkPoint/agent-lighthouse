...
function site(spec: SiteSpec): CheckContext {
  ...
  if (spec.robots !== undefined) rootFiles['/robots.txt'] = mockFetchResult(spec.robots, 200, 'text/plain');
  if (spec.tdmrep !== undefined) {
    rootFiles['/.well-known/tdmrep.json'] = mockFetchResult(spec.tdmrep, 200, 'application/json');
  }
...
describe('AiUsageSignalCoherenceAcrossChannelsAudit', () => {
  ...
  it('reads a tdm-reservation of 1 as a denial and 0 as a permission', async () => {
    const denied = await audit.audit(
      site({ head: '<meta name="tdm-reservation" content="1">', robots: 'User-agent: *\nContent-Usage: train-ai=n\n' }),
    );
    expect(denied.status).toBe('pass');
    const contradicted = await audit.audit(
      site({ head: '<meta name="tdm-reservation" content="0">', robots: 'User-agent: *\nContent-Usage: train-ai=n\n' }),
    );
  ...
  it('reports a prepended Content-Signal block as an edge override, not an ordinary contradiction', async () => {
    ...
    expect(result.remediation).toContain('above your own robots.txt directives');
  });

  it('warns on total silence, with a different remedy from a contradiction', async () => {
    const result = await audit.audit(site({ robots: 'User-agent: *\nAllow: /\n' }));
  ...
  it('reads an inline RSL document without fetching anything', async () => {
    const result = await audit.audit(
      site({
        body:
          '<script type="application/rsl+xml"><rsl xmlns="https://rslstandard.org/rsl"><content url="/"><license><prohibits type="usage">ai-input</prohibits></license></content></rsl></script>',
        headers: { 'content-usage': 'ai-input=y' },
      }),
    );
    expect(result.status).toBe('fail');
    expect((result.details?.['channels'] as string[])).toContain('inline RSL document');
  });

  it('does not use a tdmrep.json that is a bare object, and says so', async () => {
    const result = await audit.audit(
      site({ tdmrep: '{"tdm-reservation": 1}', robots: 'User-agent: *\nContent-Usage: train-ai=y\n' }),
    );
    expect(result.status).toBe('pass');
