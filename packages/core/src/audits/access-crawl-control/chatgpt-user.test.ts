...
describe("ChatgptUserAudit", () => {
  const audit = new ChatgptUserAudit();

  it("passes when ChatGPT-User is explicitly allowed in robots.txt", () => {
  ...
  it("warns when ChatGPT-User is allowed only via wildcard (not explicit)", () => {
  ...
  it("fails when ChatGPT-User is blocked via Disallow: /", () => {
    const robots = "User-agent: ChatGPT-User\nDisallow: /";
    const ctx = mockCheckContext([], {
      "/robots.txt": mockFetchResult(robots, 200),
    });
