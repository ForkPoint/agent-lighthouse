...
function run(options: RunOptions = {}) {
  ...
  const body = options.inlineRsl
    ...
    : PAGE;
  const ctx = mockCheckContext([mockPageContext('https://example.com/', body)], rootFiles);

  const challenge = (): FetchResult => {
    const result = mockFetchResult(options.challengeBody ?? '<html>Pay up.</html>', 402, 'text/html');
    Object.assign(result.headers, options.challenge ?? {});
    return result;
  };

  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    if (new URL(o.url).pathname === '/sitemap.xml') return mockFetchResult('', 404, 'text/plain');
    if (o.userAgent === BASELINE_UA) {
      return options.hitBrowsers ? challenge() : mockFetchResult(PAGE, 200, 'text/html');
    }
...
const x402 = (overrides: Record<string, unknown> = {}) =>
  Buffer.from(
    JSON.stringify({
      x402Version: 1,
      accepts: [
        { scheme: 'exact', network: 'base', amount: '1000', asset: 'USDC', payTo: '0xabc' },
      ],
      ...overrides,
    }),
  ).toString('base64');
...
describe('MachineActionable402PaidAccessAudit', () => {
  ...
  it('accepts a well-formed crawler-price header', async () => {
    const result = await run({ challenge: { 'crawler-price': 'USD 0.01', 'cache-control': 'no-store' } });
    expect(result.status).toBe('pass');
    expect(strings(result, 'mechanisms')).toContain('crawler-price header');
  });

  it('accepts an x402 PAYMENT-REQUIRED challenge', async () => {
    const result = await run({ challenge: { 'payment-required': x402(), 'cache-control': 'no-store' } });
    expect(result.status).toBe('pass');
    expect(strings(result, 'mechanisms')).toContain('x402 PAYMENT-REQUIRED challenge');
  });
  ...
  it('accepts an RSL crawl payment covering the path', async () => {
    ...
    expect(result.status).toBe('pass');
    expect(strings(result, 'mechanisms')).toContain('RSL licence with a crawl payment');
  });
  ...
  it('reports a malformed price and a malformed x402 challenge as their own findings', async () => {
    ...
    expect(strings(price, 'findings').join(' ')).toContain('is not "<ISO 4217 code> <decimal>"');

    const currency = await run({ challenge: { 'crawler-price': 'XYZ 1.00' } });
    expect(strings(currency, 'findings').join(' ')).toContain('not an active ISO 4217 code');

    const empty = await run({ challenge: { 'payment-required': x402({ accepts: [] }) } });
    expect(strings(empty, 'findings').join(' ')).toContain('no non-empty accepts array');

    const missing = await run({
      challenge: { 'payment-required': Buffer.from(JSON.stringify({ x402Version: 1, accepts: [{ scheme: 'exact' }] })).toString('base64') },
    });
    expect(strings(missing, 'findings').join(' ')).toContain('is missing network, amount, asset, payTo');
  });

  it('reports a 402 a shared cache may store', async () => {
    const result = await run({ challenge: { 'crawler-price': 'USD 0.01', 'cache-control': 'public, max-age=3600' } });
  ...
  it('reports a 402 that the browser baseline received too', async () => {
    const result = await run({ challenge: { 'crawler-price': 'USD 0.01', 'cache-control': 'no-store' }, hitBrowsers: true });
    expect(result.status).toBe('warn');
