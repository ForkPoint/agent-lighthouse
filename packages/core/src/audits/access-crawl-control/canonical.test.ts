...
const self = (url: string, index = 0) =>
  page(url, `<link rel="canonical" href="${url}">`, index);
...
describe("CanonicalLinksAudit", () => {
  ...
  it("does not warn when a minority of declaring pages share a non-root target", () => {
    const ctx = mockCheckContext([
      ...
      page(
        "https://example.com/a",
        '<link rel="canonical" href="https://example.com/hub">',
        1,
      ),
      page(
        "https://example.com/b",
  ...
  it("warns on an off-domain canonical", () => {
    const ctx = mockCheckContext([
      ...
      page(
        ...
        '<link rel="canonical" href="https://medium.com/@x/post">',
        1,
      ),
    ]);
    const result = audit.audit(ctx);
