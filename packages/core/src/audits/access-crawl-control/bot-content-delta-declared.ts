...
function declaration(body: string): Declaration {
  ...
  for (const node of flattenJsonLd(extractJsonLd($))) {
    ...
    const parts = Array.isArray(node["hasPart"])
      ? node["hasPart"]
      : [node["hasPart"]];
    for (const raw of parts) {
      ...
      const selector =
        typeof raw["cssSelector"] === "string" ? raw["cssSelector"].trim() : "";
...
export class BotContentDeltaDeclaredAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const urls = sampleEntries(tree.entries, MAX_URLS).map(
      (entry) => entry.loc,
    );
    ...
    for (const probe of comparable) {
      ...
      const similarity = jaccard(
        shingles(probe.baselineText),
        shingles(probe.probeText),
      );
