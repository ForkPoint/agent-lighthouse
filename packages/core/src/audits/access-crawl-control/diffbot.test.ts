...
describe('DiffbotAudit', () => {
  const audit = new DiffbotAudit();

  it('passes when Diffbot is explicitly allowed in robots.txt', () => {
    const robots = 'User-agent: Diffbot\nAllow: /';
  ...
  it('warns when Diffbot is allowed only via wildcard (not explicit)', () => {
  ...
  it('fails when Diffbot is blocked via Disallow: /', () => {
    const robots = 'User-agent: Diffbot\nDisallow: /';
    const ctx = mockCheckContext([], {
      '/robots.txt': mockFetchResult(robots, 200),
    });
