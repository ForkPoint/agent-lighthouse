...
describe("DuckassistbotAudit", () => {
  ...
  it("passes when DuckAssistBot is explicitly allowed in robots.txt", () => {
    const robots = "User-agent: DuckAssistBot\nAllow: /";
  ...
  it("warns when DuckAssistBot is allowed only via wildcard (not explicit)", () => {
  ...
  it("fails when DuckAssistBot is blocked via Disallow: /", () => {
    const robots = "User-agent: DuckAssistBot\nDisallow: /";
    const ctx = mockCheckContext([], {
      "/robots.txt": mockFetchResult(robots, 200),
    });
