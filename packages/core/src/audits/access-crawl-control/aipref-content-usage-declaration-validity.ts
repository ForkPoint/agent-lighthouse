...
export class AiprefContentUsageDeclarationValidityAudit extends Audit {
  static override meta: AuditMeta = {
    ...
    failureTitle: 'The AI-usage preference this site publishes cannot be read as written',
    description:
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    if (robotsBody !== '') {
      ...
      for (const line of directiveLines(robotsBody, 'content-signal')) {
        legacyOnly.push(`robots.txt line ${line.line}: Content-Signal: ${line.value}`);
      }
    }
    ...
    for (const page of ctx.pages) {
      ...
      if (!header) continue;
      readDictionary(header, 'response header', '*', `Content-Usage header on ${page.url}`);
    }
    ...
    if (declarations.length === 0 && errors.length === 0 && legacyOnly.length === 0) {
      return this.notApplicable(
        ...
        robotsBody === '' ? 'No robots.txt and no Content-Usage header' : 'No Content-Usage directive',
      );
    }
    ...
    if (errors.length > 0 || disagreements.length > 0 || (declarations.length > 0 && effective.length === 0)) {
      return {
        ...this.fail(
          errors.length > 0
