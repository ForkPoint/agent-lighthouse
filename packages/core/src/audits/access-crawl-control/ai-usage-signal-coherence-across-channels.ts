...
function tdmrepSignals(body: string): {
  signals: Signal[];
  malformed: boolean;
} {
  let parsed: unknown;
...
export class AiUsageSignalCoherenceAcrossChannelsAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    if (
      ctx.pages.length === 0 &&
      (ctx.rootFiles["/robots.txt"]?.status ?? 0) !== 200
    ) {
    ...
    const earliestOperatorLine = Math.min(
      ...signals
        .filter(
          (s) =>
            s.line !== undefined && s.channel !== "robots.txt Content-Signal",
        )
        .map((s) => s.line!),
    ...
    for (let i = 0; i < signals.length; i += 1) {
      for (let j = i + 1; j < signals.length; j += 1) {
        ...
        const prepended = [a, b].find(
          (s) =>
            s.channel === "robots.txt Content-Signal" &&
            (s.line ?? 0) < earliestOperatorLine,
        );
