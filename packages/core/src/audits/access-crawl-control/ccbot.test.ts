...
describe('CcbotAudit', () => {
  const audit = new CcbotAudit();

  it('passes when CCBot is explicitly allowed in robots.txt', () => {
  ...
  it('warns when CCBot is allowed only via wildcard (not explicit)', () => {
  ...
  it('fails when CCBot is blocked via Disallow: /', () => {
    const robots = 'User-agent: CCBot\nDisallow: /';
    const ctx = mockCheckContext([], {
      '/robots.txt': mockFetchResult(robots, 200),
    });
