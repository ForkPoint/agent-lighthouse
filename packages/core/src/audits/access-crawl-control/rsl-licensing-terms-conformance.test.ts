...
describe('RslLicensingTermsConformanceAudit', () => {
  ...
  it('collects candidates from all four channels', async () => {
    const { ctx } = site({
      ...
      headers: { link: '<https://example.com/b.xml>; rel="license"; type="application/rsl+xml"' },
  ...
  it('reports a relative License: value rather than resolving it', async () => {
    ...
    expect(fetched).toEqual(['https://example.com/license.xml', 'https://example.com/rsl.xml']);
  });
  ...
  it('fails a wrong namespace', async () => {
    const { ctx } = site({
      robots: 'License: https://example.com/a.xml\n',
      files: { 'https://example.com/a.xml': rslFile(RSL.replace('https://rslstandard.org/rsl', 'https://example.com/ns')) },
    });
    const result = await audit.audit(ctx);
  ...
  it('fails a content prefix that does not cover the audited pages', async () => {
    const { ctx } = site({
      ...
      robots: 'License: https://example.com/a.xml\n',
      files: { 'https://example.com/a.xml': rslFile(RSL.replace('url="/"', 'url="/blog/"')) },
    });
    const result = await audit.audit(ctx);
    expect(result.status).toBe('fail');
    expect(strings(result, 'conformanceErrors').join(' ')).toContain('/articles/one');
  });

  it('validates the enumerated attributes and the amount', async () => {
    const broken = RSL.replace('type="usage">search', 'type="everything">search')
  ...
  it('reports a copyright with no contact and a wrong content type', async () => {
    const { ctx } = site({
      robots: 'License: https://example.com/a.xml\n',
      files: {
        'https://example.com/a.xml': rslFile(RSL.replace(' contactEmail="legal@example.com"', ''), 'text/xml'),
      },
    });
    const errors = strings(await audit.audit(ctx), 'conformanceErrors').join(' ');
