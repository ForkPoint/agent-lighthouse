...
export function signedHeaders(
  ...
  const signature = sign(
    null,
    Buffer.from(signatureBase(url, params), "utf8"),
    privateKey,
  );
  return {
...
export class WebBotAuthRequestToleranceAudit extends Audit {
  ...
  private static readonly AGENT_DIRECTORY =
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const baseline = await probeSecurityUrl(ctx, url, {
    ...
    const signed = await probeSecurityUrl(ctx, url, {
      ...
      headers: signedHeaders(
        url,
        Date.now(),
        WebBotAuthRequestToleranceAudit.AGENT_DIRECTORY,
      ),
    });
    if (!signed)
      return this.fail("Signed probe failed", "HTTP 200", "No response");
    ...
    const ratio =
      baseline.body.length === 0
        ? 1
        : signed.body.length / baseline.body.length;
    const differs = signed.status !== baseline.status || ratio < COLLAPSE_RATIO;
    const varyNamesSignature = SIGNATURE_HEADERS.some((header) =>
      vary.includes(header),
    );
    ...
    const expected =
      "A request carrying RFC 9421 signature headers is answered as the same request without them";
    ...
    if (
      ...
      (signed.status === 401 || signed.status === 403)
    ) {
      return {
        ...this.pass(
