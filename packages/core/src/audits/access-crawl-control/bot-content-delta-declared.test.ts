...
const SENTENCES = Array.from(
  { length: 40 },
  (_v, i) => `Paragraph ${i} explains how the resole process works in a workshop.`,
);
const FULL_TEXT = SENTENCES.join(' ');
const REWRITTEN_BASE = Array.from(
  { length: 40 },
  (_v, i) => `Section ${i} covers boot repair topics inside our facility today.`,
).join(' ');
/** Different words, padded to the browser text's exact length: only the shingles differ. */
const REWRITTEN_TEXT = REWRITTEN_BASE.padEnd(FULL_TEXT.length, ' nowadays').slice(0, FULL_TEXT.length);
...
function page(text: string, options: { jsonLd?: object; selectorClass?: string } = {}): string {
...
function run(options: { browser?: string; bot?: string; sitemap?: boolean } = {}) {
  const audit = new BotContentDeltaDeclaredAudit();
  const ctx = mockCheckContext([mockPageContext('https://example.com/', page(FULL_TEXT))]);
...
describe('BotContentDeltaDeclaredAudit', () => {
  ...
  it('fails when the crawler gets a fraction of the text and nothing declares it', async () => {
    const result = await run({ browser: page(FULL_TEXT), bot: page(STUB_TEXT) });
  ...
  it('fails on a shingle mismatch at an equal character count', async () => {
    const result = await run({ browser: page(FULL_TEXT), bot: page(REWRITTEN_TEXT) });
    expect(result.status).toBe('fail');
  ...
  it('passes a declared delta whose cssSelector resolves in the served DOM', async () => {
    const result = await run({
      browser: page(FULL_TEXT, { jsonLd: PAYWALL_MARKUP, selectorClass: 'paywalled-body' }),
      bot: page(STUB_TEXT, { jsonLd: PAYWALL_MARKUP, selectorClass: 'paywalled-body' }),
    });
