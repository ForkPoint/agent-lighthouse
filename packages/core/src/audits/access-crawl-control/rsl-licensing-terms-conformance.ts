...
export class RslLicensingTermsConformanceAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const line of directiveLines(robotsBody, "license")) {
      ...
      candidates.push({
        url: line.value,
        channel: `robots.txt License: line ${line.line}`,
    ...
    for (const page of ctx.pages) {
      ...
      for (const entry of linksWithRel(header, "license")) {
        if (
          (entry.params["type"] ?? "").split(";")[0]!.trim().toLowerCase() !==
          RSL_TYPE
        )
          continue;
        try {
          ...
          errors.push(
            `Link header on ${page.url} carries an unresolvable licence URL`,
          );
        }
      }
      ...
      page.$('link[rel~="license"]').each((_i, el) => {
        ...
        try {
          ...
          errors.push(
            `<link rel="license"> on ${page.url} carries an unresolvable href`,
          );
        }
      });
    ...
    if (candidates.length === 0) {
      return this.notApplicable(
        ...
        errors.length > 0
          ? errors.join("; ")
          : "No License: directive, licence link or inline RSL document",
      );
    }
    ...
    for (const candidate of candidates.slice(0, MAX_CANDIDATES)) {
      if (candidate.inlineXml !== undefined) {
        documents.push({
          candidate,
          xml: candidate.inlineXml,
          contentType: RSL_TYPE,
        });
        continue;
      }
      ...
      if (!(await isSafeUrl(candidate.url))) {
        errors.push(
          `${candidate.channel}: ${candidate.url} is not a URL this scanner will fetch`,
        );
        continue;
      }
      const result = await probeRsl(ctx, candidate.url, {
        followRedirects: true,
      });
      if (!result || result.status !== 200) {
        errors.push(
          `${candidate.channel}: ${candidate.url} answered HTTP ${result?.status ?? 0}`,
        );
        continue;
      }
      documents.push({
    ...
    for (const doc of documents) {
      ...
      if (doc.contentType !== RSL_TYPE) {
        errors.push(
          `${where}: served as "${doc.contentType || "no content type"}" rather than ${RSL_TYPE}`,
        );
      }
      ...
      if ((root.attr("xmlns") ?? "") !== RSL_NAMESPACE) {
        errors.push(
          `${where}: root <rsl> carries xmlns "${root.attr("xmlns") ?? "none"}", not ${RSL_NAMESPACE}`,
        );
        continue;
      }
      ...
      if (contents.length === 0) {
        errors.push(
          `${where}: no <content url=…> element, so the licence covers nothing`,
        );
        continue;
      }
      ...
      const uncovered = auditedPaths.filter(
      ...
      for (const licence of licences) {
        ...
        if (terms.length === 0) {
          errors.push(
            `${where}: a <license> carries no permits, prohibits or payment`,
          );
        }
        $licence.find("permits, prohibits").each((_i, el) => {
          ...
          if (!PERMIT_TYPES.has(type)) {
            errors.push(
              `${where}: <${el.tagName} type="${type || "missing"}"> is not one of usage, user, geo`,
            );
          }
        });
      ...
      $("amount").each((_i, el) => {
        ...
        if (!isIso4217(currency)) {
          errors.push(
            `${where}: <amount currency="${currency || "missing"}"> is not an active ISO 4217 code`,
          );
        }
        if (!/^\d+(\.\d+)?$/.test($(el).text().trim())) {
          errors.push(
            `${where}: <amount> value "${$(el).text().trim()}" does not parse as a decimal`,
          );
        }
      });
      ...
      if (
        copyright.length > 0 &&
        !copyright.attr("contactEmail") &&
        !copyright.attr("contactUrl")
      ) {
        errors.push(
          `${where}: <copyright> carries neither contactEmail nor contactUrl`,
        );
      }
