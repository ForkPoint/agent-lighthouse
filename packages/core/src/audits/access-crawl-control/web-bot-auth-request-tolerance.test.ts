...
const answer =
  (status: number, headers: Record<string, string> = {}, body = PAGE) =>
  () => {
...
describe("web-bot-auth signature construction", () => {
  ...
  it("computes the thumbprint over the three required JWK members", () => {
    ...
    expect(
      jwkThumbprint(
        createPublicKey(pem).export({ format: "pem", type: "spki" }).toString(),
      ),
    ).toBe(thumbprint);
  });
});
...
describe("WebBotAuthRequestToleranceAudit", () => {
  ...
  it("says a pass means the door is not nailed shut, not that signatures are verified", () => {
    expect(WebBotAuthRequestToleranceAudit.meta.description).not.toContain(
      "verifies",
    );
    expect(WebBotAuthRequestToleranceAudit.meta.guidance?.impact).toContain(
      "identification",
    );
  });
