...
describe('NestedInteractiveAudit', () => {
  it('registers under the nested-interactive id with its dossier and grade', () => {
    expect(NestedInteractiveAudit.meta.id).toBe('operability-safety/nested-interactive');
    expect(NestedInteractiveAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/nested-interactive.md');
  ...
  it('wires exactly its a11y rule(s)', () => {
    ...
    expect(result.expected).toBe('accessibility rules pass: nested-interactive');
  });

  it('fails when the `nested-interactive` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'nested-interactive': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(NestedInteractiveAudit, ctx);
