...
const items = (n: number, offset = 0) =>
  Array.from(
    { length: n },
    (_v, i) => `<div class="product-card"><a href="/p/${i + offset}">Mug ${i + offset}</a></div>`,
  ).join('');

/** A listing page whose only variable is the affordance under test. */
function listing(affordance: string, head = '', body = items(20)): CheckContext {
...
describe('UrlAddressableStateAndPaginationFallbackAudit', () => {
  ...
  it('passes href pagination and reports the deepest item index reachable by URL', async () => {
    const result = await audit.audit(
      listing('<nav class="pagination"><a href="?page=2">2</a><a href="?page=3">3</a></nav>'),
    );
  ...
  it('passes a listing whose only pagination is <link rel="next">', async () => {
    const result = await audit.audit(listing('', '<link rel="next" href="?page=2">'));
    expect(result.status).toBe('pass');
  });

  it('fails a listing whose only affordance is an infinite-scroll sentinel', async () => {
    const result = await audit.audit(listing('<div class="infinite-sentinel"></div>'));
  ...
  it('orders href pagination above a Load more button above a scroll sentinel', async () => {
    const href = await audit.audit(listing('<nav class="pagination"><a href="?page=2">2</a></nav>'));
    const button = await audit.audit(listing('<button class="load-more">Load more</button>'));
    const sentinel = await audit.audit(listing('<div class="infinite-sentinel"></div>'));
  ...
  it('accepts a facet whose filtered URL returns a different item count', async () => {
    ...
    const ctx = mockCheckContext([mockPageContext(LISTING_URL, html, 1)]);
    ctx.fetch = async (_options: FetchOptions) => mockFetchResult(filtered, 200, 'text/html');
    const result = await audit.audit(ctx);
  ...
  it('is notApplicable when the site has no listing page', async () => {
    const ctx = mockCheckContext([
      mockPageContext('https://example.com/about', '<html><body><p>About us.</p></body></html>', 1),
    ]);
    const result = await audit.audit(ctx);
