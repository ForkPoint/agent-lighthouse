...
describe('PresentationConflictAudit', () => {
  it('registers under the presentation-conflict id with its dossier and grade', () => {
    expect(PresentationConflictAudit.meta.id).toBe('operability-safety/presentation-conflict');
    expect(PresentationConflictAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/presentation-conflict.md');
  ...
  it('wires exactly its a11y rule(s)', () => {
    ...
    expect(result.expected).toBe('accessibility rules pass: presentation-role-conflict');
  });

  it('fails when the `presentation-role-conflict` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'presentation-role-conflict': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(PresentationConflictAudit, ctx);
