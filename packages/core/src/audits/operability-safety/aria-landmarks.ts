...
const REQUIRED_LANDMARKS = [
  {
    name: "banner/header",
    check: ($: PageContext["$"]) =>
      $('[role="banner"]').length > 0 || $("header").length > 0,
  },
  {
    name: "main",
    check: ($: PageContext["$"]) =>
      $('[role="main"]').length > 0 || $("main").length > 0,
  },
  {
    name: "navigation",
    check: ($: PageContext["$"]) =>
      $('[role="navigation"]').length > 0 || $("nav").length > 0,
  },
  {
    name: "contentinfo/footer",
    check: ($: PageContext["$"]) =>
      $('[role="contentinfo"]').length > 0 || $("footer").length > 0,
  },
];

export class AriaLandmarksAudit extends Audit {
  static override meta: AuditMeta = {
    id: "operability-safety/aria-landmarks",
