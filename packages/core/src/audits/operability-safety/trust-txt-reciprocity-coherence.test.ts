...
import {
  ...
  parseTrustTxt,
} from "./trust-txt-reciprocity-coherence";
import {
  mockPageContext,
  mockCheckContext,
  mockFetchResult,
...
function run(site: Site) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (typeof other === "number")
      return mockFetchResult("", other, "text/plain");
...
describe("TrustTxtReciprocityCoherenceAudit", () => {
  ...
  it("checks at most three associations and says what it skipped", async () => {
    const { result, requests } = run({
      trustTxt: ["a", "b", "c", "d", "e"]
        .map((h) => `belongto=https://${h}.test/`)
        .join("\n"),
      others: {},
    });
    const r = await result;
    expect(
      requests.filter((o) => o.url.endsWith(".test/trust.txt")),
    ).toHaveLength(3);
    expect(strings(r, "observations").join(" ")).toContain(
      "further association(s) were not checked",
    );
  });
