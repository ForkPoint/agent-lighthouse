...
function boundaryFor(
  ...
  const marked = links.filter((a) =>
    /\b(ugc|nofollow)\b/i.test(a.attribs?.["rel"] ?? ""),
  );
  if (!bounded && links.length > 0 && marked.length === links.length)
    bounded = true;
...
function survivingMarkup(
  ...
  if (scope.find("[style]").length > 0)
    issues.push("an inline style attribute survived");
  ...
  const foreignImage = (scope.find("img[src]").toArray() as Element[]).some(
    (img) => {
...
function regionsOf(page: PageContext): Region[] {
  ...
  const forms = ($("form").toArray() as Element[]).filter((form) => {
    if (/wp-comments-post\.php/i.test(form.attribs?.["action"] ?? ""))
      return true;
    return ($(form).find("textarea[name]").toArray() as Element[]).some(
      (area) => UGC_FIELD_RE.test(area.attribs?.["name"] ?? ""),
    );
  });
  ...
  for (const el of elements) {
    ...
    const text = $(el).text();
    const instruction =
      !bounded && INSTRUCTION_LEXICON.some((re) => re.test(text));
    if (instruction)
      issues.push("an instruction-shaped payload sits in this open region");
    regions.push({
      label: label(el),
      pageUrl: page.url,
  ...
  const declared = allJsonLdNodes(page.jsonLd).filter((node) => {
    ...
    return names.some(
      (name) => typeof name === "string" && UGC_TYPES.has(name.toLowerCase()),
    );
  });
  ...
  return [
    {
      ...
      pageUrl: page.url,
      issues: [
        "visitor-written content is declared in JSON-LD with no marked region around it",
      ],
      bounded: false,
      failing: false,
    },
...
<div data-nosnippet>
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const expected =
      'Every visitor-written region contained by data-nosnippet or marked rel="ugc"';
