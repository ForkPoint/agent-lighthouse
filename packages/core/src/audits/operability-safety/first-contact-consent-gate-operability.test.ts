...
const CMP = '<script src="https://cdn.cookielaw.org/scripttemplates/otSDKStub.js"></script>';
...
describe('FirstContactConsentGateOperabilityAudit', () => {
  ...
  it('reports a dialog whose root is a cross-origin iframe as unreachable', async () => {
    const result = await audit.audit(
      page(`<iframe id="onetrust-banner-sdk" src="https://consent.example.net/gate"></iframe>${ARTICLE}`),
    );
  ...
  it('reports main content hidden from the accessibility tree while the gate is open', async () => {
    const result = await audit.audit(
      page(`${DIALOG}<main aria-hidden="true"><article><p>Body</p></article></main>`),
    );
