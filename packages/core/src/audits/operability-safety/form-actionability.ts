...
const AUTOCOMPLETE_QUALIFIERS = new Set([
  "shipping",
  "billing",
  "home",
  "work",
  ...
  const tokens = value
    ...
    .filter(
      (t) =>
        ...
        !AUTOCOMPLETE_QUALIFIERS.has(t),
    );
  return (
    tokens.length > 0 &&
    STANDARD_AUTOCOMPLETE_TOKENS.has(tokens[tokens.length - 1])
  );
}
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const page of ctx.pages) {
      ...
      $("form").each((formIndex, formEl) => {
        ...
        const isWebmcpForm =
          ($(formEl).attr("toolname") ?? "").trim().length > 0;
        ...
        $(formEl)
          ...
          .each((_, el) => {
            ...
            issues.push({
              ...
              failedChecks: [
                "not a native form element (div pretending to be an input)",
              ],
            });
          });
        ...
        $(formEl)
          ...
          .each((_, inputEl) => {
            ...
            const type = (
              $input.attr("type") ?? (tag === "input" ? "text" : tag)
            ).toLowerCase();
            ...
            const hasAriaLabel =
              ($input.attr("aria-label") ?? "").trim().length > 0;
            const hasAriaLabelledby =
              ($input.attr("aria-labelledby") ?? "").trim().length > 0;
            if (
              !hasForLabel &&
              !hasWrappingLabel &&
              !hasAriaLabel &&
              ...
              failedChecks.push(
                "no explicit label (missing label[for], wrapping label, aria-label, or aria-labelledby)",
              );
            }
            ...
            const expectedToken = expectedAutocompleteToken(
              tag,
              type,
              name,
              id ?? "",
            ...
            if (
              expectedToken &&
              (!autocomplete || !isStandardAutocomplete(autocomplete))
            ) {
              failedChecks.push(
                autocomplete
    ...
    const issueSummary = issues
      .slice(0, 10)
      .map(
        (i) => `form #${i.formIndex} ${i.field}: ${i.failedChecks.join("; ")}`,
      )
      .join(" | ");
    const truncated =
    ...
    const found =
      issues.length > 0
        ? `${foundBase}. Non-actionable: ${issueSummary}${truncated}`
        : foundBase;
