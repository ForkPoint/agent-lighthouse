...
const IDENTITY_TYPES =
  /^(Organization|Corporation|NewsMediaOrganization|LocalBusiness|Person|Brand)$/;
...
export function wikidataId(raw: string): string | undefined {
  const match =
    /^https?:\/\/(www\.)?wikidata\.org\/(wiki|entity)\/(Q\d+)/i.exec(
      raw.trim(),
    );
...
export function officialWebsite(body: string): string | undefined {
  ...
  for (const claim of claims) {
    ...
    const datavalue =
      snak && isObject(snak["datavalue"]) ? snak["datavalue"] : undefined;
...
export class WikidataRoundTripVerificationAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    const entities = [
      ...new Set(ctx.pages.flatMap((page) => claimedEntities(page.jsonLd))),
    ];
    ...
    for (const id of entities.slice(0, MAX_ENTITIES)) {
      ...
      if (!(await isSafeUrl(url))) {
        warnings.push(
          `${id}: Wikidata could not be reached, so the claim is unverified`,
        );
        continue;
      }
      ...
      if (!response || response.status !== 200) {
        warnings.push(
          `${id}: Wikidata answered HTTP ${response?.status ?? 0}, so the claim is unverified`,
        );
        continue;
      }
    ...
    const expected =
      "Each claimed Wikidata entity gives this site’s domain as its official website (P856)";
