...
describe('DialogNameAudit', () => {
  it('registers under the dialog-name id with its dossier and grade', () => {
    ...
    expect(DialogNameAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/dialog-name.md');
    expect(DialogNameAudit.meta.evidenceGrade).toBe('A');
    expect(DialogNameAudit.meta.tier).toBe('scored');
  });

  it('wires exactly its a11y rule(s)', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'aria-dialog-name': { status: 'pass', nodes: [] },
      }),
    ]);
  ...
  it('fails when the `aria-dialog-name` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'aria-dialog-name': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(DialogNameAudit, ctx);
  ...
  it('passes when every constituent rule passes', () => {
    ...
    expect(runA11yAudit(DialogNameAudit, ctx).status).toBe('pass');
  });

  it('is na when no constituent rule applies', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'aria-dialog-name': { status: 'inapplicable', nodes: [] },
      }),
    ]);
    expect(runA11yAudit(DialogNameAudit, ctx).status).toBe('na');
  });
});
