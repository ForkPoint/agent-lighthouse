...
export class C2paManifestSurvivesDeliveryAudit extends Audit {
  static override meta: AuditMeta = {
    ...
    failureTitle: 'This site’s image pipeline strips Content Credentials before delivery',
    description:
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const manifestOf = async (url: string): Promise<boolean | undefined> => {
      ...
      if (bytes.length >= MAX_RESPONSE_BODY_BYTES) {
        skipped.push(`${url} (over the ${MAX_RESPONSE_BODY_BYTES}-byte read cap)`);
        return undefined;
      }
    ...
    for (const url of sample) {
      ...
      if (origin === undefined || origin === url || fetches >= MAX_IMAGES + sample.length) continue;
      const originHasManifest = await manifestOf(origin);
      if (originHasManifest === undefined) continue;
      if (originHasManifest && !hasManifest) {
        stripped.push(`${origin} carries a manifest; the served variant ${url} does not`);
    ...
    const expected = 'Every served image variant carries the Content Credentials its origin asset carries';
