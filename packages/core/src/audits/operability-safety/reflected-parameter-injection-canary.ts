...
type Sink = 'title' | 'meta description' | 'canonical link' | 'JSON-LD' | 'rendered text';

/** The four sinks lifted verbatim into an AI answer. Reflection there fails. */
const HIGH_TRUST: readonly Sink[] = ['title', 'meta description', 'canonical link', 'JSON-LD'];
...
function mintCanary(): string {
  let token = 'AGLH';
  for (let i = 0; i < 8; i += 1) token += Math.floor(Math.random() * 16).toString(16);
  return token;
}
...
function isNoindex(result: FetchResult, html: string): boolean {
  ...
  return $('meta[name="robots"], meta[name="googlebot"]').toArray().some((el) => {
...
function sinksFor(html: string, canary: string): Sink[] {
  ...
  if (jsonLdStrings(html).some((value) => value.includes(canary))) found.push('JSON-LD');
...
export class ReflectedParameterInjectionCanaryAudit extends Audit {
  static override meta: AuditMeta = {
    ...
    failureTitle: 'URL input is reflected into fields agents read as the page speaking',
    description:
...
<p>Results for <span>{{ query | escape }}</span></p>`,
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const probes = [
      ...PROBE_PARAMS.map((param) => `${origin}/?${param}=${encodeURIComponent(payload)}`),
    ...
    for (const url of probes) {
      ...
      for (const sink of sinks) if (!reflections.has(sink)) reflections.set(sink, url);
    }
    ...
    const expected = 'No probe token reflected into title, meta, canonical, JSON-LD or page text';
    ...
    if (sinkList.length === 0) {
      return {
        ...
        details: { probes: reachable, sinks: [], raw, indexable: indexableReflection },
      };
    }
