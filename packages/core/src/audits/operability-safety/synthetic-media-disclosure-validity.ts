...
export const DIGITAL_SOURCE_BASE =
...
const CAPTURE_CONCEPTS = new Set([
  "digitalCapture",
  "negativeFilm",
  "positiveFilm",
  "print",
...
export function digitalSourceType(xmp: string): string | undefined {
  const attribute =
    /Iptc4xmpExt:DigitalSourceType\s*=\s*["']([^"']*)["']/i.exec(xmp);
  if (attribute) return attribute[1]!.trim();
  const element =
    /<Iptc4xmpExt:DigitalSourceType[^>]*>([^<]*)<\/Iptc4xmpExt:DigitalSourceType>/i.exec(
      xmp,
    );
  if (element) return element[1]!.trim();
  const resource =
    /<Iptc4xmpExt:DigitalSourceType[^>]*rdf:resource\s*=\s*["']([^"']*)["']/i.exec(
      xmp,
    );
...
export function classifyDisclosure(raw: string): DisclosureVerdict {
  ...
  if (trimmed.startsWith(DIGITAL_SOURCE_BASE)) {
    ...
    if (!DIGITAL_SOURCE_CONCEPTS.has(concept))
      return { kind: "unknown-concept", concept };
    return hadSlash
      ? { kind: "trailing-slash", concept }
      : { kind: "valid", concept };
  }
  if (trimmed.startsWith(httpsBase)) {
    return { kind: "wrong-scheme", concept: trimmed.slice(httpsBase.length) };
  }
  if (DIGITAL_SOURCE_CONCEPTS.has(trimmed))
    return { kind: "bare-concept", concept: trimmed };
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const url of candidates.slice(0, MAX_IMAGES)) {
      ...
      if (manifest && verdict.kind !== "free-text") {
        ...
        const manifestSynthetic = [...SYNTHETIC_CONCEPTS].some((concept) =>
          store.includes(concept),
        );
        const manifestCapture = [...CAPTURE_CONCEPTS].some((concept) =>
          store.includes(concept),
        );
        if (manifestSynthetic && CAPTURE_CONCEPTS.has(verdict.concept)) {
          ...
          manifestCapture &&
          !manifestSynthetic &&
          SYNTHETIC_CONCEPTS.has(verdict.concept)
        ) {
          failures.push(
    ...
    const coverage =
      imagesRead === 0 ? 0 : Math.round((declared / imagesRead) * 100);
    const details = {
