...
const KEYBOARD_STATE =
  /:focus-within|:focus-visible|:focus|\[aria-expanded=["']?true["']?\]|\[data-open\b|\[open\b/i;
  ...
  for (const page of ctx.pages) {
    ...
    for (const rule of css.rules) {
      ...
      for (const el of match($, stripState(rule.selector))) {
        ...
        const declared = nearby.some((node) => {
          ...
          if (HANDLER_ATTRS.some((name) => $n.attr(name) !== undefined))
            return true;
          return false;
        });
        const controlled =
    ...
    $("[class]").each((_i, node) => {
      ...
      if (
        ...
        return;
      result.hoverCards += 1;
      if (!result.hoverCardSample)
        result.hoverCardSample = `.${cls.split(/\s+/)[0]}`;
    });
  }
...
export class HoverOnlyContentAndNavigationAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const more =
      urls.length > shown.length ? `, +${urls.length - shown.length} more` : "";
    const parts: string[] = [];
    if (s.menus.length > 0) {
      parts.push(
    ...
    if (s.hoverCards > 0)
      parts.push(`${s.hoverCards} unreferenced hover card(s)`);
