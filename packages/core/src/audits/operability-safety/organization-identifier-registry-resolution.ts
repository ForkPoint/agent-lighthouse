...
function claimsOn(page: PageContext): Claim[] {
  ...
  for (const node of allJsonLdNodes(page.jsonLd)) {
    ...
    const claim: Claim = {
      iso6523: (Array.isArray(iso) ? iso : [iso]).filter(
    ...
    if (
      claim.lei !== undefined ||
      claim.iso6523.length > 0 ||
      claim.duns !== undefined
    )
      out.push(claim);
  }
  return out;
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const claim of claims) {
      ...
      for (const value of claim.iso6523) {
        if (!/^\d{4}:/.test(value)) {
          failures.push(
            `iso6523Code "${value}" carries no four-digit issuing-agency prefix`,
          );
        }
      }
      ...
      if (
        claim.duns !== undefined &&
        !claim.iso6523.some((v) => v.startsWith(DUNS_PREFIX))
      ) {
        advisories.push(
      ...
      if (!LEI_SHAPE.test(lei)) {
        failures.push(
          `"${lei}" is not the shape of an LEI: 18 alphanumerics followed by two check digits`,
        );
        continue;
      }
      if (!leiCheckDigitsValid(lei)) {
        failures.push(
          `LEI "${lei}" fails its ISO/IEC 7064 check digits, so it is a typo or a fabrication`,
        );
        continue;
      }
      ...
      if (!(await isSafeUrl(url))) {
        warnings.push(
          `The GLEIF registry could not be reached to resolve ${lei}`,
        );
        continue;
      }
      const response = await probeSecurityUrl(ctx, url, {
        headers: { Accept: "application/vnd.api+json" },
      });
      if (!response || response.status !== 200) {
        warnings.push(
          `The GLEIF registry answered HTTP ${response?.status ?? 0} for ${lei}, so it could not be resolved`,
        );
        continue;
      }
      ...
      if (records.length === 0) {
        failures.push(
          `GLEIF holds no record for LEI ${lei}, so the identifier resolves to nothing`,
        );
        continue;
      }
      ...
      const registration = isObject(attributes["registration"])
        ? attributes["registration"]
        : {};
      const entityStatus = asString(entity["status"]) ?? "UNKNOWN";
      const registrationStatus = asString(registration["status"]) ?? "UNKNOWN";
      const legalNameNode = isObject(entity["legalName"])
        ? entity["legalName"]
        : {};
      const registeredName = asString(legalNameNode["name"]) ?? "";
      resolved.push(
        `${lei}: ${registeredName || "no registered name"} (${entityStatus}/${registrationStatus})`,
      );

      if (entityStatus !== "ACTIVE") {
        warnings.push(
          `LEI ${lei} resolves to an entity whose GLEIF status is ${entityStatus}`,
        );
      }
