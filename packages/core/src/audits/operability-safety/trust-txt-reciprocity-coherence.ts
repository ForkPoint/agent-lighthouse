  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const entry of entries) {
      if (!KNOWN_ATTRIBUTES.has(entry.name)) {
        observations.push(
          `line ${entry.line}: "${entry.name}" is not a trust.txt attribute`,
        );
      }
    }
    ...
    const associations = entries.filter(
      (entry) => entry.name === "belongto" || entry.name === "controlledby",
    );
    let checked = 0;
    ...
    for (const entry of associations) {
      if (checked >= MAX_ASSOCIATIONS) {
        observations.push(
          `${associations.length - checked} further association(s) were not checked`,
        );
        break;
      }
      ...
      if (!(await isSafeUrl(url))) {
        observations.push(
          `${entry.name}=${entry.value}: that domain could not be reached`,
        );
        continue;
      }
      checked += 1;
      ...
      if (listsUs) reciprocated.push(`${entry.name}=${entry.value}`);
        ...
        observations.push(
          `${entry.name}=${entry.value}: that domain's trust.txt carries no ${back}= line naming ${domain}`,
        );
    }

    // Policy coherence: two channels stating the same thing must agree.
    const declared = entries
      .find((entry) => entry.name === "datatrainingallowed")
      ?.value.toLowerCase();
    const robots = ctx.rootFiles["/robots.txt"];
    const groups =
      robots && robots.status === 200 ? parseRobots(robots.body) : [];
    const allowedCrawlers = AI_CRAWLERS.filter((bot) =>
      isPathAllowed(groups, bot, "/"),
    );
