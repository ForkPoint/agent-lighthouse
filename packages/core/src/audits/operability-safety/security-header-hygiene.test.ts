...
describe('SecurityHeaderHygieneAudit — never fails a site', () => {
  const cases: Array<[string, CheckContext]> = [
    ['a valid file', ctxWith({ '/.well-known/security.txt': mockFetchResult(VALID_SECURITY_TXT, 200) })],
    [
      'an expired file',
      ctxWith({
    ...
    [
      'an HTML soft-404',
      ctxWith({
        '/.well-known/security.txt': mockFetchResult('<!doctype html><html><body>Nope</body></html>', 200),
      }),
    ],
    ['a 404', ctxWith({ '/.well-known/security.txt': mockFetchResult('', 404) })],
...
describe('SecurityHeaderHygieneAudit — when the audit stays silent', () => {
  ...
  it('reports a site with no security.txt as not applicable, not a warning', () => {
    const result = audit.audit(ctxWith({ '/.well-known/security.txt': mockFetchResult('', 404) }));
...
describe('SecurityHeaderHygieneAudit — RFC 9116 conformance', () => {
  const withFile = (file: FetchResult) => audit.audit(ctxWith({ '/.well-known/security.txt': file }));
  ...
  it('falls back to the legacy top-level location', () => {
    const result = audit.audit(ctxWith({ '/security.txt': mockFetchResult(VALID_SECURITY_TXT, 200) }));
    expect(result.status).toBe('pass');
...
describe('SecurityHeaderHygieneAudit — the removed header signals', () => {
  ...
  const rootFiles = { '/.well-known/security.txt': mockFetchResult(VALID_SECURITY_TXT, 200) };

  it('ignores response headers entirely', () => {
    const withHeaders = audit.audit(mockCheckContext([pageWith(HEALTHY_HEADERS)], rootFiles));
    const withoutHeaders = audit.audit(mockCheckContext([pageWith({})], rootFiles));
  ...
  it('passes a valid security.txt on a site with none of the three headers', () => {
    expect(audit.audit(mockCheckContext([pageWith({})], rootFiles)).status).toBe('pass');
  });
