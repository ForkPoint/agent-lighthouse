...
type AfterFn = (
  results: { data: unknown; result: boolean | undefined }[],
) => { data: unknown; result: boolean | undefined }[];
...
const ariaAllowedRole: EvaluateFn = function (
  _node,
  options = {},
  virtualNode,
) {
  const { allowImplicit = true, ignoredTags = [] } = options as {
...
const ariaValidAttr: EvaluateFn = function (_node, options, virtualNode) {
  const opts = Array.isArray((options as { value?: unknown }).value)
    ? (options as { value: string[] }).value
    : [];
  const invalid: string[] = [];
  const aria = /^aria-/;
  virtualNode.attrNames.forEach((attr) => {
    if (opts.indexOf(attr) === -1 && aria.test(attr) && !validateAttr(attr))
      invalid.push(attr);
  });
  if (invalid.length) {
    this.data(invalid);
...
const ariaValidAttrValue: EvaluateFn = function (_node, options, virtualNode) {
  ...
  const preChecks: Record<string, (validValue?: boolean) => boolean | void> = {
    "aria-controls": () => {
      ...
      return (
        getAriaValue(virtualNode, "aria-expanded", { lowercase: true })
          ?.value !== "false" &&
        getAriaValue(virtualNode, "aria-selected", { lowercase: true })
          ?.value !== "false" &&
        hasPopup === false
      );
    },
    ...
    "aria-owns": () =>
      getAriaValue(virtualNode, "aria-expanded", { lowercase: true })?.value !==
      "false",
...
const ariaAllowedAttrElm: EvaluateFn = function (_node, _options, virtualNode) {
  const elmSpec = getElementSpec(virtualNode) as {
    allowedAriaAttrs?: string[];
  };
...
const ariaUnsupportedAttr: EvaluateFn = function (
  _node,
  _options,
  virtualNode,
) {
...
function listProhibitedAttrs(
  vNode: VNode,
  role: string | null,
  nodeName: string,
  elementsAllowedAriaLabel: string[],
): string[] {
...
const ariaProhibitedAttr: EvaluateFn = function (
  _node,
  options = {},
  virtualNode,
) {
  const elementsAllowedAriaLabel =
    (options as { elementsAllowedAriaLabel?: string[] })
...
function hasImplicitAttr(
  elmSpec: { implicitAttrs?: Record<string, unknown> },
  attr: string,
): boolean {
...
const ariaRequiredAttr: EvaluateFn = function (
  _node,
  options = {},
  virtualNode,
) {
  ...
  const opts = options as Record<string, unknown>;
  if (Array.isArray(opts[role as string])) {
    requiredAttrs = uniqueArray(
      opts[role as string] as string[],
      requiredAttrs,
    );
  }
  ...
  if (
    ...
    return true;
  if (role === "slider" && getAriaValue(virtualNode, "aria-valuetext")?.value)
    return true;
  const elmSpec = getElementSpec(virtualNode) as {
    implicitAttrs?: Record<string, unknown>;
  };
  const missingAttrs = requiredAttrs.filter(
    (ra) =>
      !getAriaValue(virtualNode, ra)?.value && !hasImplicitAttr(elmSpec, ra),
  );
...
function getOwnedRoles(
  virtualNode: VNode,
  required: string[],
): { role: string | null; attr?: string; vNode: VNode }[] {
  ...
  while ((vNode = ownedVirtual.shift())) {
    ...
    if (
      ...
      (["group", "rowgroup"].includes(role as string) &&
        required.some((r) => r === role))
    ) {
...
const ariaRequiredChildren: EvaluateFn = function (
  _node,
  options,
  virtualNode,
) {
  const reviewEmpty =
    options && Array.isArray((options as { reviewEmpty?: unknown }).reviewEmpty)
  ...
  const unallowed = ownedRoles.filter(
    ({ role, vNode }) =>
      vNode.props.nodeType === 1 && !required.includes(role as string),
  );
  ...
  this.data(required);
  if (
    reviewEmpty.includes(explicitRole as string) &&
    !ownedRoles.some(isContentOwned)
  )
    return undefined;
  return false;
};
...
function getMissingContext(
  ...
  while (vNode) {
    ...
    if (!role) {
      ...
      if (ownGroupRoles.includes(explicitRole as string))
        reqContext.push(explicitRole as string);
...
function getAriaOwnsIndex(root: AnyNode): Map<string, AnyNode[]> {
  ...
  const owners = scope.querySelectorAll
    ? Array.from(scope.querySelectorAll("[aria-owns]"))
    : [];
  for (const owner of owners) {
    const val =
      (
        owner as unknown as { getAttribute: (a: string) => string | null }
...
function getAriaOwners(element: AnyNode): AnyNode[] | null {
  ...
  let el: {
    ...
    parentElement?: AnyNode | null;
  } | null = element as unknown as {
    getAttribute?: (a: string) => string | null;
    parentElement?: AnyNode | null;
  };
  while (el) {
...
const ariaRequiredParent: EvaluateFn = function (node, options, virtualNode) {
  const ownGroupRoles =
    options &&
    Array.isArray((options as { ownGroupRoles?: unknown }).ownGroupRoles)
...
const hasGlobalAriaAttribute: EvaluateFn = function (
  _node,
  _options,
  virtualNode,
) {
...
const duplicateId: EvaluateFn = function (node) {
  const id = (
    (node as unknown as { getAttribute: (a: string) => string }).getAttribute(
      "id",
    ) || ""
  ).trim();
  if (!id) return true;
  const root = getRootNode(node) as unknown as {
    querySelectorAll: (s: string) => ArrayLike<AnyNode>;
  };
  ...
  cells.forEach((cell) => {
    if (hasContent(cell) && tableUtils.isDataCell(cell) && !label(cell)) {
      const hasHeaders = tableUtils
        ...
        .some((header) => header !== null && !!hasContent(header));
...
const thHasDataCells: EvaluateFn = function (node) {
  ...
  cells.forEach((cell) => {
    const headers = (
      cell as unknown as { getAttribute: (a: string) => string | null }
  ...
  const headers = cells.filter((cell) => {
    if (
      sanitize((cell as unknown as { textContent: string }).textContent) === ""
    )
      return false;
    return (
      (cell as unknown as { nodeName: string }).nodeName.toUpperCase() ===
        "TH" ||
      ["rowheader", "columnheader"].indexOf(getExplicitRole(cell) as string) !==
        -1
    );
  });
  const tableGrid = tableUtils.toGrid(node as never);
  ...
  headers.forEach((header) => {
    ...
    if (
      h.getAttribute("id") &&
      reffedHeaders.includes(h.getAttribute("id") as string)
    )
      return;
...
const tdHeadersAttrMessageKeys = [
...
const tdHeadersAttr: EvaluateFn = function (node) {
  ...
  const tableNode = node as unknown as {
    rows: ArrayLike<{ cells: ArrayLike<AnyNode> }>;
  };
  const cells: AnyNode[] = [];
    ...
    headers.forEach((headerId) => {
      if (cellId && headerId === cellId) badCells[selfRef].add(cell);
        ...
        !["columnheader", "rowheader"].includes(
          cellRoleById[headerId] as string,
        )
      )
...
const html5Scope: EvaluateFn = function (node) {
  ...
  return (
    (node as unknown as { nodeName: string }).nodeName.toUpperCase() === "TH"
  );
};
...
const ariaLabelledbyCheck: EvaluateFn = function (
  _node,
  _options,
  virtualNode,
) {
...
const metaRefresh: EvaluateFn = function (_node, options, virtualNode) {
  const { minDelay, maxDelay } = (options || {}) as {
    minDelay?: number;
    maxDelay?: number;
  };
  ...
  try {
    ...
    if (labelNode) {
      const implicit = sanitize(
        accessibleTextVirtual(labelNode, {
...
const landmarkIsUniqueAfter: AfterFn = function (results) {
  ...
  return results.filter((currentResult) => {
    const cur = currentResult.data as {
      role: string | null;
      accessibleText: string | null;
    };
    const matched = uniqueLandmarks.find((some) => {
      const s = some.data as {
        role: string | null;
        accessibleText: string | null;
      };
...
export const checks: Record<string, CheckDef> = {
  ...
  "aria-allowed-role": {
    evaluate: ariaAllowedRole,
    options: { allowImplicit: true, ignoredTags: [] },
  },
  ...
  "unique-frame-title": {
    evaluate: uniqueFrameTitle,
    after: uniqueFrameTitleAfter,
  },
  "meta-refresh": {
    evaluate: metaRefresh,
    options: { minDelay: 0, maxDelay: 72000 },
  },
};
