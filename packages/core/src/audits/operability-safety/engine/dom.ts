...
export function idrefs(
  ...
  try {
    const doc = getRootNode(
      node instanceof VNode ? node.actualNode : node,
    ) as unknown as {
    ...
    if (attrValue) {
      ...
      for (let i = 0; i < tokens.length; i++)
        result.push(doc.getElementById(tokens[i]));
    }
    return result;
  } catch {
...
export function getResolvedRefs(
  ...
  try {
    ...
    if (!attrValue) return [];
    const doc = getRootNode(domNode) as unknown as {
      getElementById: (id: string) => AnyNode | null;
    };
...
function contentVisibilityHidden(
  vNode: VNode,
  { isAncestor }: { isAncestor?: boolean } = {},
): boolean {
  return (
...
function isHiddenSelf(vNode: VNode, isAncestor: boolean): boolean {
  ...
  if (hiddenMethods.some((method) => method(vNode, { isAncestor })))
    return true;
  if (!(vNode.actualNode as unknown as { isConnected?: boolean }).isConnected)
    return true;
  return false;
}
...
export function isHiddenForEveryone(
  vNode: VNode | AnyNode,
  {
    skipAncestors,
    isAncestor = false,
  }: { skipAncestors?: boolean; isAncestor?: boolean } = {},
): boolean {
...
export function isVisibleOnScreen(node: VNode | AnyNode): boolean {
  ...
  while (v) {
    if (isHiddenForEveryone(v, { skipAncestors: true, isAncestor }))
      return false;
    if (
      v.actualNode &&
      screenHiddenMethods.some((method) => method(v as VNode))
    )
      return false;
    v = v.parent;
    isAncestor = true;
...
export function isVisibleToScreenReaders(node: VNode | AnyNode): boolean {
  ...
  let isAncestor = false;
  while (v) {
    if (ariaHidden(v) || isInert(v, { skipAncestors: true, isAncestor }))
      return false;
    if (isHiddenForEveryone(v, { skipAncestors: true, isAncestor }))
      return false;
    v = v.parent;
...
export function focusDisabled(el: VNode | AnyNode): boolean {
  ...
  if (
    (allowedDisabledNodeNames.includes(vNode.props.nodeName) &&
...
export function isNativelyFocusable(el: VNode | AnyNode): boolean {
  ...
  switch (vNode.props.nodeName) {
    ...
    case "button":
      return true;
    case "details":
      return !(vNode.actualNode as unknown as Element).querySelectorAll(
        "summary",
      ).length;
  }
  return false;
...
export function hasChildTextNodes(elm: VNode): boolean {
  ...
  return elm.children.some(
    ({ props }) => props.nodeType === 3 && (props.nodeValue || "").trim(),
  );
}

export function hasContentVirtual(
  elm: VNode,
  noRecursion?: boolean,
  ignoreAria?: boolean,
): boolean {
