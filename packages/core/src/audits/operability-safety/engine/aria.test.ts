...
describe("getAriaValue and hasAriaValue", () => {
  it("reads an aria attribute", () => {
    ...
    expect(getAriaValue(n, "aria-checked", { lowercase: true })?.value).toBe(
      "true",
    );
  ...
  it("reports an absent attribute", () => {
    expect(hasAriaValue(node('<div id="t"></div>'), "aria-checked")).toBe(
      false,
    );
  });
});
...
describe("getExplicitRole", () => {
  it("reads the role attribute", () => {
    expect(getExplicitRole(node('<div id="t" role="button">x</div>'))).toBe(
      "button",
    );
  });
  ...
  it("takes the first valid role of a fallback list", () => {
    expect(
      getExplicitRole(node('<div id="t" role="nope button">x</div>'), {
        fallback: true,
      }),
    ).toBe("button");
  });
  ...
  it("is null for an invented role", () => {
    expect(
      getExplicitRole(node('<div id="t" role="not-a-real-role">x</div>')),
    ).toBeNull();
  });
});
...
describe("getRole", () => {
  ...
  it('resolves role="presentation" rather than the implicit role', () => {
    expect(
      getRole(node('<img id="t" src="/x.png" alt="" role="presentation">')),
    ).toBe("presentation");
  });
});
...
describe("validateAttrValue", () => {
  it("accepts a valid token", () => {
    expect(
      validateAttrValue(
        node('<div id="t" role="checkbox" aria-checked="true"></div>'),
        "aria-checked",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a token outside the allowed set", () => {
    expect(
      validateAttrValue(
        node('<div id="t" role="checkbox" aria-checked="maybe"></div>'),
        "aria-checked",
      ),
    ).toBe(false);
  });
  ...
  it("accepts a valid integer value", () => {
    expect(
      validateAttrValue(
        node('<div id="t" role="heading" aria-level="2"></div>'),
        "aria-level",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a non-numeric value where a number is required", () => {
    expect(
      validateAttrValue(
        node('<div id="t" role="heading" aria-level="two"></div>'),
        "aria-level",
      ),
    ).toBe(false);
  });
  ...
  it("accepts an idref that resolves", () => {
    const n = node(
      '<span id="l">L</span><div id="t" aria-labelledby="l"></div>',
    );
...
describe("role allowance on an element", () => {
  it("allows a button role on an anchor with href", () => {
    expect(
      isAriaRoleAllowedOnElement(node('<a id="t" href="/x">x</a>'), "button"),
    ).toBe(true);
  });
  ...
  it("lists the roles an element may not carry", () => {
    expect(
      getElementUnallowedRoles(
        node('<input id="t" type="checkbox" role="button">'),
      ),
    ).toContain("button");
  });
  ...
  it("lists nothing for an element carrying an allowed role", () => {
    expect(
      getElementUnallowedRoles(node('<div id="t" role="button">x</div>')),
    ).toEqual([]);
  });
});
...
describe("getOwnedVirtual", () => {
  ...
  it("includes a node pulled in by aria-owns", () => {
    const n = node(
      '<ul id="t" aria-owns="extra"><li>a</li></ul><li id="extra">b</li>',
    );
...
describe("arialabelText", () => {
  it("reads aria-label", () => {
    expect(arialabelText(node('<div id="t" aria-label="Close">x</div>'))).toBe(
      "Close",
    );
  });
