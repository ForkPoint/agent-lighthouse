...
function build(body: string, head = '<title>T</title>'): Document {
  const dom = new JSDOM(`<!doctype html><html lang="en"><head>${head}</head><body>${body}</body></html>`);
...
describe('ariaHidden', () => {
  it('is true for aria-hidden="true"', () => {
    expect(ariaHidden(node('<div id="t" aria-hidden="true">x</div>'))).toBe(true);
  });

  it('is false for aria-hidden="false"', () => {
    expect(ariaHidden(node('<div id="t" aria-hidden="false">x</div>'))).toBe(false);
  });
...
describe('isHiddenForEveryone', () => {
  ...
  it('is true for an element inside a display:none ancestor', () => {
    expect(isHiddenForEveryone(node('<div style="display:none"><span id="t">x</span></div>'))).toBe(
      true,
    );
  });
});
...
describe('focus', () => {
  ...
  it('an anchor with href is focusable, one without is not', () => {
    expect(isNativelyFocusable(node('<a id="t" href="/x">Link</a>'))).toBe(true);
    expect(isNativelyFocusable(node('<a id="t">Link</a>'))).toBe(false);
  });
});

describe('isVisualContent', () => {
  it.each(['img', 'canvas', 'video', 'audio', 'svg'])('is true for %s', (tag) => {
...
describe('hasContent', () => {
  ...
  it('is true for an element holding only an image', () => {
    expect(hasContent(node('<div id="t"><img src="/x.png" alt="Hat"></div>').actualNode)).toBe(true);
  });
});

describe('idrefs', () => {
  it('resolves every id in the attribute, in order', () => {
    const doc = build('<span id="a">A</span><span id="b">B</span><div id="t" aria-labelledby="a b"></div>');
...
describe('isHTML5', () => {
  ...
  it('is false for a document with no doctype', () => {
    const dom = new JSDOM('<html lang="en"><head><title>T</title></head><body><p>x</p></body></html>');
