...
export function visibleVirtual(
...
function nativeSelectValue(node: VNode): string {
  ...
  return (
    selectedOptions.map((option) => visibleVirtual(option)).join(" ") || ""
  );
}
...
function ariaTextboxValue(node: VNode): string {
  ...
  if (!domNode || !isHiddenForEveryone(domNode))
...
function formControlValue(
  ...
  return formControlValueMethods.reduce(
    (accName: string, step) => accName || step(virtualNode, context),
    "",
  );
}
...
function titleText(node: VNode): string {
  ...
  if (
    !alwaysTitleElements.includes(vNode.props.nodeName) &&
    ["none", "presentation"].includes(getRole(vNode) as string)
  ) {
    return "";
  }
...
function descendantText(
  ...
  if (
    !candidate ||
    (candidate as unknown as { nodeName: string }).nodeName.toLowerCase() !==
      nodeName
  ) {
    return "";
  }
...
const nativeTextMethods: Record<
  string,
  (vNode: VNode, context: AccContext) => string
> = {
  valueText: (vNode) => vNode.props.value || "",
  buttonDefaultText: (vNode) =>
    defaultButtonValues[vNode.props.type as string] || "",
  tableCaptionText: (vNode, context) =>
    descendantText("caption", vNode, context),
  ...
  fieldsetLegendText: (vNode, context) =>
    descendantText("legend", vNode, context),
...
function nativeTextAlternative(
  ...
  if (
    ...
    ["presentation", "none"].includes(getRole(virtualNode) as string)
  ) {
    return "";
  }
  const elmSpec = getElementSpec(virtualNode, {
    noMatchAccessibleName: true,
  }) as { namingMethods?: string[] };
  const methods = (elmSpec.namingMethods || []).map(
    (name) => nativeTextMethods[name],
  );
  return methods.reduce(
    (accName: string, step) =>
      accName || (step ? step(virtualNode, context) : ""),
    "",
  );
}
...
function findElmsInContext(
  ...
  (context as unknown as { nodeType: number }).nodeType === 11
    ? context
    : (context as unknown as { ownerDocument: AnyNode })
        .ownerDocument) as unknown as {
    querySelectorAll: (s: string) => ArrayLike<AnyNode>;
  };
  // value is an id; escape minimally via attribute selector quoting
  return Array.from(
    root.querySelectorAll(`${elm}[${attr}="${cssAttrEscape(value)}"]`),
  );
}
...
function nodeSorterDom(a: AnyNode, b: AnyNode): number {
  if (a === b) return 0;
  return (
    a as unknown as { compareDocumentPosition: (n: AnyNode) => number }
  ).compareDocumentPosition(b) & 4
    ? -1
    : 1;
}
...
function phrasingElements(): string[] {
  if (!phrasingElementsCache) {
    phrasingElementsCache = getElementsByContentType("phrasing").concat([
      "#text",
    ]);
  }
...
export function subtreeText(
  ...
  return getOwnedVirtual(virtualNode).reduce(
    (contentText: string, child) =>
      appendAccessibleText(contentText, child, context),
    "",
  );
}
...
export function accessibleTextVirtual(
  virtualNode: VNode,
  context: AccContext = {},
): string {
