...
function doc(body: string, head = '<title>Test page</title>'): Document {
  return new JSDOM(`<!doctype html><html lang="en"><head>${head}</head><body>${body}</body></html>`)
    .window.document;
}
...
const CASES: Case[] = [
  ...
  },
  {
    ...
    fail: '<p>Body</p>',
    head: '<title>T</title>',
    failHead: '<title>T</title><meta http-equiv="refresh" content="5; url=/next">',
  },
  {
...
describe('accessibility engine — rule conformance', () => {
  ...
  for (const c of CASES) {
    ...
    it(`${c.rule}: rejects violating markup`, () => {
      expect(status(c.fail, c.rule, c.failHead ?? c.head)).toBe(c.reject ?? 'fail');
    });
  }
  ...
  it('aria-hidden-body fails a document whose <body> is aria-hidden', () => {
    ...
    expect(runRules(d, ['aria-hidden-body'])['aria-hidden-body']?.status).toBe('fail');
  });
  ...
  it('document-title fails a document with an empty <title>', () => {
    ...
    expect(runRules(d, ['document-title'])['document-title']?.status).toBe('fail');
  });

  it('reports the offending node so a report can point at it', () => {
    const result = runRules(doc('<button></button>'), ['button-name'])['button-name'];
  ...
  it('runs every rule over one document without throwing', () => {
    ...
    for (const [id, r] of Object.entries(results)) {
      expect(['pass', 'fail', 'incomplete', 'inapplicable'], id).toContain(r.status);
    }
  });
});
