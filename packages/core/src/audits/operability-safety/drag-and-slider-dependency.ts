...
function hasDiscreteValue($: PageContext["$"], node: unknown): boolean {
  ...
  return (
    group.find(
      'input[type="number"], input[type="text"], input[type="tel"], input[inputmode]',
    ).length > 0
  );
}
...
function survey(ctx: CheckContext): Finding[] {
  ...
  for (const page of ctx.pages) {
    ...
    $('input[type="range"], [role="slider"]').each((_i, node) => {
      ...
      if (isAria) {
        const missingValues = SLIDER_VALUES.filter(
          (name) => attribs[name] === undefined,
        );
    ...
    if (critical) {
      $('[draggable="true"], [class]').each((_i, node) => {
        ...
        const list = $(node as never).closest(
...
export class DragAndSliderDependencyAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    const constructs = ctx.pages.some(
      (page) =>
        page.$('input[type="range"], [role="slider"], [draggable="true"]')
          .length > 0 ||
        page.$("[class]").filter((_i, node) => {
          const cls =
            (node as { attribs?: Record<string, string> }).attribs?.["class"] ??
            "";
          return (
            DRAG_CLASS.test(cls) ||
            DROP_ZONE_CLASS.test(cls) ||
            CAROUSEL_CLASS.test(cls)
          );
        }).length > 0,
    );
