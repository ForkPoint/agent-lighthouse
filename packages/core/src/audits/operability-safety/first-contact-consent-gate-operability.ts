...
const DIALOG_ROOT =
  /onetrust|cookie-?banner|cookie-?consent|cookie-?notice|cc-banner|consent|didomi|usercentrics|gdpr/i;

/** Labels a one-click refusal carries. */
const REJECT_LABEL =
  /reject|decline|refuse|deny|only\s+(?:necessary|essential)|(?:necessary|essential)\s+only|continue\s+without/i;

/** Labels a multi-step refusal journey starts with. */
const PREFERENCES_LABEL =
  /manage|preferences|settings|customi[sz]e|options|choices|more\s+info/i;
...
function surveyPage(page: PageContext): Gate | null {
  ...
  $("iframe[src]").each((_i, node) => {
    ...
    try {
      if (new URL($n.attr("src") ?? "", page.url).origin !== origin)
        gate.crossOriginDialog = true;
    } catch {
  ...
  $(
    ...
    const inDialog =
      ...
      $(node as never)
        ...
        .filter((_j, ancestor) => {
          const a =
            (ancestor as { attribs?: Record<string, string> }).attribs ?? {};
...
export class FirstContactConsentGateOperabilityAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    if (gate.unnamedControls > 0) {
      defects.push(
        `${gate.unnamedControls} control(s) in the layer carry no accessible name`,
      );
    }
    if (gate.actionCost > 1) {
      defects.push(
        "refusing takes more than one click, because the only direct control accepts",
      );
    }
