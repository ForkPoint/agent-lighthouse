...
vi.mock('../../fetcher', async (importOriginal) => {
  ...
  return { ...actual, isSafeUrl: async (url: string) => url.startsWith('https://api.gleif.org') };
});
...
interface Site {
  ...
  gleif?: { entityStatus?: string; registrationStatus?: string; legalName?: string } | null;
  gleifStatus?: number;
}

function run(site: Site) {
  const audit = new OrganizationIdentifierRegistryResolutionAudit();
  const jsonLd = JSON.stringify({ '@context': 'https://schema.org', '@type': 'Organization', ...site.organization });
          ...
          : [
              {
                attributes: {
                  entity: {
                    legalName: { name: record.legalName ?? 'Example Incorporated' },
                    status: record.entityStatus ?? 'ACTIVE',
                  },
                  registration: { status: record.registrationStatus ?? 'ISSUED' },
                },
              },
            ],
    });
...
describe('OrganizationIdentifierRegistryResolutionAudit', () => {
  ...
  describe('nameSimilarity', () => {
    it('matches across legal suffixes and punctuation', () => {
      expect(nameSimilarity('Example Incorporated', 'Example, Inc.')).toBeGreaterThan(0.6);
      expect(nameSimilarity('Example Incorporated', 'Totally Different Holdings')).toBeLessThan(0.6);
    });
  });

  it('passes an LEI that resolves to an active, issued record with a matching name', async () => {
    const { result, requests } = run({
      organization: { name: 'Example Inc', legalName: 'Example Incorporated', iso6523Code: `0199:${VALID_LEI}` },
    });
  ...
  it('fails an LEI whose check digits are wrong, before spending a request', async () => {
    const { result, requests } = run({ organization: { name: 'Example Inc', leiCode: '5493001KJTIIGC8Y1R13' } });
    const r = await result;
  ...
  it('fails an identifier GLEIF holds no record for', async () => {
    const { result } = run({ organization: { name: 'Example Inc', leiCode: VALID_LEI }, gleif: null });
    const r = await result;
    expect(r.status).toBe('fail');
  ...
  it('fails when the registered legal name names a different organization', async () => {
    const { result } = run({
      organization: { name: 'Example Inc', legalName: 'Example Incorporated', leiCode: VALID_LEI },
  ...
  it('fails an iso6523Code with no issuing-agency prefix', async () => {
    const { result } = run({ organization: { name: 'Example Inc', iso6523Code: VALID_LEI } });
    const r = await result;
    expect(r.status).toBe('fail');
