...
export function certificatesIn(store: Uint8Array): X509Certificate[] {
  ...
  for (
    let at = 0;
    at + 4 < store.length && attempts < MAX_PARSE_ATTEMPTS;
    at += 1
  ) {
...
export function leafOf(certs: X509Certificate[]): X509Certificate | undefined {
  for (const cert of certs) {
    const issuesAnother = certs.some(
      (other) => other !== cert && other.checkIssued(cert),
    );
...
export class C2paSignerTrustStatusAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const url of candidates.slice(0, MAX_IMAGES)) {
      ...
      const store = bytes.subarray(
        location.start,
        location.start + location.length,
      );
      const hasTimestamp = Buffer.from(store)
        .toString("latin1")
        .includes(TIMESTAMP_LABEL);
      ...
      if (!leaf) {
        unreadable.push(
          `${url}: a manifest is present but no signing certificate could be read from it`,
        );
        continue;
      }
      ...
      signers.push(
        `${url}: ${subject} (issued by ${leaf.issuer.replace(/\n/g, ", ")})`,
      );

      if (selfSigned) {
        failures.push(
          `${url}: signed by a self-signed certificate (${subject}); no validator trusts one`,
        );
      } else if (Number.isFinite(notAfter) && notAfter < now) {
        failures.push(
          `${url}: the signing certificate expired on ${leaf.validTo}`,
        );
      } else if (Number.isFinite(notBefore) && notBefore > now) {
        failures.push(
          `${url}: the signing certificate is not valid until ${leaf.validFrom}`,
        );
