...
function page(body: string, url = 'https://example.com/'): CheckContext {
  return mockCheckContext([mockPageContext(url, `<html><head></head><body>${body}</body></html>`)]);
}
...
describe('DragAndSliderDependencyAudit', () => {
  ...
  it('flags a lone range input and names the missing numeric input', async () => {
    const result = await audit.audit(
      page('<fieldset><label for="p">Max price</label><input id="p" type="range"></fieldset>'),
    );
  ...
  it('reports a role=slider missing aria-valuenow on its own arm', async () => {
    const result = await audit.audit(
      page('<fieldset><div role="slider" aria-label="Max price"></div><input type="number"></fieldset>'),
    );
  ...
  it('flags a drag-to-reorder list on a checkout path', async () => {
    const result = await audit.audit(page(DRAG_LIST, 'https://example.com/checkout/seats'));
  ...
  it('does not flag the same list on a blog path', async () => {
    const result = await audit.audit(page(DRAG_LIST, 'https://example.com/blog/seat-tips'));
  ...
  it('flags a drop zone with no file input anywhere near it', async () => {
    const result = await audit.audit(page('<div class="file-drop">Drop your CV here</div>'));
  ...
  it('flags a carousel whose only next/prev affordance is a swipe handler', async () => {
    const result = await audit.audit(
      page('<div class="carousel" ontouchstart="swipe()"><div class="slide">One</div></div>'),
    );
