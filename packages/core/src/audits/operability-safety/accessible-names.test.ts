...
describe('AccessibleNamesAudit', () => {
  it('registers under the accessible-names id with its dossier and grade', () => {
    expect(AccessibleNamesAudit.meta.id).toBe('operability-safety/accessible-names');
    expect(AccessibleNamesAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/accessible-names.md');
  ...
  it('wires exactly its a11y rule(s)', () => {
    ...
    expect(result.expected).toBe('accessibility rules pass: button-name, link-name');
  });

  it('fails when the `button-name` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'button-name': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
        'link-name': { status: 'pass', nodes: [] },
      }),
    ]);
    const result = runA11yAudit(AccessibleNamesAudit, ctx);
  ...
  it('fails when the `link-name` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'button-name': { status: 'pass', nodes: [] },
        'link-name': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(AccessibleNamesAudit, ctx);
    expect(result.status).toBe('fail');
