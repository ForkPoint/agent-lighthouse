...
import { C2paSignerTrustStatusAudit, certificatesIn, leafOf } from './c2pa-signer-trust-status';
...
function run(images: Record<string, Uint8Array>, html = '<html><body><img src="/hero.jpg"></body></html>') {
...
describe('C2paSignerTrustStatusAudit', () => {
  ...
  it('is notApplicable when no image carries a manifest', async () => {
    const plain = new Uint8Array([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x04, 0x00, 0x00, 0xff, 0xd9]);
  ...
  describe('certificatesIn', () => {
    ...
    it('finds nothing in bytes that hold no certificate', () => {
      expect(certificatesIn(new Uint8Array(Buffer.from('no certificate here', 'latin1')))).toHaveLength(0);
    });
  ...
  it('fails a self-signed signing certificate', async () => {
    const { result } = run({ 'https://example.com/hero.jpg': signedJpeg(store(SELF_SIGNED)) });
  ...
  it('fails a signing certificate outside its validity window', async () => {
    const { result } = run({ 'https://example.com/hero.jpg': signedJpeg(store(EXPIRED)) });
    const r = await result;
    expect(r.status).toBe('fail');
  ...
  it('passes a CA-issued certificate inside its window with a timestamp', async () => {
    const { result } = run({ 'https://example.com/hero.jpg': signedJpeg(store(CA_ISSUED)) });
    const r = await result;
  ...
  it('warns on a CA-issued certificate with no timestamp token', async () => {
    const { result } = run({ 'https://example.com/hero.jpg': signedJpeg(store(CA_ISSUED, false)) });
    const r = await result;
  ...
  it('reports a manifest whose certificate cannot be read, and does not throw', async () => {
    const { result } = run({ 'https://example.com/hero.jpg': signedJpeg('not a certificate at all') });
    const r = await result;
    expect(r.status).toBe('warn');
    expect(strings(r, 'unreadable').join(' ')).toContain('no signing certificate');
  });
  ...
  it('never claims the certificate is on the C2PA Trust List', async () => {
    ...
    expect(C2paSignerTrustStatusAudit.meta.description).not.toMatch(/Trust List membership is checked/i);
  });
