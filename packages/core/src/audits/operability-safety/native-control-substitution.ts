...
const SUBSTITUTE_CLASS =
  /select|dropdown|picker|calendar|datepicker|chooser|combobox/i;
...
const CONVERSION_PATH =
  /checkout|cart|signup|sign-up|register|book|order|search/i;
...
function contractDefect(
  ...
  if (
    $popup.find('[role="option"], [role="treeitem"], [role="row"]').length === 0
  ) {
...
function survey(ctx: CheckContext): Survey {
  ...
  for (const page of ctx.pages) {
    ...
    const pathWeight = (formAction: string) =>
      CONVERSION_PATH.test(page.url) || CONVERSION_PATH.test(formAction)
        ? 2
        : 1;
    ...
    $("[aria-controls], [aria-owns]").each((_, el) => {
      for (const attr of ["aria-controls", "aria-owns"]) {
        ...
        if (value)
          for (const id of value.split(/\s+/)) if (id) popupIds.add(id);
      }
    });
