...
describe('DocumentTitleAudit', () => {
  it('registers under the document-title id with its dossier and grade', () => {
    expect(DocumentTitleAudit.meta.id).toBe('operability-safety/document-title');
    expect(DocumentTitleAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/document-title.md');
  ...
  it('fails when the `document-title` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'document-title': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(DocumentTitleAudit, ctx);
