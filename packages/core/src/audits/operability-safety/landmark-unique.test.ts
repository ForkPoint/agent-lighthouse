...
describe('LandmarkUniqueAudit', () => {
  it('registers under the landmark-unique id with its dossier and grade', () => {
    expect(LandmarkUniqueAudit.meta.id).toBe('operability-safety/landmark-unique');
    expect(LandmarkUniqueAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/landmark-unique.md');
  ...
  it('fails when the `landmark-unique` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'landmark-unique': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(LandmarkUniqueAudit, ctx);
