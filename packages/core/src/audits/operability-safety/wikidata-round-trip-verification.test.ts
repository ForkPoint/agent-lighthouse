...
vi.mock('../../fetcher', async (importOriginal) => {
  ...
  return { ...actual, isSafeUrl: async (url: string) => url.startsWith('https://www.wikidata.org') };
});
...
function claims(website: string | undefined, rank = 'normal'): string {
  ...
  return JSON.stringify({
    claims: {
      P856: [{ rank, mainsnak: { datavalue: { value: website, type: 'string' } } }],
    },
  });
}
...
function run(site: Site) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (typeof answer === 'number') return mockFetchResult('', answer, 'application/json');
    return mockFetchResult(answer ?? claims('https://example.com/'), 200, 'application/json');
  };
...
describe('WikidataRoundTripVerificationAudit', () => {
  ...
  it('is notApplicable when no page claims a Wikidata entity', async () => {
    const { result, requests } = run({ sameAs: ['https://twitter.com/example'] });
  ...
  describe('wikidataId', () => {
    it('reads a Q-id from both URL forms and rejects anything else', () => {
      ...
      expect(wikidataId('https://en.wikipedia.org/wiki/Google')).toBeUndefined();
    });
  });

  describe('claimedEntities', () => {
    it('collects Q-ids from identity nodes only', () => {
      const $ = parseHtml(`<script type="application/ld+json">${JSON.stringify({
        '@context': 'https://schema.org',
        '@graph': [
          { '@type': 'Organization', sameAs: ['https://www.wikidata.org/wiki/Q95'] },
          { '@type': 'WebPage', sameAs: ['https://www.wikidata.org/wiki/Q1'] },
        ],
      })}</script>`);
  ...
  describe('officialWebsite', () => {
    it('prefers a preferred-rank statement and ignores a deprecated one', () => {
      const body = JSON.stringify({
        claims: {
          P856: [
            { rank: 'deprecated', mainsnak: { datavalue: { value: 'https://old.test/' } } },
            { rank: 'normal', mainsnak: { datavalue: { value: 'https://normal.test/' } } },
            { rank: 'preferred', mainsnak: { datavalue: { value: 'https://preferred.test/' } } },
          ],
        },
      });
  ...
  it('passes when the entity points back at this domain', async () => {
    const { result, requests } = run({ sameAs: ['https://www.wikidata.org/wiki/Q95'] });
  ...
  it('warns when Wikidata itself does not answer', async () => {
    const { result } = run({ sameAs: ['https://www.wikidata.org/wiki/Q95'], wikidata: { Q95: 503 } });
    const r = await result;
    expect(r.status).toBe('warn');
  ...
  it('resolves at most two entities per scan', async () => {
    const { result, requests } = run({
      sameAs: ['Q1', 'Q2', 'Q3'].map((id) => `https://www.wikidata.org/wiki/${id}`),
    });
    await result;
