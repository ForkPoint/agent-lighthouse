...
describe('AriaHiddenBodyAudit', () => {
  it('registers under the aria-hidden-body id with its dossier and grade', () => {
    expect(AriaHiddenBodyAudit.meta.id).toBe('operability-safety/aria-hidden-body');
    expect(AriaHiddenBodyAudit.meta.dossier).toBe('docs/evidence/audits/operability-safety/aria-hidden-body.md');
  ...
  it('fails when the `aria-hidden-body` rule reports a violation', () => {
    const ctx = mockCheckContext([
      pageWithA11y('https://example.com/', {
        'aria-hidden-body': { status: 'fail', nodes: [{ target: '#offender', summary: 'violation' }] },
      }),
    ]);
    const result = runA11yAudit(AriaHiddenBodyAudit, ctx);
