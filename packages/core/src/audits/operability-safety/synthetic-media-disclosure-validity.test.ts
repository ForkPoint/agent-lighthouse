...
function jpegWith(xmp?: string, manifestPayload?: string): Uint8Array {
  ...
  const segment = (marker: number, body: string) => {
    ...
    parts.push(0xff, marker, (length >> 8) & 0xff, length & 0xff, ...Buffer.from(body, 'latin1'));
  };
  if (xmp !== undefined) segment(0xe1, `http://ns.adobe.com/xap/1.0/\x00${xmp}`);
  if (manifestPayload !== undefined) segment(0xeb, `JP\x00\x01jumbc2pa${manifestPayload}`);
  parts.push(0xff, 0xd9);
  return new Uint8Array(parts);
}

function run(images: Record<string, Uint8Array>, html = '<html><body><img src="/a.jpg"></body></html>') {
  const audit = new SyntheticMediaDisclosureValidityAudit();
  const ctx = mockCheckContext([mockPageContext('https://example.com/post', html)]);
...
describe('SyntheticMediaDisclosureValidityAudit', () => {
  ...
  describe('digitalSourceType', () => {
    it('reads the attribute, element and rdf:resource forms', () => {
      ...
      expect(
        digitalSourceType(`<Iptc4xmpExt:DigitalSourceType>${VALID_URI}</Iptc4xmpExt:DigitalSourceType>`),
      ).toBe(VALID_URI);
      expect(digitalSourceType(`<Iptc4xmpExt:DigitalSourceType rdf:resource="${VALID_URI}"/>`)).toBe(VALID_URI);
  ...
  describe('classifyDisclosure', () => {
    it('grades each near-miss class of its own', () => {
      ...
      expect(classifyDisclosure('trainedAlgorithmicMedia').kind).toBe('bare-concept');
      expect(classifyDisclosure(VALID_URI.replace('http://', 'https://')).kind).toBe('wrong-scheme');
      expect(classifyDisclosure(`${VALID_URI}/`).kind).toBe('trailing-slash');
      expect(classifyDisclosure(`${DIGITAL_SOURCE_BASE}madeUpConcept`).kind).toBe('unknown-concept');
      expect(classifyDisclosure('AI-generated').kind).toBe('free-text');
    });
  });

  it('passes an exact vocabulary member', async () => {
    const result = await run({ 'https://example.com/a.jpg': jpegWith(packet(VALID_URI)) });
    expect(result.status).toBe('pass');
    expect(result.details?.['declaredCoverage']).toBe(100);
  });

  it('fails a bare concept id', async () => {
    const result = await run({ 'https://example.com/a.jpg': jpegWith(packet('trainedAlgorithmicMedia')) });
  ...
  it('fails an https spelling of the http vocabulary URI', async () => {
    const result = await run({
      'https://example.com/a.jpg': jpegWith(packet(VALID_URI.replace('http://', 'https://'))),
    });
    expect(result.status).toBe('fail');
    expect(strings(result, 'failures').join(' ')).toContain('https where the vocabulary uses http');
  });
  ...
  it('fails free text', async () => {
    const result = await run({ 'https://example.com/a.jpg': jpegWith(packet('AI-generated')) });
    expect(result.status).toBe('fail');
  ...
  it('fails when the XMP and the C2PA manifest contradict each other', async () => {
    ...
    expect(result.status).toBe('fail');
    expect(strings(result, 'failures').join(' ')).toContain('contradict each other');
  });

  it('accepts an XMP declaration that agrees with the manifest', async () => {
    const result = await run({
      'https://example.com/a.jpg': jpegWith(packet(VALID_URI), 'actions trainedAlgorithmicMedia'),
    });
    expect(result.status).toBe('pass');
  });
  ...
  it('never claims to detect undisclosed synthetic imagery', () => {
    ...
    expect(`${meta.description} ${meta.guidance?.impact}`).not.toMatch(/detect(s|ing)? undisclosed|classif/i);
  });
