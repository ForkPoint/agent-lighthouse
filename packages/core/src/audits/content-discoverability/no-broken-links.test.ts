...
describe('NoBrokenLinksAudit', () => {
  ...
  const twoLinks = '<html><body><a href="/a">A</a><a href="/b">B</a></body></html>';

  it('passes when all sampled internal links return 200', async () => {
    const ctx = mockCheckContext([mockPageContext('https://example.com/', twoLinks)]);
    ctx.fetch = fetchStub({});
  ...
  it('fails when a majority of internal links are broken', async () => {
    const ctx = mockCheckContext([mockPageContext('https://example.com/', twoLinks)]);
    ctx.fetch = fetchStub({ 'https://example.com/a': 404, 'https://example.com/b': 500 });
    const result = await audit.audit(ctx);
