...
export class ThreeWayFreshnessLagAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const newestFeed = newest(
      feeds
        .flatMap((feed) => feed.entries.map((entry) => entry.updated))
        .filter((time): time is number => time !== undefined),
    );

    const surfaces = [newestOnPage, newestSitemap, newestFeed].filter(
      (time) => time !== undefined,
    );
    ...
    for (const feed of feeds) {
      ...
      if (
        ...
        feed.lastBuild !== undefined &&
        feed.lastBuild < newestItem
      ) {
        failures.push(
      ...
      if (items.length > 1 && !isNewestFirst(feed)) {
        warnings.push(
          `${feed.url}: items are not in newest-first order, and many consumers read only the head`,
        );
      }
    }
    ...
    const details = {
      ...
      sitemapLagDays:
        newestOnPage !== undefined && newestSitemap !== undefined
          ? lagDays(newestOnPage, newestSitemap)
          : 0,
      feedLagDays:
        newestOnPage !== undefined && newestFeed !== undefined
          ? lagDays(newestOnPage, newestFeed)
          : 0,
