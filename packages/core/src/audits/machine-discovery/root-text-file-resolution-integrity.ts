...
export class RootTextFileResolutionIntegrityAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const robots = await sharedProbeUrl(ctx, robotsUrl, {
      followRedirects: true,
    });
    if (!robots)
      return this.fail(
        "Robots file probe failed",
        "200 OK or 404",
        "No response",
      );
    ...
    if (answering.length > 0) {
      ...
      const looksHtml = answering.some(
        (probe) =>
          sniff(probe.result.body).trimStart().startsWith("<") ||
          sniff(probe.result.body).includes("<html"),
      );
      const htmlType = answering.some((probe) =>
        /^text\/html/i.test(probe.result.contentType),
      );
      const identical =
        answering.length === 2 &&
        first !== undefined &&
        second !== undefined &&
        first.result.body === second.result.body;
    ...
    if (robots.status !== 200) {
      warnings.push(
        `/robots.txt answered HTTP ${robots.status}, so the positive control did not run`,
      );
    } else if (!robotsPlain) {
      failures.push(
    ...
    const discoveryProbeReliable =
      absent.length === probes.length && robots.status === 200 && robotsPlain;
    const details = {
