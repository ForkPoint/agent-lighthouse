...
describe('SitemapExistsAudit', () => {
  ...
  it('passes when a valid <urlset> sitemap exists', () => {
    const body =
  ...
  it('fails when the sitemap lacks <urlset> or <sitemapindex>', () => {
    const ctx = mockCheckContext([], {
      '/sitemap.xml': mockFetchResult('<html><body>not a sitemap</body></html>', 200, 'text/html'),
    });
    const result = audit.audit(ctx);
  ...
  it('passes with a <sitemapindex> root element (covers <sitemapindex> found branch)', () => {
