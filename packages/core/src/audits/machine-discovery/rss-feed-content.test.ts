...
const feed = (descriptions: string[]) =>
  `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0"><channel>${descriptions
    .map((d) => `<item><title>Item</title><link>https://example.com/a</link><description>${d}</description></item>`)
    .join('')}</channel></rss>`;
...
describe('RssFeedContentAudit', () => {
  ...
  it('passes when all items have full content', async () => {
    const ctx = mockCheckContext([], {
      '/rss.xml': mockFetchResult(feed([longText, longText]), 200, 'application/rss+xml'),
    });
  ...
  it('warns when only some items have full content', async () => {
    const ctx = mockCheckContext([], {
      '/rss.xml': mockFetchResult(feed([longText, shortText]), 200, 'application/rss+xml'),
    });
    const result = await audit.audit(ctx);
  ...
  it('fails when fewer than half the items have full content', async () => {
    const ctx = mockCheckContext([], {
      '/rss.xml': mockFetchResult(feed([longText, shortText, shortText]), 200, 'application/rss+xml'),
    });
    const result = await audit.audit(ctx);
  ...
  it('warns when the feed has no items', async () => {
    const ctx = mockCheckContext([], {
      '/rss.xml': mockFetchResult('<rss version="2.0"><channel></channel></rss>', 200, 'application/rss+xml'),
    });
    const result = await audit.audit(ctx);
    expect(result.status).toBe('warn');
  ...
  it('passes when atom.xml found via direct fetch fallback (covers atom fetch true branch)', async () => {
    ...
    ctx.fetch = async ({ url }) => {
      ...
      const r = mockFetchResult(ok ? atomBody : '', ok ? 200 : 404, 'application/atom+xml');
      r.url = url;
      r.finalUrl = url;
      return r;
    };
  ...
  it('skips non-alternate headLinks and uses well-known paths (covers if-condition false branch)', async () => {
    ...
    const ctx = mockCheckContext([mockPageContext('https://example.com/', html)], {
      '/rss.xml': mockFetchResult(feed([longText]), 200, 'application/rss+xml'),
    });
    ctx.fetch = async ({ url }) => {
      const r = mockFetchResult('', 404);
      r.url = url;
