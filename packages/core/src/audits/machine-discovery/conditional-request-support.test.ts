...
const SITEMAP = '<?xml version="1.0"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://example.com/a</loc></url></urlset>';
...
function run(behaviour: SurfaceBehaviour = { etag: '"v1"', lastModified: 'Wed, 20 Aug 2026 10:00:00 GMT' }) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    const conditional = o.headers?.['If-None-Match'] !== undefined || o.headers?.['If-Modified-Since'] !== undefined;
    if (conditional && !behaviour.ignoreConditionals) return mockFetchResult('', 304, 'text/plain');

    const count = (seen.get(o.url) ?? 0) + 1;
    seen.set(o.url, count);
    const body = path === '/robots.txt' ? 'User-agent: *\nAllow: /\n' : SITEMAP;
    const result = mockFetchResult(body, path === '/sitemap.xml' || path === '/robots.txt' ? 200 : 404, 'text/plain');
    if (result.status !== 200) return result;
    const etag = count > 1 && behaviour.etagAfter ? behaviour.etagAfter : behaviour.etag;
    if (etag) result.headers['etag'] = etag;
    if (behaviour.lastModified) result.headers['last-modified'] = behaviour.lastModified;
    if (behaviour.cacheControl) result.headers['cache-control'] = behaviour.cacheControl;
    return result;
  };
...
describe('ConditionalRequestSupportAudit', () => {
  ...
  it('fails a 200 answer to If-None-Match and to If-Modified-Since', async () => {
    const { result } = run({ etag: '"v1"', lastModified: 'Wed, 20 Aug 2026 10:00:00 GMT', ignoreConditionals: true });
  ...
  it('fails a surface with no validator at all, and reports what a poll costs', async () => {
    ...
    const r = await result;
    expect(r.status).toBe('fail');
    expect(strings(r, 'failures')[0]).toContain('neither ETag nor Last-Modified');
  ...
  it('adds at most four requests per surface, at most two of them conditional', async () => {
    ...
    for (const request of requests) {
      ...
      if (request.headers?.['If-None-Match'] !== undefined || request.headers?.['If-Modified-Since'] !== undefined) {
        conditionalPerUrl.set(request.url, (conditionalPerUrl.get(request.url) ?? 0) + 1);
      }
    }
