...
export class FeedEntryIdentityAndCanonicalIntegrityAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const feed of feeds) {
      ...
      if (expected && !expected.test(type)) {
        if (GENERIC_XML.test(type)) {
          ...
          failures.push(
            `${where}: served as "${type}", which is not a ${feed.declaredType} media type`,
          );
        }
      }
      if (feed.bomOrLeadingSpace) {
        failures.push(
          `${where}: a byte-order mark or whitespace precedes the first element, which strict parsers reject`,
        );
      }
      ...
      for (const [index, entry] of entries.entries()) {
        ...
        if (feed.declaredType === "atom") {
          ...
          if (entry.updatedCount !== 1) {
            failures.push(
              `${label}: ${entry.updatedCount} atom:updated elements; RFC 4287 requires exactly one`,
            );
          }
        ...
        if (entry.link === "") {
          ...
          failures.push(
            `${label}: item link ${entry.link} is not absolute HTTPS`,
          );
        }
      }
      ...
      for (const [id, count] of ids) {
        if (count > 1)
          failures.push(`${where}: id "${id}" appears on ${count} entries`);
      }
    }
    ...
    const newest = feeds
      .flatMap((feed: FeedDocument) =>
        feed.entries.map((entry) => ({ feed: feed.url, entry })),
      )
    ...
    for (const item of newest) {
      ...
      if (!result || result.status < 200 || result.status >= 300) {
        failures.push(
          `${link}: the feed links to a URL that answered HTTP ${result?.status ?? 0}`,
        );
        continue;
      }
      if (result.finalUrl !== "" && result.finalUrl !== link) {
        failures.push(
          `${link}: redirects to ${result.finalUrl}, so the feed advertises a URL the site does not serve`,
        );
      }
      const params = new URL(link).searchParams;
      const tracking = [...params.keys()].filter((key) =>
        TRACKING_PARAM.test(key),
      );
      const canonical =
        parseHtml(result.body)('link[rel~="canonical"]').first().attr("href") ??
        "";
      const resolved = (() => {
        try {
          return canonical === ""
            ? ""
            : new URL(canonical, result.finalUrl || link).toString();
        } catch {
          return "";
        }
      })();
      ...
      if (resolved !== "" && resolved !== link) {
        const because =
          tracking.length > 0
            ? ` (the link carries ${tracking.join(", ")})`
            : "";
        failures.push(
          `${link}: the page names ${resolved} as canonical${because}`,
        );
      ...
      canonicalChecks.push(
        `${link} -> canonical ${resolved || "not declared"}`,
      );
    }
