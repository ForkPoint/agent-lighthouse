...
const EXPECTATIONS: Expectation[] = [
  {
    path: "/llms.txt",
    expectedTypes: ["text/plain", "text/markdown"],
    label: "llms.txt",
  },
  {
...
function hasNosniff(file: FetchResult): boolean {
  return (
    (file.headers["x-content-type-options"] ?? "").trim().toLowerCase() ===
    "nosniff"
  );
}
...
export class AiFileDeliveryAudit extends Audit {
  ...
  audit(ctx: CheckContext): AuditResult {
    ...
    for (const { path, expectedTypes, label } of EXPECTATIONS) {
      ...
      if (isHtmlShell(file) && !expectedTypes.some((t) => t.includes("html")))
        continue;
    ...
    if (incorrect.length > 0) {
      ...
      return this.fail(
        ...
        `Incorrect: ${details}; ${cachingNote}`,
        {
          ...
          code: incorrect
            .map((i) => `${i.label}: Content-Type: ${i.expected}`)
            .join("\n"),
        },
        page?.url,
      );
