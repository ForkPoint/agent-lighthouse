  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const surface of surfaces) {
      ...
      if (result.honoursIfNoneMatch === false) {
        failures.push(
          `${where}: answered If-None-Match with 200 rather than 304`,
        );
      }
      if (result.honoursIfModifiedSince === false) {
        failures.push(
          `${where}: answered If-Modified-Since with 200 rather than 304`,
        );
      }
      ...
      if (surface.kind === "sitemap") {
        ...
        if (!tree.truncated && tree.entries.length > MAX_SITEMAP_URLS) {
          warnings.push(
            `${where}: ${tree.entries.length} URLs, over the sitemap protocol's 50,000 limit`,
          );
        }
      }
    }
    ...
    const revalidatable = surfaces.filter(
      (s) =>
        s.result.honoursIfNoneMatch === true ||
        s.result.honoursIfModifiedSince === true,
    );
