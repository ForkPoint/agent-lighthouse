  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (url === "https://example.com/feed.xml") {
      const result = mockFetchResult(
        options.feed,
        200,
        options.feedType ?? "application/rss+xml",
      );
      result.url = url;
      result.finalUrl = url;
      return result;
...
describe("FeedEntryIdentityAndCanonicalIntegrityAudit", () => {
  ...
  it("fails an item link whose tracking parameters the canonical does not carry", async () => {
    ...
    const { result } = run({
      feed,
      items: {
        "https://example.com/a?utm_source=feed": itemPage(
          "https://example.com/a",
        ),
      },
    });
  ...
  it("compares at most five item URLs against their canonical", async () => {
    ...
    const items = Object.fromEntries(
      Array.from({ length: 8 }, (_v, i) => [
        ...
        itemPage(`https://example.com/p${i}`),
      ]),
    );
    ...
    expect(
      requests.filter((o) => o.url.startsWith("https://example.com/p")),
    ).toHaveLength(5);
  });
