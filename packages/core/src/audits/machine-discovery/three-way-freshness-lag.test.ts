...
function run(site: Site) {
  ...
  const sitemap = `<?xml version="1.0"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${(
    ...
    .map(
      (lastmod, i) =>
        `<url><loc>${locs[i]}</loc><lastmod>${lastmod}</lastmod></url>`,
    )
    .join("")}</urlset>`;
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    const robotsMeta = site.noindex?.includes(o.url)
      ? '<meta name="robots" content="noindex">'
      : "";
    return mockFetchResult(
      `<html><head>${robotsMeta}</head><body><p>Post.</p></body></html>`,
      200,
      "text/html",
    );
  };
...
const iso = (days: number) =>
  new Date(
    Date.parse("2026-08-20T10:00:00Z") - days * 86_400_000,
  ).toISOString();
/** The same instant in RFC 822, which is what RSS pubDate uses. */
const rfc = (days: number) =>
  new Date(
    Date.parse("2026-08-20T10:00:00Z") - days * 86_400_000,
  ).toUTCString();
...
describe("ThreeWayFreshnessLagAudit", () => {
  ...
  it("samples at most five sitemap URLs for the dead-URL check", async () => {
    ...
    await result;
    expect(
      requests.filter((o) => new URL(o.url).pathname.startsWith("/post-")),
    ).toHaveLength(5);
  });
