...
export class AiCrawlerSurfaceReachabilityAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    const wildcard: RobotsGroup[] = groups.filter(
      (group) => group.userAgent.trim() === "*",
    );
    const optOut: string[] = [];
    ...
    for (const bot of UA_PANEL) {
      ...
      const allowed = samplePaths.filter((path) =>
        isPathAllowed(groups, bot, path),
      ).length;
      ...
      const baseline =
        samplePaths.filter((path) =>
          isPathAllowed(wildcard, WILDCARD_BASELINE, path),
        ).length / samplePaths.length;
