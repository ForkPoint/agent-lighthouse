...
function productNode(
  ...
  for (const node of flattenJsonLd(jsonLd)) {
    if (
      isObject(node) &&
      typesOf(node).some((type) => PRODUCT_TYPES.includes(type))
    )
      return node;
  }
...
export class AgentCommerceFeedParityAudit extends Audit {
  ...
  async audit(ctx: CheckContext): Promise<AuditResult> {
    ...
    for (const candidate of candidates) {
      ...
      for (const defect of assessment.defects)
        defects.push(`${candidate.url}: ${defect}`);
      for (const risk of assessment.risks)
        risks.push(`${candidate.url}: ${risk}`);
  ...
  private assess({ url, $, product }: Candidate): Assessment {
    ...
    const gtinKey = ["gtin", "gtin8", "gtin12", "gtin13", "gtin14"].find(
    ...
    if (gtinKey) {
      ...
      if (!/^\d{8}$|^\d{12,14}$/.test(gtin)) {
        defects.push(
          `${gtinKey} "${gtin}" is not 8, 12, 13 or 14 digits with no separators`,
        );
    ...
    mark(
      ...
      brand
        ? `brand is over the ${CAPS.brand}-character cap`
        : "no brand name, which the OpenAI feed requires",
    );
    ...
    if (!offer) {
      defects.push(
        "no Offer on the Product, so there is no price, availability or seller to send",
      );
      mark("price", false);
      ...
      if (offerUrl && canonicalHref) {
        ...
        try {
          same =
            new URL(canonicalHref, url).toString() ===
            new URL(offerUrl, url).toString();
        } catch {
          same = false;
        }
        mark(
    ...
    const condition =
      ...
      text(first(product["offers"])?.["itemCondition"]);
    mark(
      "condition",
      Boolean(condition) &&
        CONDITIONS.has(condition!.replace(/^http:/, "https:")),
      condition
