...
function run(site: Site) {
  ...
  ctx.fetch = async (o: FetchOptions): Promise<FetchResult> => {
    ...
    if (o.url === FEED_URL) {
      ...
      if (site.linkHeader !== undefined)
        result.headers["link"] = site.linkHeader;
      return result;
    }
    if (new URL(o.url).pathname === "/feed")
...
describe("WebsubHubAdvertisementAudit", () => {
  ...
  it("probes the hub with a HEAD and never subscribes", async () => {
    ...
    await result;
    const hub = requests.filter((o) =>
      o.url.startsWith("https://hub.example.net"),
    );
