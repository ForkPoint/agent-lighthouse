...
describe('Corpus Analysis & False Positive Inspection', () => {
  it('runs all 215 audits across 41 real-world fixtures and generates a comprehensive report', { timeout: 120000 }, async () => {
    ...
    const totalStats = {
      ...
      outcomes: { ran: 0, skipped: 0, gated: 0, error: 0 } as Record<string, number>,
    ...
    for (const name of fixtureNames) {
      ...
      const wafProtection = detectWafProtection(provenance.url, pageCtx.fetchResult, {}, 1);
      ...
      const traces: any[] = [];
      const result = await runAudits(ctx, defaultConfig, undefined, undefined, (t) => traces.push(t));
      ...
      const fixtureSummary = {
        ...
        outcomeCounts: { ran: 0, skipped: 0, gated: 0, error: 0 } as Record<string, number>,
        statusCounts: { pass: 0, warn: 0, fail: 0, na: 0 } as Record<string, number>,
        fails: [] as any[],
        errors: [] as any[],
      };
      ...
      for (const t of traces) {
        ...
        if (t.outcome === 'error') {
          const errItem = { fixture: name, id: t.id, explanation: t.explanation };
        ...
        if (t.status === 'fail') {
          ...
          if (t.explanation?.toLowerCase().includes('no ') && t.explanation?.toLowerCase().includes('found') && t.status === 'fail' && t.weight > 0) {
    ...
    const artifactPath = '/Users/kirov/.gemini/antigravity-ide/brain/db4a0b18-38da-44e5-ae69-f21e9d98519d/corpus_audit_analysis_report.md';
...
${
  totalStats.errors.length === 0
    ...
    : totalStats.errors.map((e) => `- **${e.fixture}** \`${e.id}\`: ${e.explanation}`).join('\n')
}
