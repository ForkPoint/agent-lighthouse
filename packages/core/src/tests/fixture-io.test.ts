...
describe('the real-page fixtures', () => {
  ...
  it('records what kind of thing was captured, and the response that decided it', () => {
    for (const name of listFixtures()) {
      ...
      expect(Object.keys(provenance.headers).length, `${name} stored no headers`).toBeGreaterThan(
        0,
      );
  ...
  it('replays to the kind it was recorded with', () => {
    for (const name of listFixtures()) {
      ...
      expect(kind, `${name} was recorded as ${provenance.kind} but replays as ${kind}`).toBe(
        provenance.kind,
      );
    }
  });
...
describe('classifyCapture', () => {
  ...
  it('calls an Akamai-fronted 2xx page a page, not a wall', () => {
    const akamai = response({ headers: { server: 'AkamaiGHost' }, body: article });
    expect(classifyCapture(akamai)).toBe('page');
    expect(classifyCapture(response({ headers: { 'x-akamai-transformed': '9 - 0' }, body: article })))
      .toBe('page');
  });
