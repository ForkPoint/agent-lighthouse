...
describe('outcomeOf', () => {
  ...
  it('reads a scan-error stub as an error', () => {
    expect(outcomeOf(check({ status: 'na', tags: [TAG_SCAN_ERROR] }))).toBe('error');
  });

  it('reads a page-type stub as skipped', () => {
    expect(outcomeOf(check({ status: 'na', tags: [TAG_SKIPPED_PAGE_TYPE] }))).toBe('skipped');
  });
...
describe('traceFromCheck', () => {
  it('carries the verdict and what it contributed', () => {
    const trace = traceFromCheck(check({ status: 'fail', score: 0, weight: 0.6 }), 12);
...
describe('formatTrace', () => {
  it('names the audit, the outcome and the verdict', () => {
    const line = formatTrace(traceFromCheck(check({ status: 'fail', score: 0 }), 12));
  ...
  it('leaves the timing off an audit that never ran', () => {
    const line = formatTrace(traceFromCheck(check({ status: 'na', tags: [TAG_SKIPPED_PAGE_TYPE] }), 0));
    expect(line).toContain('skipped/na');
    expect(line).not.toContain('0ms');
  });

  it('appends the display value when there is one', () => {
    expect(formatTrace(traceFromCheck(check({ displayValue: '3 blocks' }), 1))).toContain('3 blocks');
  });
});
