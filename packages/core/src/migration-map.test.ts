...
describe("migration-map.json", () => {
  ...
  it("splits the 207 entries exactly 26 removed / 181 renamed", () => {
    ...
    for (const [, entry] of entries)
      census.set(entry.status, (census.get(entry.status) ?? 0) + 1);
  ...
  it.each(REMOVED_IDS)(
    "entry %s is removed with a sunset rationale link",
    (id) => {
      const entry = map[id]!;
  ...
  it("gives every surviving entry a v1 slug, a v2 target and a dossier link", () => {
    for (const [id, entry] of surviving) {
      ...
      expect(entry.link, `entry ${id} link`).toMatch(
        /^docs\/evidence\/audits\/.+\.md$/,
      );
    }
  });
  ...
  it('has no "merging" entries left — every fold has landed', () => {
    ...
    const merging = entries
      .filter(([, e]) => e.status === "merging")
      .map(([id]) => id);
    expect(merging).toEqual([]);
    const withInterim = entries
      .filter(([, e]) => e.interim !== undefined)
      .map(([id]) => id);
  ...
  it('points every "renamed" entry at a registered v2 audit', () => {
    ...
    const unregistered = surviving
      .filter(([, e]) => !registeredIds.includes(e.to!))
      .map(label);
    ...
    const allowed = surviving
      .filter(([, e]) => DOCUMENTED_DEFERRALS.includes(e.to!))
      .map(label);
  ...
  it("links every surviving entry to the dossier of the audit it resolves to", () => {
    for (const [id, entry] of surviving) {
      expect(entry.link, `entry ${id} link`).toBe(
        `docs/evidence/audits/${entry.to}.md`,
      );
    }
  });
});
