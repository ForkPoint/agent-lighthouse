...
function check(
  over: Partial<CheckResult> & { id: string; category: string },
): CheckResult {
  return {
    title: "t",
    description: "d",
...
describe("hydrateReport", () => {
  ...
  it("includes a category that has a score but no checks", () => {
    const r = hydrateReport(
      row({
        categoryScores: { "agent-interfaces": 80, "structured-data": 50 },
      }),
    );
  ...
  it("coerces Date timestamps to ISO strings", () => {
    ...
    const r = hydrateReport(
      row({ createdAt: created, completedAt: completed }),
    );
  ...
  it("passes through string timestamps and defaults missing ones", () => {
    const r = hydrateReport(
      row({ createdAt: null, completedAt: "2026-05-05T00:00:00.000Z" }),
    );
