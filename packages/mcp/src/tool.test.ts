...
import { AUDIT_TOOL, MAX_OPPORTUNITIES, buildAuditSummary, targetUrl } from './tool';
...
describe('buildAuditSummary', () => {
  ...
  it('reports the duration in seconds, not milliseconds', () => {
    expect(summarise(report({ durationMs: 12_340 })).durationSeconds).toBe('12.3');
  });
  ...
  it('carries each category status count', () => {
    const entry = summarise(report()).categories.find((c) => c.name === 'agent-interfaces');
  ...
  it(`caps the fix list at ${MAX_OPPORTUNITIES}, so a model is not handed all 215`, () => {
    const fails = Array.from({ length: 25 }, (_, i) => check({ id: `f${i}`, status: 'fail' }));
