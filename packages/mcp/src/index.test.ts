...
vi.mock('@forkpoint/agent-lighthouse-core', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@forkpoint/agent-lighthouse-core')>()),
  runScan,
}));
...
describe('auditWebsite', () => {
  ...
  it('returns every top fail, uncapped', async () => {
    ...
    expect(result.topFails[0]).toMatchObject({ id: 'f0', impact: 'impact', fix: 'fix' });
  });
  ...
  it('propagates a scan failure rather than returning a partial result', async () => {
    ...
    await expect(auditWebsite('https://shop.test/')).rejects.toThrow('host unreachable');
  });
});
