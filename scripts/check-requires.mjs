 ...
 *       evidence — come from the allowlist in `lib/requires-analysis.mjs`, not
...
import {
  ...
  declaredRequires,
  expectedRequires,
} from './lib/requires-analysis.mjs';
  ...
  for (const name of unknownGatherers) {
    violations.push(
      `${where}: imports gatherer '${name}', which is missing from GATHERER_EVIDENCE ` +
        'in scripts/lib/requires-analysis.mjs. Add it with the evidence its input needs.',
    );
  }
...
for (const id of registered.keys()) {
  if (!seen.has(id)) {
    violations.push(`${id}: registered, but no source file under packages/core/src/audits declares it.`);
  }
}
