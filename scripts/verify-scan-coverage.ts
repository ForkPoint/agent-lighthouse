...
function main(): void {
  ...
  console.log(
    `${stores.length} store(s): ${scanned.length} scanned, ${blocked.length} bot-blocked, ${failed.length} errored`,
  );
  console.log(
    `${expected.size} audits expected per scan (experimental tier excluded)\n`,
  );
  ...
  for (const v of verdicts) {
    for (const e of v.scanErrors) {
      const entry = errorsByAudit.get(e.id) ?? {
        stores: 0,
        example: e.explanation,
      };
      entry.stores += 1;
      errorsByAudit.set(e.id, entry);
    }
    for (const id of v.missing)
      missingByAudit.set(id, (missingByAudit.get(id) ?? 0) + 1);
  }
  ...
  if (errorsByAudit.size > 0) {
    ...
    for (const [id, { stores: n, example }] of [...errorsByAudit].sort(
      (a, b) => b[1].stores - a[1].stores,
    )) {
  ...
  if (missingByAudit.size > 0) {
    console.log(
      `MISSING — ${missingByAudit.size} audit(s) produced no result at all:`,
    );
  ...
  if (blocked.length > 0) {
    console.log(
      `Bot-blocked (not evidence either way): ${blocked.map((s) => s.url).join(", ")}\n`,
    );
  }
  if (failed.length > 0) {
    console.log("Errored:");
    for (const s of failed)
      console.log(`  ${s.url} — ${firstLine(s.error ?? "")}`);
    console.log();
  }

  const clean =
    errorsByAudit.size === 0 &&
    missingByAudit.size === 0 &&
    failed.length === 0;
  console.log(
    clean
