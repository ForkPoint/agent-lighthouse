...
async function main() {
  console.log(`Starting deep false-positive investigation on ${STORES.length} stores...\n`);
  ...
  for (const store of STORES) {
    ...
    try {
      ...
      if (report.wafProtection?.isBlocked) {
        console.log(`  🛡️ WAF: ${report.wafProtection.name} - ${report.wafProtection.reason}`);
      }
      
      const fails = report.recommendations.filter(r => r.status === 'fail');
      const warns = report.recommendations.filter(r => r.status === 'warn');
      console.log(`  Summary: ${report.topPasses.length} top passes, ${warns.length} warnings, ${fails.length} failures`);
    } catch (err) {
  ...
  fs.writeFileSync(path.join(outDir, 'stores-audit-data.json'), JSON.stringify(reports, null, 2));
