...
const dataPath = path.resolve(
  __dirname,
  "../reports/investigation/stores-audit-data.json",
);
const reports: Record<string, ScanReport> = JSON.parse(
    ...
    for (const check of cat.checks) {
      ...
      if (check.status === "warn") {
        ...
        checkStats[check.id].sampleExplanations.push(
          `[${storeName}] WARN: ${check.explanation || check.displayValue}`,
        );
      }
      if (check.status === "fail") {
        checkStats[check.id].failCount++;
        checkStats[check.id].sampleExplanations.push(
          `[${storeName}] FAIL: ${check.explanation || check.displayValue}`,
        );
      }
    }
  }
...
console.log(
  `POTENTIAL FALSE-POSITIVE / HARSH AUDIT CANDIDATES (>=7 stores failing/warning)`,
);
console.log(
  `========================================================================\n`,
);
...
for (const c of highFailureChecks) {
  ...
  console.log(
    `  Verdict: ${c.passCount} pass, ${c.warnCount} warn, ${c.failCount} fail`,
  );
