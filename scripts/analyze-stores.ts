...
const successStores = stores.filter(
  ([_, r]) => r.status === "success" && r.report,
);
...
if (blockedStores.length > 0) {
  ...
  for (const [domain, res] of blockedStores) {
    console.log(
      `  • ${domain}: ${res.waf || "Aggressive Bot Protection / Cloudflare Challenge / Akamai"}`,
    );
  }
}
...
if (successStores.length > 0) {
  const avgScore =
    successStores.reduce((acc, [_, r]) => acc + (r.score ?? 0), 0) /
    successStores.length;
  console.log(`\n--- SCORE STATISTICS ---`);
  console.log(`Average Overall Score: ${avgScore.toFixed(1)}/100`);

  const scores = successStores
    .map(([_, r]) => r.score ?? 0)
    .sort((a, b) => b - a);
  ...
  const lowestStores = [...successStores]
    ...
    for (const check of cat.checks) {
      ...
      if (check.status === "warn") {
        ...
        if (checkStats[check.id].sampleExplanations.length < 3) {
          checkStats[check.id].sampleExplanations.push(
            `[${domain}] WARN: ${check.explanation || check.displayValue}`,
          );
        }
      }
      if (check.status === "fail") {
        ...
        if (checkStats[check.id].sampleExplanations.length < 3) {
          checkStats[check.id].sampleExplanations.push(
            `[${domain}] FAIL: ${check.explanation || check.displayValue}`,
          );
        }
      }
    }
...
console.log(
  `AUDIT HEURISTIC ACCURACY & DISTRIBUTION ACROSS ${successStores.length} LIVE STORES`,
);
console.log(
  `========================================================================\n`,
);
...
const categories = Array.from(
  new Set(Object.values(checkStats).map((c) => c.category)),
);
for (const cat of categories) {
  ...
  for (const c of catChecks) {
    ...
    console.log(
      `  • [${c.id}] ${c.title.padEnd(45)} -> ${passPct}% pass | ${warnPct}% warn | ${failPct}% fail`,
    );
  }
}
