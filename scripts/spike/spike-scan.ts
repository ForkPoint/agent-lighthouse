...
interface SiteResult {
  ...
  error?: string;
  score: number | null;
  waf?: { provider?: string; isBlocked: boolean; statusCode?: number; isRateLimit?: boolean };
  pagesScanned: number;
  ...
  claimIds: string[];
  traces?: Array<{ id: string; category: string; status: string; score: number; weight: number }>;
}
...
async function scanOne(t: { url: string; kind: string }): Promise<SiteResult> {
  ...
  try {
    const report = await runScan(t.url, { onAuditTrace: (tr) => traces.push(tr) });
    res.ok = true;
    ...
    if (report.wafProtection) {
      res.waf = {
        ...
        isRateLimit: (report.wafProtection as { isRateLimit?: boolean }).isRateLimit,
      };
    }
  } catch (err) {
  ...
  for (const tr of traces) {
    res.totals[tr.outcome === 'ran' ? 'ran' : tr.outcome === 'skipped' ? 'skipped' : 'errored']++;
  ...
  res.traces = traces.map((t) => ({ id: t.id, category: t.category, status: t.status, score: t.score, weight: t.weight }));
  return res;
}
