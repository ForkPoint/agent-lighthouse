...
async function probe(url: string): Promise<Probe> {
  ...
  try {
    ...
    const noscriptJs =
      /enable\s+(javascript|js)|requires\s+javascript|doesn't work properly without javascript/i.test(
        noscriptText,
      );
    ...
    const scriptTextRatio =
      chars > 0 ? scriptBytes / chars : scriptBytes > 0 ? Infinity : 0;
    ...
    base.wouldGate =
      lowText && (emptyRoot || noscriptJs || scriptTextRatio > 50);
  } catch (err) {
