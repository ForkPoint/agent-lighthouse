...
const READS_PAGES = new Set(
  fs.readFileSync(`${__dirname}/reads-pages.txt`, 'utf8').split('\n').map((s) => s.trim()).filter(Boolean),
);
...
function categoryScore(traces: Trace[]): number | null {
  ...
  return Math.round((scored.reduce((s, t) => s + t.score * t.weight, 0) / total) * 100);
}

/** `calculateOverallScore`, restated. A category with no assessable check drops out. */
function overall(traces: Trace[]): { score: number; droppedCategories: string[] } {
  ...
  return { score: mass === 0 ? 0 : Math.round(weighted / mass), droppedCategories: dropped };
}

function main() {
  const sites: Site[] = JSON.parse(fs.readFileSync(`${__dirname}/results-traces.json`, 'utf8'));
  console.log('site                          pages  baseline  gated   delta  categories dropped');
  ...
  for (const s of sites) {
    ...
    console.log(
      `${s.url.padEnd(30)}${String(s.pagesScanned).padStart(4)}  ${String(before.score).padStart(8)}  ${String(after.score).padStart(5)}  ${(delta >= 0 ? '+' : '') + delta}`.padEnd(72) +
