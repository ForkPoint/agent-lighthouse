...
**Grade: D** — the annotation _concept_ is real in both MCP and WebMCP, but neither of the two places this audit reads it from exists in any specification: there is no `/.well-known/webmcp` manifest, and the declarative WebMCP proposal defines no `data-read-only-hint`-style attributes. The measured signal has no consumer.
