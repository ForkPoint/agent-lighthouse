---
audit: agent-interfaces/webmcp-tool-naming
audit_id: "5.23"
category: agent-interfaces
source_file: packages/core/src/audits/agent-interfaces/webmcp-tool-naming.ts
...
**Merged into:** `agent-interfaces/openapi-operation-ids` (Plan 4, 2026-08-22) — [merged dossier](../../audits/agent-interfaces/openapi-operation-ids.md). The naming rule folded in as a structural legality check; the English-verb allowlist, the 20-character description floor and the runtime (live MCP `tools/list`) half did not survive — that audit's dossier records each decision.
