...
**Evidence:** This is the path most widely repeated in blog posts and the one an audit tool is most likely to be asked for, and it does have real adopters — a live check on 2026-08-20 found that cloudflare.com, notion.so and sentry.io all return HTTP 200 JSON at /.well-known/mcp.json. That is genuine partial adoption by serious engineering organisations, which is why this is C and not D.
