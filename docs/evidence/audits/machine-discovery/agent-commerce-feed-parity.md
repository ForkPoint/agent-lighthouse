...
- **[Product data specification](https://support.google.com/merchants/answer/7052112)** — Google Merchant Center (vendor-doc, URL verified 2026-08-20)
  - Required attributes: id (max 50, unique), title (max 150), description (max 5000, 'should match landing page content'), link, image_link (500x500 minimum, enforced 2027-01-31), and brand (max 70). availability is drawn from {in_stock,out_of_stock,preorder,backorder} and 'must match the availability from your landing page'; price 'must match landing page and checkout prices'. gtin is strongly recommended, and mpn is required when there is no manufacturer GTIN. condition is required for used or refurbished items, and item_group_id is required for variants in BR/FR/DE/JP/UK/US.
