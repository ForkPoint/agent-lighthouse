...
**Two label-association defects both dossiers named are fixed.** The attribute-value escape regex escaped `:` and `]`, which need no escaping inside a quoted value, so `label[for="form:email"]` matched nothing and JSF/PrimeFaces and ASP.NET WebForms ids were reported unlabelled; only `"` and `\` are escaped now. And the two audits disagreed on scope — 5.22 searched the document, 5.27 searched inside the `<form>` — where HTML permits a `<label for>` anywhere in the document; the document-wide lookup wins, so a label in a sibling grid cell now counts.
