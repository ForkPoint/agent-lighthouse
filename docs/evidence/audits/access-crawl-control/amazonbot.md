...
**Grade: A** — Amazon publishes a crawler page naming the exact product token, stating its purpose — "Amazonbot is used to improve our products and services", and that content "may be used to train Amazon AI models" — and asserting robots.txt compliance for that user agent. That is documented consumer behaviour for the directive this audit reads, which is the grade-A bar. What Amazon does *not* publish is any statement of what blocking costs, so the audit reports the directive's state and does not argue a visibility case.
