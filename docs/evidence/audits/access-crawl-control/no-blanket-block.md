...
**Grade: B** — The answer-side half of the claim has a direct vendor statement behind it — OpenAI: "Sites that are opted out of OAI-SearchBot will not be shown in ChatGPT search answers", and Perplexity recommends allowing PerplexityBot "to ensure your site appears in search results" — and the traffic cost has been measured independently. What holds the grade at B is that this audit's claim is broader than any single vendor sentence: it is about the aggregate effect of a blanket block across a mixed population of training and answer agents, which no vendor documents and no study isolates. The counter-case is also real and recorded: blocking is a legitimate rights choice, and Cloudflare's crawl-to-refer data makes the economic argument for blocking training crawlers a serious one.
