...
**Grade: A** — Common Crawl publishes the exact user agent and the canonical opt-out snippet, and the downstream leverage is measured rather than asserted: the Data Provenance Initiative's "Consent in Crisis" audited 14,000 domains and found robots.txt restrictions had rendered "~5%+ of all tokens in C4, or 28%+ of the most actively maintained, critical sources in C4, fully restricted from use" within a single year. A documented token with a measured corpus effect is grade A.
