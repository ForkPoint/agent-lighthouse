...
Note the internal consistency this restores: the *same* TollBit finding capped Bytespider at C ("the consumer exists but the mechanism is empirically unreliable") while YouBot kept an A. One of the two stamps had to give.
...
- **"Allow" is not universally correct.** Blocking either scored bot is a defensible publisher choice; the audit's fix text says so and uses medium, never high, priority. What it penalises is an *unstated* policy inherited from `User-agent: *`.
