...
- `<meta name="robots">` **and** the per-bot forms (`googlebot`, `google-extended`, `gptbot`, `oai-searchbot`, `claudebot`, `applebot-extended`, `bingbot`, `perplexitybot`, …), read off the DOM so _all_ tags count rather than the last one;
...
1.13 adds the explicit Google chain for the `noindex` token itself — "`noindex` is used to prevent indexing content by search engines that support the `noindex` rule, such as Google", implementable as a meta tag, a per-bot meta tag, _or_ an `X-Robots-Tag` header — plus the precondition this audit still cannot see: "For the `noindex` rule to be effective, the page or resource must not be blocked by a robots.txt file."
