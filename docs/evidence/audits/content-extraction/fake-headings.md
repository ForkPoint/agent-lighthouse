...
**Grade: B** — Documented consumer behaviour exists on the retrieval side: LangChain's `HTMLHeaderTextSplitter` operates on `<h1>`, `<h2>` and `<h3>` and "adds metadata for each header 'relevant' to any given chunk", so a real heading element is what a chunker segments on and a styled `<div>` is not. That is a named consumer acting on the element, but the consumer is a library in a pipeline rather than an AI vendor's own documented behaviour, and no measurement attaches a magnitude — grade B. The falsifiable claim that survives is narrow: headings must exist and be real elements. The stricter claim that levels must never skip has no documented consumer at all; every splitter and snapshot cited records whatever level it finds.
