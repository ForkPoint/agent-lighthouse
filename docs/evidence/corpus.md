...
`w` or more _and_ whose character arm ends at `c` or more; either arm left
...
A `shell` fixture is flipped by a mutant that loosens _either_ arm below it.
...
mutant that flips it — word arm at 54 or more _and_ character arm at 321 or
...
may _claim_ — chiefly that a scan the gate could not feed reports no score.
