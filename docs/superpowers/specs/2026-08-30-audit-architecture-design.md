...
| 7 | Gate every URL taken from scanned content with `isSafeUrl()` | — | **DEBT** — nothing enforces it. 9 of 33 fetching audits skip it; the truthful figure is **8 of 36 network-reaching audits**, counting registered audits that can reach the network rather than files containing `ctx.fetch` |
| 8 | Bound what you fetch | — | **DEBT** — 36 audits reach the network: 31 call `ctx.fetch` in their own file and 5 fetch through the shared `_mcp-client.ts` helper. A verified count found 14 files with named top-level request limits; other bounds vary, and no shared scan budget sums or enforces them |
