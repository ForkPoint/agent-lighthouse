...
- Branch: `feat/v2-engine` (PR #10), pushed through `e4a4e13`; Plan 5b Wave A is committed locally and **not pushed** — the controller pushes after approval.
- Registry: **184 audits, 8 categories**, ids `category/slug`, evidence-mass scoring.
- Gates at HEAD: `pnpm test` 2842 passed / 0 failed / 208 skipped · `pnpm typecheck` clean · `rtk err pnpm lint` clean · `node scripts/check-dossiers.mjs` → "184 audits OK, no orphans" · `npx changeset status` all-major (core/report/cli/mcp → 2.0.0).
- migration-map.json: 207 entries — 26 removed, 181 renamed, **zero** merging/interim. `migration-map.test.ts` pins the census, extinction, dossier-link existence, registry cross-pin; `sunset.test.ts` pins the v1 roster; `new-in-v2.ts` carries the 36 ids added by Plans 5 and 5b.
...
Source: `packages/core/src/audits/proposed/` (**40 stubs left**, none registered) + `docs/evidence/proposals/`.
- Wave A is done: the 12 `agent-operability` + `injection-safety` stubs shipped into `operability-safety`. **36 feasible stubs remain across Waves B–D**, plus 3 infra-blocked and 1 deferred on the operator base URL.
...
- Intended grouping for the rest (user chose the four-wave split):
  - **Wave B** — token-economics (7) + answer-selection-forensics (5)
